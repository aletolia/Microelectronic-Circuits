...
使用刚刚概述的方案，我们看到为了实现低通功能，节点 $x$ 与地断开并连接到 $V_i$，如图 17.18(b) 所示。 该电路的传输零点将位于串联阻抗变为的 $s$ 值
...
无穷大（sL 在 s=∞ 处变为无穷大）并且分流阻抗变为零时的 s 值（1/[sC +(1/R)] 在 s=∞ 处变为零）。 因此，该电路在 s=∞ 处有两个传输零点，正如二阶 LP 所假设的那样。 传递函数可以通过检查或使用分压器规则来编写。 按照后一种方法，我们得到
