...
在第 7.3.4 节中，我们讨论了在 CS 放大器的源极引线中包含电阻 $R_s$ 时获得的一些好处，如图 8.22 所示。 这种电阻被称为源极衰减电阻，因为它的作用是将 CS 级的有效跨导降低到 $g_m/(1+g_mR_s)$，即减少因子 $(1+g_mR_s)$。 这也是许多性能参数增加的因素，例如线性度和带宽（将在第 10 章中看到）。 在这一点上，我们只想指出，我们上面推导出的 CG 放大器输出电阻表达式直接适用于源衰减 CS 放大器的情况。 这是因为当我们确定 $R_o$ 时，我们将输入端接地，使晶体管 $Q$ 表现为 CG 晶体管。 因此 Ro 由方程给出。  (8.56)，即
...
其次，回想一下 $Q_2$ 集电极中的电流信号将大约等于 $g_{m1} v_i$。 因此，共源共栅放大器输出的等效电路如图 8.37(b) 所示。 为了获得 $R_o$，我们使用等式中的公式。  (8.68) 并注意到 Q2 发射极的电阻 $R_e$ 为 $r_{o1}$，因此
