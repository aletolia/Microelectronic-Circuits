...
通常，$fT$的范围是100兆赫到几十兆赫。fT的值可用于公式（10.41）中，以确定Cπ + Cμ. 电容Cμ 通常通过在所需反向偏置电压VCB下测量基极和集电极之间的电容来分别确定。
从图10.14（a）的高频模型中可以观察到的一个重要现象是，频率在5到10F之间β , 人们可以忽略阻力rπ . 可以看出，rx成为高频输入阻抗中唯一的电阻部分。因此，rx在确定晶体管电路在高频下的频率响应方面起着重要的作用。因此，只有通过高频测量才能准确确定rx。
在离开本节之前，我们应该提到，图10.14中的高频模型相当准确地描述了晶体管在约0.2英尺的频率下的运行。
在更高的频率，人们必须添加其他寄生元素的模型。
