...
在二极管的众多应用中，它们在整流器（将交流电转换为直流电）设计中的使用最为常见。 因此，我们将详细研究整流器电路，并简要介绍一些其他二极管应用。 使用二极管和其他器件的更多非线性电路将在整本书中找到，尤其是在第 18 章中。

结二极管无非就是我们在第 3 章研究的 $pn$ 结，而本章大部分内容都是关于硅 $pn$ 结二极管的研究。 然而，在最后一节中，我们简要介绍了一些特殊的二极管类型，包括光电二极管和发光二极管。

### 4.1 The Ideal Diode 

#### 4.1.1 Current-Voltage Characteristic

理想二极管可以被认为是**最基本的非线性电路元件**。 它是具有图 4.1(a) 的电路符号和图 4.1(b) 所示的 $i-v$ 特性的两端器件。 理想二极管的终端特性可以解释如下：如果**==向二极管施加负电压（相对于图 4.1a 中指示的参考方向），则没有电流流过，二极管表现为开路==**（图 4.1a）。  4.1c)。 在这种模式下工作的二极管被称为反向偏置，或反向工作。 理想二极管在反向工作时电流为零，被称为被切断或简单地称作关闭。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220306202755422.png" alt="image-20220306202755422" style="zoom:80%;" />

另一方面，如果向理想二极管施加正电流（相对于图 4.1(a) 中所示的参考方向），则二极管两端出现零压降。换句话说，理想二极管表现为在正向短路（图 4.1d）；**==通过的任何电流产生的电压降均为零。==**一个正向偏置的二极管被称为导通状态，或简单地写作导通。

从上面的描述应该注意到，外部电路的设计应当可以将通过导通二极管的正向电流大小和截止二极管两端的反向电压大小限制在预定值。 图 4.2 显示了说明这一点的两个二极管电路。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220306203143599.png" alt="image-20220306203143599" style="zoom:80%;" />

在图 4.2(a) 的电路中，二极管显然是导通的。 因此，它的电压降为零，通过它的电流将由 $+10V$ 电源和 $1k$ 电阻确定为 $10 mA$。 图 4.2(b) 电路中的二极管明显被切断，因此其电流为零，这反过来意味着整个 $10V$ 电源将表现为二极管两端的反向偏置。

二极管的**==正极称为阳极，负极称为阴极==**，这是从真空管二极管时代遗留下来的。 理想二极管的 $i-v$ 特性（在一个方向上传导而不是在另一个方向上）应该解释其箭头状电路符号的选择。

从前面的描述可以看出，理想二极管的 $i-v$ 特性是高度非线性的。 虽然它由两条直线段组成，但它们彼此成 90°。 由直线段组成的非线性曲线称为分段线性。 如果在**==特定应用中使用具有分段线性特性的器件，其端子上的信号仅沿其中一个线性段摆动==**，则就该特定电路而言，该器件可被视为线性电路元件。 另一方面，如果信号摆动超过特性中的一个或多个断点，则不再可能进行线性分析

#### 4.1.2 A Simple Application: The Rectifier

二极管的基本应用是利用其严重非线性的 $i-v$ 曲线，例如图 4.3(a) 所示的整流电路。 该电路由二极管 $D$ 和电阻器 $R$ 串联组成。设输入电压 $v_I$ 为图 4.3(b) 所示的正弦波，并假设二极管是理想的。 在输入正弦波的正半周期内，正 $v_I$ 将导致电流沿正向流过二极管。 因此，**二极管电压 $v_D$ 将非常小——理想情况下为零**。 因此，电路将具有图 4.3(c) 所示的等效电路，输出电压 $v_O$ 将等于输入电压 $v_I$。 另一方面，在 <u>$v_I$ 的负半周期期间，二极管将不导通</u>。 因此，电路将具有图 4.3(d) 所示的等效电路，并且 $v_O$ 将为零。 因此，输出电压将具有图 4.3(e) 所示的波形。 请注意，**==虽然 $v_I$ 极性交替且平均值为零，但 $v_O$ 是单向的并且具有有限平均值或直流分量。==**

因此，图 4.3(a) 的电路对信号进行整流，因此称为整流器。 它可<u>**用于从交流电产生直流电**</u>。 我们将在 4.5 节研究整流电路。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220306203925198.png" alt="image-20220306203925198" style="zoom:80%;" />

#### ![image-20220306204413646](C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220306204413646.png)Example 4.1

图 4.4(a) 显示了为 $12V$ 电池充电的电路。 如果 $v_S$ 是峰值幅度为 $24V$ 的正弦曲线，求出二极管导通的每个周期的分数。 另外，求二极管电流的峰值和二极管两端出现的最大反向偏置电压。

![image-20220306204701840](C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220306204701840.png)**Solution** 

当 $v_S$ 超过 $12 V$ 时，二极管导通，如图 4.4(b) 所示。 导通角为 $2θ$，其中 $θ$ 由下式给出
$$
24\cos\theta=12
$$
因此 $θ = 60°$，导通角为 $120°$，或三分之一周期。二极管电流的峰值由下式给出
$$
I_d=\frac{24-12}{100}=0.12A
$$
二极管两端的最大反向电压出现在 $v_S$ 处于其负峰值且等于 $24+12 = 36 V$ 时。

#### 4.1.3 Another Application: Diode Logic Gates

**==二极管与电阻器一起可用于实现数字逻辑功能==**。 图 4.5 显示了两个二极管逻辑门。 要了解这些电路的功能，请考虑一个正逻辑系统，其中接近 $0 V$ 的电压值对应于逻辑 $0$（或低），接近 $+5 V$ 的电压值对应于逻辑 $1$（或高）。 图 4.5(a) 中的电路有三个输入，$v_A$、$v_B$ 和 $v_C$。 很容易看出，连接到 $+5V$ 输入的二极管将导通，**==从而将输出 $v_Y$ 钳位到等于 $+5 V$ 的值==**。输出端的这个正电压将保持输入低（约 $0 V$）的二极管隔断。 因此，**如果一个或多个输入为高，则输出将为高。 因此，该电路实现了逻辑或功能，在布尔符号中表示为**
$$
Y=A+B+C
$$
图 4.5b 展示了一个实现以下逻辑关系的电路，我们鼓励读者自行证明其原理
$$
Y= A\cdot B\cdot C
$$
<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220306210012284.png" alt="image-20220306210012284" style="zoom:80%;" />

#### Example 4.2

假设二极管是理想的，电路如图 4.6 所示，计算 $I$ 和 $V$ 的值。

![image-20220306210327495](C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220306210327495.png)**Solution**

在这些电路中，乍一看可能并不明显，是否没有一个、另一个或两个二极管都在导电。==**对于这种情况下，我们一般会先做出一个合理的假设，继续分析，然后检查我们是否最终得到了一致的解决方案。**== 对于图 4.6(a) 中的电路，我们首先**<u>假设两个二极管都导通</u>**。 因此 $V_B$ 以及 $V = 0$。通过 $D_2$ 的电流现在可以由下式确定
$$
...
I+1=\frac{0-(-10)}{5}
$$
结果 $I = 1 mA$。 **<u>因此 $D_1$ 像最初假设的那样导通</u>**，最终结果是 $I = 1 mA$ 和 $V = 0 V$。

对于图 4.6(b) 中的电路，**<u>如果我们假设两个二极管都导通</u>**，则 $V_B = 0$ 和 $V = 0$。$D_2$ 中的电流由下式获得
$$
...
I+2=\frac{0-(-10)}{10}
$$
这表明 $I = -1 mA$。 由于这是不可能的，因此我们最初的假设是不正确的。让我们重新开始，**<u>假设 $D_1$ 反偏，$D_2$ 正偏</u>**。 当前 $I_{D2}$ 由下式给出
$$
I_{D2}=\frac{10-(-10)}{15}=1.33mA
$$
$B$ 点的节点电压方程为
$$
V_B=-10+10\times1.33=+3.3V
$$
因此 $D_1$ 如所假设的那样反向偏置，最终结果是 $I = 0$ 和 $V = 3.3 V$。

### 4.2 Terminal Characteristics of Junction Diodes

二极管最常见的实现方式是使用 $pn$ 结。 我们在第 3 章研究了 $pn$ 结的物理特性并推导出了它的 $i-v$ 特性。$pn$ 结用于实现二极管功能应该不足为奇：$pn$ 结可以正向传导大量电流，并且几乎 反方向没有电流。 在本节中，我们将详细研究 $pn$ 结二极管的 $i-v$ 特性，以便为二极管电路应用做好准备。 图 4.7 显示了硅结二极管的 $i-v$ 特性。 图 4.8 显示了相同的特征，其中一些尺度被扩大，另一些被压缩以显示细节。 请注意，比例变化导致原点明显不连续。 如图所示，特征曲线由三个不同的区域组成：

![image-20220306215318560](C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220306215318560.png)
...
当端电压 $v$ 为正时，进入正向偏置（或简称为正向）工作区域。 在前向区域中，$i-v$ 关系非常近似于
$$
i=I_s(e^{v/V_T}-1)\tag{4.1}
$$
在这个等式中，**$I_S$ 是给定二极管在==给定温度==下的常数。**根据二极管的物理参数和温度计算 IS 的公式在方程式 (3.41)中给出。**<u>$I_S$ 通常称为饱和电流</u>**（原因将很快变得显而易见）。 $I_S$ 的另一个名称，也是我们偶尔使用的名称，是标度电流。**==此名称源于 $I_S$ 与二极管的横截面积成正比这一事实。==**因此，**结面积加倍会导致二极管的 $I_S$ 值加倍**，正如二极管方程所示，对于给定的正向电压 $v$，电流 $i$ 的值加倍。对于小尺寸的“小信号”二极管（用于低功率应用的二极管），$I_S$ 约为 $10^{-15} A$。然而，**==$I_S$ 的值是关于温度的一个非常强的函数==**。根据经验，温度每升高 $5°C$，$I_S$ 值就会翻倍。方程式 (4.1)中的电压 $V_T$ 是称为热电压的常数，由下式给出
$$
V_T=\frac{kT}q\tag{4.2}
$$

> 注：方程（4.1），二极管方程，有时写成在指数中包含一个常数 $n$，
> $$
> i = I_S(e^{v/nV_T} -1)
> $$
>
> $n$ 的值介于 1 和 2 之间，具体取决于二极管的材料和物理结构。 使用标准集成电路制造工艺的二极管在正常条件下工作时表现出 $n=1$。 为简单起见，除非另有说明，否则我们将在本书中使用 $n = 1$。

其中

- $k =$ 玻尔兹曼常数 $= 8.62×10^{−5} eV/K = 1.38×10^{−23}$ 焦耳/开尔文 
- $T =$ 绝对温度（开尔文）$= 273+$ 温度（°C） 
- $q =$ 电荷量 $= 1.60×10^{−19}$ 库仑

将 $k = 8.62×10^{-5} eV/K$ 代入方程式 (4.2) 给出
$$
V_T=0.0862T,mV\tag{4.2a}
$$
因此，在室温 (20°C) 下，$V_T$ 的值为 $25.3 mV$。 在快速近似电路分析中，**我们将在室温下使用 $V_T\simeq 25 mV$。==对于正向电流 $i$，特别是 $i\gg I_S$，==Eq.  (4.1) 可以近似为指数关系**

> 注：对于在机柜内运行的电子设备，通常假设环境温度略高（25°C 左右）。 在此温度下，$V_T$ 为 25.8 mV。 尽管如此，为了简单起见和促进快速的电路分析，我们将在本书中使用 $V_T \simeq25 mV$ 这个在算术上更方便的值。

$$
i\simeq I_Se^{v/V_T}\tag{4.3}
$$
...
$$
其中 $\ln$ 表示自然（以 $e$ 为底）对数。

电流 $i$ 与电压 $v$ 的这种指数关系即使在电流增大数十倍后也仍然正确（已经发现即使在 $10^7$ 倍中也正确）。 这是结型二极管的一个非常显着的特性，这种特性也出现在双极结型晶体管中，并且已在许多有趣的应用中得到利用。

让我们考虑方程式 (4.3) 中的前向 $i-v$ 关系，并计算对应于二极管电压 $V_1$ 的电流 $I_1$： 
$$
I_1=I_Se^{V_1/V_T}
$$
同样，如果电压为 $V_2$，则二极管电流 $I_2$ 为
...
$$
V_2-V_1=2.3V_T\lg\frac{I_2}{I_1}\tag{4.5}
$$
该等式简单说明，**对于电流的10 倍变化，二极管电压降变化 $2.3V_T$，约为 60 mV。 这也表明二极管 $i-v$ 关系<u>==最方便地绘制在半对数纸上==</u>**。 使用 $v$ 的垂直线性轴和 $i$ 的水平对数轴，可以获得每十倍电流斜率为 $60 mV$ 的直线。

看一眼正向区域中的 $i-v$ 特性（图 4.8）可以发现，==**当 $v$ 小于约 $0.5 V$ 时，电流可以忽略不计。这个值通常被称为切入电压。**== 然而，应该强调的是，特性中的这个明显阈值仅仅是指数关系的结果。 这种关系的**另一个结果是 $i$ 的迅速增加**。 因此，对于“全导通”二极管，**电压降位于一个狭窄的范围内，大约为 0.6 V 到 0.8 V。**这就产生了一个简单的二极管“模型”，**假设导通二极管的电压约为 0.7V**。具有不同额定电流（即不同面积和相应不同 $I_S$）的二极管**在不同电流下会表现出 0.7V 的压降。** 例如，可以认为小信号二极管在 $i = 1 mA$ 时具有 $0.7V$ 压降，而更高功率二极管在 $i = 1A$ 时可能具有 $0.7V$ 压降。我们将研究二极管的主题 - 下一节的电路分析和二极管模型

由于 $I_S$ 和 $V_T$ 都是温度的函数，**因此==正向== $i-v$ 特性随温度而变化**，如图 4.9 所示。 **在给定的恒定二极管电流下，温度每升高 1°C，二极管两端的电压降大约降低 2 mV**。二极管电压随温度的变化已被用于电子温度计的设计 

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220307114008969.png" alt="image-20220307114008969" style="zoom:80%;" />

#### 4.2.2 The Reverse-Bias Region

当二极管电压 $v$ 为负时，进入反向偏置工作区。等式 (4.1) 预测，如果 $v$ 为负且幅度比 $V_T (25 mV)$ 大几倍，则指数项与单位数相比变得可以忽略不计，二极管电流变为
$$
i\simeq-I_S
$$
**即反向电流是恒定的，等于$I_S$**。 这种恒定性是术语饱和电流背后的原因。

真正的二极管表现出的反向电流虽然很小，但比 $I_S$ 大得多。例如，$I_S$ 为 $10^{-14} A$ 到 $10^{-15} A$ 数量级的小信号二极管可以显示大约 $1 nA$ 的反向电流。 **<u>反向电流也随着反向电压幅度的增加而有所增加。</u>** 请注意，由于电流幅度非常小，这些细节在图 4.8 的二极管 $i-v$ 特性上并不明显。

==**很大一部分反向电流是由于泄漏效应造成的**==。 这些漏电流与结面积成正比，就像 $I_S$ 一样。 然而，它们<u>对温度的依赖性与 $I_S$ 不同</u>。 因此，虽然温度每升高 5°C，$I_S$ 翻倍，但反向电流与温度相关的相应经验法则是，温度每升高 10°C，它就会翻倍。

#### 4.2.3 The Breakdown Region

二极管工作的第三个不同区域==**是击穿区域**==，可以在图 4.8 中的二极管 $i-v$ 特性上轻松识别。 当**==反向电压的幅度超过特定于特定二极管的阈值==**（称为击穿电压）时，进入击穿区域。 这是图 4.8 中 $i-v$ 曲线的“拐点”处的电压，用 $V_{ZK}$ 表示，其中下标 $Z$ 代表齐纳二极管（参见第 3.5.3 节），$K$ 表示拐点。

从图 4.8 可以看出，在击穿区域，**反向电流迅速增加**，而相关的电压降增加非常小。 二极管击穿通常不会造成破坏，只要二极管中消耗的功率被外部电路限制在“安全”水平。 该安全值通常在器件数据表中指定。 因此，有必要将击穿区的反向电流限制在与允许的功耗一致的值。二==**极管 $i-v$ 击穿特性几乎是一条垂直线，这一事实使其可用于电压调节。**== 本主题将在第 4.5 节中进行研究。

### 4.3 Modeling the Diode Forward Characteristic

研究了二极管端子特性后，我们现在准备考虑分析采用正向导通二极管的电路。 图 4.10 显示了这样一个电路。 它由一个直流电源 $V_{DD}$、一个电阻器 $R$ 和一个二极管组成。我们希望分析这个电路以确定二极管电压 $V_D$ 和电流 $I_D$。 为了帮助我们的分析，**==我们需要用模型来表示二极管。==**

二极管模型有很多种，我们现在知道的有两种：理想二极管模型和指数模型。 在下面的讨论中，我们将评估这两种模型在各种分析情况下的适用性。 此外，我们将开发和评论其他模型。该材料除了可用于二极管电路的分析和设计外，还为我们将在接下来的三章中研究的晶体管操作建模奠定了基础。

![image-20220307120011243](C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220307120011243.png)

#### 4.3.1 The Exponential Model

指数模型提供了正向区域中二极管工作的最准确描述。 然而不幸的是，其严重的非线性特性使得该模型最难使用。 为了说明，让我们使用指数二极管模型分析图 4.10 中的电路。

假设 $V_{DD}$ 大于 0.5 V 左右，二极管电流会远大于 $I_S$，我们可以用指数关系来表示二极管 $i-v$ 特性，
$$
I_D=I_Se^{V_D/V_T}\tag{4.6}
$$
控制电路操作的另一个方程是通过编写一个基尔霍夫回路方程获得的，这会得到

$$
I_D=\frac{V_{DD}-V_D}R\tag{4.7}
$$
假设二极管参数 $I_S$ 已知，方程式 (4.6) 和 (4.7) 是两个未知量 $I_D$ 和 $V_D$ 中的两个方程。 **获得解决方案的两种替代方法是图形分析和迭代分析。**

#### 4.3.2 Graphical Analysis Using the Exponential Model

通过在 $i-v$ 平面上**==绘制方程式 (4.6) 和 (4.7) 的关系进行图形分析==**，然后可以将**解作为两个图的交点的坐标**来获得。 图形结构的草图如图 4.11 所示。 曲线表示指数二极管方程（方程 4.6），**直线表示方程 (4.7)，这样的直线被称为==负载线==**，这个名称在后面的章节中会变得更有意义。 **负载线在 $Q$ 点<u>与二极管曲线相交</u>，该点代表电路的==<u>工作点</u>==**。 **它的坐标给出了 $I_D$ 和 $V_D$ 的值。**

图形分析有助于电路操作的可视化。 然而，执行这种分析所涉及的工作量，特别是对于复杂电路，在实践中由于大量的工作量显得不太可行

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220307123958855.png" alt="image-20220307123958855" style="zoom:80%;" />

#### 4.3.3 Iterative Analysis Using the Exponential Model

方程 (4.6) 和 (4.7) 可以使用简单的迭代过程求解，如下例所示。

#### Example 4.4

确定图 4.10 中电路的电流 $I_D$ 和二极管电压 $V_D$ ，其中 $V_{DD} =5 V$ 和 $R=1 k$ 。假设二极管在电压为 $0.7 V$ 时电流为 $1 mA$。

**Solution**

为了开始迭代，我们假设 $V_D = 0.7 V$ 并使用方程式 (4.7) 确定电流，

$$
\begin{aligned}I_D=\frac{V_{DD}-V_D}{R}\\=\frac{5-0.7}1=4.3mA\end{aligned}
$$
然后，我们使用二极管方程来获得对 $V_D$ 的更好估计。 这可以通过使用方程式来完成 (4.5)，即，
...
$$
代入 $V_1 = 0.7 V、I_1 = 1 mA$ 和 $I_2 = 4.3 mA$ 得到 $V_2 = 0.738 V$。因此第一次迭代的结果是 $I_D = 4.3 mA$ 和 $V_D = 0.738 V$。第二次迭代以类似的方式进行
$$
I_D=\frac{5-0.738}1=4.262mA
$$

$$
\begin{aligned}V_2=0.738+0.06\log\left[\frac{4.262}{4.3}\right]\\=0.738V\end{aligned}
$$

因此第二次迭代得到 $I_D =4.262 mA$ 和 $V_D =0.738 V$。由于这些值非常接近第一次迭代后获得的值，因此无需进一步迭代，解为 $I_D =4.262 mA$ 和 $V_D =0.738 V$。

#### 4.3.4 The Need for Rapid Analysis

上面示例中使用的迭代分析过程很简单，并且在两次或三次迭代后产生准确的结果。 然而，在某些情况下，所需的努力和时间仍然超出了合理的范围。 具体来说，如果要对相对复杂的电路进行纸笔设计，则必须进行快速电路分析。 通过快速分析，设计人员能够在决定合适的电路设计之前评估各种可能性。 为了加快分析过程，人们必须满足于不太精确的结果。 然而，这很少成为问题，因为更准确的分析可以推迟到获得最终或几乎最终的设计。 可以借助 SPICE 等计算机电路分析程序对几乎最终的设计进行准确分析（参见附录 B 和网站）。 然后可以使用这种分析的结果来进一步细化或“微调”设计。

为了加快分析过程，我们必须找到一个更简单的二极管正向特性模型。

#### 4.3.5 The Constant-Voltage-Drop Model

最简单和最广泛使用的二极管模型是**==恒压降模型==**。 该模型基于以下观察：正向导通二极管的电压降在相对较窄的范围内变化，例如 0.6 到 0.8 V。**该模型假设该电压恒定在某个值**，例如 0.7 V，如图 4.12 所示。

恒压降模型是分析和设计初始阶段最常用的模型。 如果在这些阶段没有关于二极管特性的详细信息，我们经常简单的假设其电压降为 0.7V

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220307140411935.png" alt="image-20220307140411935" style="zoom:80%;" />

最后，注意如果我们使用恒压降模型来解决例 4.4 中的问题，我们得到
$$
V_D=0.7V
$$
以及

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220307140756841.png" alt="image-20220307140756841" style="zoom:67%;" />

这与之前使用更精细的指数模型获得的值没有太大不同。

#### 4.3.6 The Ideal-Diode Model

在**涉及电压远大于二极管压降** (0.6 V–0.8 V) 的应用中，我们可以在计算二极管电流时**完全忽略**二极管压降。结果就是我们在第 4.1 节中研究的理想二极管模型。 对于例 4.4 中的电路（即图 4.10，$V_{DD} =5 V$ 和 $R=1 k$），使用理想二极管模型会导致 

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220307140928297.png" alt="image-20220307140928297" style="zoom:67%;" />

对于一个非常快速的分析来说，这作为一个粗略的估计并不坏。 然而，几乎没有额外的工作，0.7V 压降模型产生了更现实的结果。 然而，我们注意到，**理想二极管模型的最大用途在于确定多二极管电路中哪些二极管是导通的，哪些是关断的，例如第 4.1 节中考虑的那些。**

#### 4.3.7 The Small-Signal Model

考虑图 4.13(a) 中的情况，其中直流电压 $V_{DD}$ 通过电阻 $R$ 和二极管 $D$ 的串联组合建立直流电流 $I_D$。产生的二极管电压表示为 $V_D$。 如上所述，$I_D$ 和 $V_D$ 的值可以通过使用二极管指数特性求解电路来获得，或者更快地，可以使用二极管恒压降模型找到近似值

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220307141817969.png" alt="image-20220307141817969" style="zoom:80%;" />

接下来，**考虑 $V_{DD}$ 发生==微小变化== $\Delta V_{DD}$ 的情况**，如图 4.13(b) 所示。 如图所示，电流 $I_D$ 变化一个增量 $\Delta I_D$，二极管电压 $V_D$ 变化一个增量 $\Delta V_D$。 我们希望找到一种快速的方法来确定这些增量更改的值。 为此，我们为二极管开发了一个**<u>“小信号”</u>**模型。

这里的信号一词强调，一般来说，$\Delta V_{DD}$ 可以是一个时变量。 限定词“小”表示该二极管模型**仅适用于 $V_D$ 保持足够小的情况**，“足够”可以很快量化。

要开发二极管小信号模型，请参见图 4.14。 我们将二极管两端的电压表示为直流电压 $V_D$ 和时变信号 $v_d(t)$ 之和，

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220307142654217.png" alt="image-20220307142654217" style="zoom:80%;" />
$$
v_D(t) = V_D +v_d(t)\tag{4.8}
$$
相应地，总瞬时二极管电流 $i_D(t)$ 将为
$$
i_D(t) = I_Se^{v_D /V_T}\tag{4.9}
$$
用方程式 (4.8) 代替 $v_D$，得到
$$
i_D(t) = I_Se^{(V_D +v_d )/V_T}
$$
可以重写为
$$
i_D (t) = I_Se^{V_D / V_T} e^{v_d / V_T}\tag{4.10}
$$
**在没有信号 $v_d(t)$ 的情况下**，二极管电压等于 $V_D$，**二极管电流为 $I_D$**，由下式给出
$$
I_D = I_Se^{V_D /V_T}\tag{4.11}
$$
因此，方程 (4.10) 中的 $i_D(t)$ 可以表示为
$$
i_D(t) = I_De^{v_d /V_T}\tag{4.12}
$$
现在，如果信号 $v_d(t)$ 的幅度保持足够小，使得
$$
\frac{v_d}{V_T}\ll 1\tag{4.13}
$$
那么我们可以扩展等式（4.12）的指数，在一个级数中并截断前两项后的级数，得到近似表达式
$$
i_D(t)\simeq I_D\left( 1+ \frac{v_d} {V_T}\right)\tag{4.14}
$$
这即是小信号近似，它适用于幅度小于约 $5 mV$ 的信号（参见公式 4.13，并回想 $V_T = 25 mV$）。

> 对于 $v_d = 5 mV$，$v_d /V_T = 0.2$。 因此，指数级数展开的下一项将是 $\frac1 2 ×0.2^2 = 0.02$，比我们保留的线性项低 10 倍。

从方程式 (4.14) 我们有
$$
i_D(t) = I_D + \frac{I_D} {V_T} v_d\tag{4.15}
$$
因此，叠加在直流电流 $I_D$ 上，我们有一个**与信号电压 $v_d$ 成正比的信号电流分量**，那是，
$$
i_D = I_D + i_D\tag{4.16}
$$
其中
$$
i_d = \frac{I_D} {V_T} v_d\tag{4.17}
$$
信号电流 $i_d$ 与信号电压 $v_d$ 相关的量，其量纲为电导 mhos ( )，称为**二极管小信号电导**。 此参数的**倒数是二极管小信号电阻或增量电阻 $r_d$，**
$$
r_d=\frac{V_T}{I_D}\tag{4.18}
$$
请注意，$r_d$ 的值与**偏置电流 $I_D$** 成反比。

通过再次考虑图 4.14 中的图形结构，可以进一步了解小信号近似和小信号二极管模型。 在这里，可以看到二极管工作在**直流偏置点 $Q$**，其特征是直流电压 $V_D$ 和相应的直流电流 $I_D$。 叠加在 $V_D$ 上，我们有一个信号 $v_d(t)$，假设（任意）具有三角波形。

很容易看出，**<u>使用小信号近似等效于假设信号幅度足够小，使得沿 $i-v$ 曲线的==偏移被限制在一个短的几乎线性段内==。</u>** 该段的<u>斜率等于**工作点 $Q$ 处 $i-v$ 曲线的切线斜率**</u>，等于小信号电导。鼓励读者证明 $i-v$ 曲线在 $i = I_D$ 处的斜率等于 $I_D/V_T$ ，即 $1/r_d$ ； 那是，
$$
r_d=1/\left[\frac{\partial i_D}{\partial v_D}\right]_{i_D-I_D}
$$
从前面我们得出结论，**叠加**在定义二极管直流偏置点或静态点的量 $V_D$ 和 $I_D$ 上的是与二极管相关的小信号量 $v_d(t)$ 和 $i_d(t)$ 在偏置点所计算出的小信号电阻 $r_d$ (Eq. 4.18)。 **因此，小信号分析可以与直流偏置分析分开执行**，这是由于小信号近似中固有的二极管特性的线性化带来的极大便利。 具体来说，在进行直流分析后，通过消除所有直流源（即短路直流电压源和开路直流电流源）<u>**==并用二极管的小信号电阻代替二极管==**</u>，得到小信号等效电路。 因此，对于图 4.13(b) 中的电路，使用图 4.13(a) 中的电路得到直流分析，**而增量 $\Delta I_D$ 和 $\Delta V_D$ 可以使用小信号等效电路确定**，如图 4.15 所示。 下面的例子应该进一步说明小信号模型的应用。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220307150950421.png" alt="image-20220307150950421" style="zoom:80%;" />

<u>**==直流分析 $\to$ 确定工作点 $\to$ 确定等效电阻 $\to$ 消除直流源 $\to$ 小信号分析==**</u>

 图 4.15 确定图 4.13(b) 中电路的增量量 $\Delta I_D$ 和 $\Delta V_D$ 的电路。请注意，**用其小信号电阻 $r_d$ 替换二极管会<u>产生线性电路</u>。**

#### Example 4.5

对于 $R = 10 k$ 的情况，考虑图 4.16(a) 所示的电路。 电源 $V^+$ 的直流值为 $10 V$，其上叠加有 1 V 峰值幅度的 $60 Hz$ 正弦波。  （电源电压的这个“信号”分量是电源设计中的一个缺陷。它被称为电源纹波。稍后会详细介绍。）计算出现在二极管的直流电压和 正弦波信号它上面。 假设二极管在 $1mA$ 电流下有 $0.7V$ 压降。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220308101045415.png" alt="image-20220308101045415" style="zoom:80%;" />

**Solution**

仅考虑直流量，我们假设 $V_D\simeq0.7 V$ 并计算二极管直流电流

$$
I_D = \frac{10−0.7} {10} = 0.93 mA
$$
由于该值非常接近 $1 mA$，因此二极管电压将非常接近假设值 $0.7 V$。在此工作点，二极管增量电阻 $r_d$ 为
$$
r_d = \frac{V_T} {I_D} = \frac{25} {0.93} = 26.9\Omega
$$
二极管两端的信号电压可以从图 4.16(c) 的小信号等效电路中找到。这里 $v_s$ 表示 $V^+$ 的 $60Hz- 1V$ 峰值正弦分量，$v_d$ 是二极管两端的相应信号。 使用分压器规则可提供 $v_d$ 的峰值幅度如下： 
$$
v_d(peak)=V_t\frac{r_d}{R+r_d}=2.68mV
$$
最后，我们注意到由于这个值非常小，我们使用二极管的小信号模型是合理的。

由上可知，对于**==同时涉及直流量和信号量的二极管电路，去掉直流源，将每个二极管换成小信号电阻 $r_d$ 即可得到小信号等效电路。==** 这样的电路是线性的，可以使用线性电路分析来解决。

最后，我们注意到虽然 $r_d$ 模拟了**二极管在低频下**的小信号操作，但它的动态操作由我们在第 3.6 节中研究的电容 $C_j$ 和 $C_d$ 建模，它们也是小信号参数，二极管的完整模型包括与 $r_d$ 并联的 $C_j$ 和 $C_d$。

#### 4.3.8 Use of the Diode Forward Drop in Voltage Regulation

二极管小信号模型的进一步应用是在一个流行的二极管应用中，即**使用二极管来产生稳压。** 稳压器是一种电路，其目的是在其**输出端子之间提供恒定的直流电压**。 尽管 (a) 从稳压器输出端子汲取的负载电流发生变化，以及 (b) 为稳压器电路供电的直流电源电压发生变化，但输出电压仍需尽可能保持恒定。 由于二极管的正向压降几乎保持恒定在大约 0.7 V，而通过它的电流变化相对较大，<u>因此正向偏置二极管可以构成一个简单的稳压器</u>。 例如，我们在示例 4.5 中看到，虽然 10 V 直流电源电压具有 2 V 峰间纹波（±10% 变化），但二极管电压中的相应纹波仅为约 ±2.7 mV  （±0.4% 的变化）。 **<u>通过串联多个二极管可以获得大于 0.7 V 的稳压电压，例如，使用三个串联的正向偏置二极管可提供大约 2 V 的电压。</u>**以下示例中研究了一个这样的电路，它利用二极管小信号模型来量化实现的稳压器的功效 .

##### Example 4.6

考虑图 4.17 所示的电路。 一串三个二极管用于提供约 2.1 V 的恒定电压。我们要计算由 (a) 电源电压的 ±10% 变化和 (b) 连接 1-k 负载电阻所引起的稳压值变化。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220308103219957.png" alt="image-20220308103219957" style="zoom:80%;" />

**Solution**

空载时，二极管串中电流的标称值由下式给出

$$
I = \frac{10-2.1} 1 = 7.9 mA
$$
因此每个二极管将有一个增量电阻
...
r = 3r_d = 9.6\Omega
$$
该电阻与电阻 $R$ 一起构成一个分压器，其比率可用于计算由于电源电压的 $±10\%$（即 ±1-V）变化而导致的输出电压变化。 因此，输出电压的峰峰值变化将是
$$
\Delta v_O = 2 \frac r {r +R} = 2 \frac{0.0096} {0.0096+1} = 19 mV
$$
也就是说，对应于电源电压的 $±1V (±10\%)$ 变化，输出电压将变化 $±9.5 mV$ 或 $±0.5\%$，由于这意味着每个二极管的变化约为 ±3.2 mV，因此我们使用小信号模型是合理的。

当二极管串上连接一个 1 k 的负载电阻时，它会吸收大约 2.1 mA 的电流。 因此，二极管中的电流减少了 2.1 mA，导致二极管串上的电压降低，由下式给出
$$
\Delta v_O = -2.1 × r = -2.1 × 9.6 = -20 mV
$$
由于这意味着每个二极管两端的电压降低了约 6.7 mV，因此我们使用小信号模型并不完全合理。 然而，使用指数模型对电压变化的详细计算得出 $v_O = -23 mV$，这与使用增量模型获得的近似值相差不大

### 4.4 Operation in the Reverse Breakdown Region Zener Diodes

二极管在击穿区域表现出的**<u>非常陡峭的 $i-v$ 曲线（图 4.8）和几乎恒定的电压降</u>**表明在击穿区域工作的二极管可用于电压调节器的设计。 在上一节中，读者会记得<u>==稳压器是在负载电流和系统电源电压发生变化时提供恒定直流输出电压的电路==</u>。 事实上，这成为**工作在反向击穿区**的二极管的一个重要应用，并且制造了专门在击穿区工作的特殊二极管。 这种二极管称为击穿二极管，或更常见的是，如前所述，齐纳二极管。

图 4.18 显示了齐纳二极管的电路符号。 在齐纳二极管的正常应用中，电流流入阴极，阴极相对于阳极为正。因此图 4.18 中的 $I_Z$ 和 $V_Z$ 具有正值

![image-20220308104557409](C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220308104557409.png)

#### 4.4.1 Specifying and Modeling the Zener Diode

图 4.19 显示了击穿区域中二极管 $i-v$ 特性的详细信息。 我们观察到，对于大于**拐点电流 $I_{ZK}$**（在齐纳二极管的数据表中指定）的电流，$i-v$ 特性几乎是一条直线。 制造商通常会在指定的测试电流 $I_{ZT}$ 下指定齐纳二极管 $V_Z$ 两端的电压。 我们在图 4.19 中将这些参数表示为标记为 $Q$ 的点的坐标。因此，<u>一个 6.8V 的齐纳二极管将在指定的测试电流（例如 10mA）下表现出 6.8V 的压降</u>。 当通过齐纳管的电流偏离 $I_{ZT}$ 时，其两端的电压会发生变化，尽管变化很小。 图 4.19 显示，对应于电流变化 $\Delta I$，齐纳电压变化 $\Delta V$，并且与 $\Delta I$ 相关
$$
\Delta V = r_z\Delta I
$$
其中 $r_z$ 是几乎线性 $i-v$ 曲线在 $Q$ 点的斜率的倒数。电阻 $r_z$ 是齐纳二极管在工作点 $Q$ 的增量电阻。它也称为齐纳二极管的动态电阻，其值在器件数据手册中指定。 通常，$r_z$ 在几欧姆到几十欧姆的范围内。 <u>**显然，$r_z$ 值越低，齐纳电压在其电流变化时保持的越恒定，因此其在稳压器设计中的性能就越理想。**</u> 在这方面，我们从图 4.19 中观察到，虽然 $r_z$ 在很宽的电流范围内保持较低且几乎恒定，但其值在拐点附近显着增加。 因此，作为一般设计准则，应避免在此低电流区域操作齐纳二极管。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220308105435206.png" alt="image-20220308105435206" style="zoom:80%;" />

齐纳二极管的制造电压 $V_Z$ 在几伏到几百伏的范围内。 除了指定 $V_Z$（在特定电流 $I_{ZT}$ 下）、$r_z$ 和 $I_{ZK}$ 之外，制造商还指定了设备可以安全消散的最大功率。 因此，一个 0.5W、6.8V 的齐纳二极管可以在最大约 70mA 的电流下安全工作。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220308105732934.png" alt="image-20220308105732934" style="zoom:80%;" />

齐纳二极管几乎线性的 $i-v$ 特性表明该器件可以如图 4.20 所示进行建模。 这里 $V_{Z0}$ 表示斜率 $1/r_z$ 的直线与电压轴相交的点（参见图 4.19）。 尽管图 4.19 中显示的 $V_{Z0}$ 与拐点电压 $V_{ZK}$ 略有不同，但实际上它们的值几乎相等。图 4.20 的等效电路模型可以解析描述为**（实际上，这就是 $Q$ 点假设的“负载线”的解析式，因此这里与小信号模型不同）**
$$
V_Z = V_{Z0} + r_zI_Z\tag{4.20}
$$
它适用于 $I_Z > I_{ZK}$，显然，$V_Z > V_{Z0}$。

#### 4.4.2 Use of the Zener as a Shunt Regulator

我们现在举例说明在并联稳压器设计中使用齐纳二极管，之所以如此命名，是因为稳压器电路与负载并联（并联）。

##### Example 4.7

图 4.21(a) 电路中的 6.8V 齐纳二极管规定在 $I_Z =5 mA$、$r_z =20$ 和 $I_{ZK} = 0.2 mA$ 时 $V_Z =6.8 V$。 电源电压 $V^+$ 标称为 10 V，但可以变化 ±1 V。

(a) 找到无负载且 $V^+$ 为其标称值的 $V_O$。

(b) 找出由 $V^+$ 的 $±1V$ 变化引起的 $V_O$ 变化。 请注意，通常以 mV/V 表示的 $V_O/V^+$ 称为线路调节。

(c) 求出连接负载电阻 $R_L$ 导致的 $V_O$ 变化，该负载电阻 $R_L$ 消耗电流 $I_L = 1 mA$，从而求出负载调整率 $V_O/I_L$，单位为 mV/mA。

(d) 求 $R_L = 2 k$ 时 $V_O$ 的变化。

(e) 求 $R_L = 0.5 k$ 时的 $V_O$ 值。

(f) 二极管仍在击穿区工作的 $R_L$ 最小值是多少？

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220308110801382.png" alt="image-20220308110801382" style="zoom:67%;" />

**Solution**

首先我们必须确定齐纳二极管模型的参数 $V_{Z0}$ 的值。 将 $V_Z = 6.8 V、I_Z = 5 mA$ 和 $r_z = 20$ 代入方程式 (4.20) 得出 $V_{Z0} = 6.7 V$。图 4.21(b) 显示了用**其模型替换齐纳二极管的电路。**

(a) <u>**在没有连接负载的情况下，通过齐纳二极管的电流由下式给出**</u> **<u>（这一段给出了在没有连接负载的情况下，使得稳压管正常工作干路所能提供的最大电流）</u>**
$$
I_Z = I = \frac{V^+ -V_{Z0}} {R+r_z} = \frac{10-6.7} {0.5+0.02} = 6.35 mA
$$
因此（注意按照电流的正方向来计算，图中回路的方向是从上至下，$V_{Z0}$ 因此是“负方向的”（降低电势），因此电流经过 $V_{Z0}$ 降低 6.7V，再经过 $r_z$ 降低一次，最后变为 0）
$$
V_O = V_{Z0} +I_Z r_z = 6.7+6.35×0.02 = 6.83V
$$
(b) 对于 $V^+$ 的 $±1V$​ 变化，输出电压的变化可以从下式获得
$$
\Delta V_O = \Delta V^+ \frac{r_z} {R+r_z} =±1× \frac{20} {500+20} =±38.5 mV
$$
因此

线路调节 = 38.5 mV/V

(c) 连接负载电流 $I_L = 1 mA$ 的负载电阻 $R_L$ 时，**齐纳电流将减少 1 mA**。 齐纳电压的相应变化可以从下式获得
$$
\Delta V_O = r_z\Delta I_Z = 20×-1=-20 mV
$$
因此负载调节是
$$
负载调节 ≡ \frac{\Delta V_O} {\Delta I_L} = -20 mV / mA
$$
(d) 连接 2 k 的负载电阻时，负载电流约为 $6.8 V/2 k = 3.4 mA$。 因此齐纳电流的变化将是 $I_Z =-3.4 mA$，因此齐纳电压（输出电压）的相应变化将是
$$
\Delta V_O = r_z \Delta I_Z = 20×-3.4=-68 mV
$$
该值可以通过将负载调节率乘以 $I_L$ (3.4 mA) 的值来获得。

(e) 0.5 k 的 $R_L$ 将消耗 $6.8/0.5=13.6 mA$ 的负载电流。 这是不可能的，<u>因为通过 $R$ 提供的电流仅为 6.4 mA（对于 $V^+ = 10 V$）</u>。 **==（注意检查支路电流与干路电流的大小关系！）==**因此，齐纳二极管必须被切断。 如果确实如此，则 $V_O$ 由 $R_L$ 和 $R$ 形成的分压器确定（图 4.21a），
$$
V_O = V^+ \frac{R_L} {R+R_L} = 10 \frac{0.5} {0.5+0.5} = 5 V
$$
**<u>由于这个电压低于齐纳二极管的击穿电压，二极管确实不再工作在击穿区。</u>**

(f) 对于处于击穿区域边缘的齐纳二极管，$I_Z = I_{ZK} = 0.2 mA$ 和 $V_Z\simeq V_{ZK} \simeq6.7 V$。此时通过 $R$ 提供的最低（最坏情况）电流为 $(9−6.7)  /0.5 = 4.6 mA$，因此负载电流为 $4.6 -0.2 = 4.4 mA$，$R_L$的对应值为 
$$
R_L = \frac{6.7} {4.4} \simeq 1.5 k\Omega
$$

#### 4.4.3 Temperature Effects

齐纳电压 $V_Z$ 对温度的依赖性根据其温度系数 TC 或众所周知的 temco 来指定，通常以 mV/°C 表示。TC 的值取决于齐纳电压，对于给定的二极管，TC 随工作电流而变化。  $V_Z$ 低于约 5 V 的齐纳二极管呈现负 TC。 另一方面，具有较高电压的齐纳二极管表现出正的 TC，$V_Z$ 约为 5 V 的齐纳二极管的 TC 可以通过在指定电流下操作二极管来使其为零。 另一种获得低温度系数参考电压的常用技术

#### 4.4.4 A Final Remark

齐纳二极管虽然简单实用，但近年来已失去了很大的知名度。 在电压调节器设计中，它们实际上已被专门设计的集成电路 (IC) 所取代，这些集成电路 (IC) 比齐纳二极管更有效地执行电压调节功能并具有更大的灵活性。

### 4.5 Rectifier Circuits

二极管最重要的应用之一是整流电路的设计。 二极管整流器构成了为电子设备供电所需的直流电源的基本组成部分。 这种电源的框图如图 4.22 所示。 如图所示，电源从 120-V (rms) 60-Hz 交流线路馈电，并将直流电压 $V_O$（通常在 4 V 到 20 V 范围内）输送到由负载块表示的电子电路 . 尽管交流线路电压和负载消耗的电流发生变化，但直流电压 $V_O$ 必须尽可能恒定。

直流电源中的第一个模块是电源变压器。 它由两个独立的线圈组成，这些线圈缠绕在铁芯上，铁芯磁耦合两个绕组。 具有 $N_1$ 匝的初级绕组连接到 120-V 交流电源，具有 $N_2$ 匝的次级绕组连接到直流电源电路。 因此，在次级绕组的两个端子之间会产生 120($N_2/N_1$) V (rms) 的交流电压 $v_S$。 通过为变压器选择合适的匝数比 $(N_1/N_2)$，设计人员可以将线路电压逐步降低到产生特定直流电压输出所需的值。 例如，8V rms 的次级电压可能适用于 5V 的直流输出。这可以通过 15:1 的匝数比来实现。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220308151906016.png" alt="image-20220308151906016" style="zoom:80%;" />

除了为直流电源提供适当的正弦幅度外，电源变压器还提供电子设备和电力线电路之间的电气隔离。 这种隔离将设备用户的电击风险降至最低。

二极管整流器将输入正弦波 $v_S$ 转换为单极输出，其脉动波形如图 4.22 所示。 尽管该波形具有非零平均值或直流分量，但其脉动特性使其不适合作为电子电路的直流源，因此需要滤波器。 图 4.22 中的滤波器块大大减少了整流器输出幅度的变化。 在本节中，我们将研究一些整流器电路和输出滤波器的简单实现。

整流器滤波器的输出虽然比没有滤波器时稳定得多，<u>但仍包含一个与时间相关的分量，称为纹波。</u> 为了减少纹波并稳定直流输出电压的幅度以防止负载电流变化引起的变化，采用了电压调节器。 这种稳压器可以使用第 4.4 节中研究的齐纳并联稳压器配置来实现。 或者，目前更为普遍的是，可以使用集成电路调节器。

#### 4.5.1 The Half-Wave Rectifier

半波整流器利用输入正弦波的交替半周期。 图 4.23(a) 显示了半波整流器的电路。 该电路在第 4.1 节（见图 4.3）中进行了分析，假设为理想二极管。 使用更现实的恒压降二极管模型，我们得到
$$
v_O = 0, v_S < V_D\tag{4.21a}
$$

$$
v_O = v_S -V_D, v_S ≥ V_D\tag{4.21b}
$$

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220308152853997.png" alt="image-20220308152853997" style="zoom:67%;" />

这些方程表示的传输特性如图 4.23(b) 所示，其中 $V_D = 0.7 V$ 或 $0.8 V$。图 4.23(c) 显示了当输入 $v_S$ 为正弦曲线时获得的输出电压。

在为整流器设计选择二极管时，必须指定两个重要参数：**<u>二极管所需的电流处理能力，由二极管预期传导的最大电流决定，以及二极管必须能够承受的峰值反向电压 (PIV) 承受不击穿，由预计出现在二极管两端的最大反向电压决定。</u>** 在图 4.23(a) 的整流电路中，我们观察到当 $v_S$ 为负时，二极管将被切断，$v_O$ 将为零。 由此可见，PIV 等于 $v_S$ 的峰值，
$$
PIV=V_S\tag{4.22}
$$
然而，<u>通常谨慎的做法是选择反向击穿电压至少比预期 PIV 大 50% 的二极管。</u>

在离开半波整流器之前，读者应该注意两点。 首先，可以使用二极管指数特性来确定整流器的准确传输特性（见习题 4.68）。 但是，所涉及的工作量通常太大而无法在实践中证明是合理的。 当然，使用诸如 SPICE 之类的计算机电路分析程序可以很容易地完成这种分析。

其次，无论我们是否准确地分析电路，应该很明显，<u>当输入信号较小时，该电路不能正常工作。 例如，该电路不能用于对幅度为 100 mV 的输入正弦波进行整流。</u> 对于这样的应用，人们求助于所谓的精密整流器，这是一种利用二极管和运算放大器的电路。第 4.5.5 节介绍了一种这样的电路。

##### EXERCISE

对于图 4.23(a) 中的半波整流器电路，我们可以得到下列量： 

(a) 对于二极管导通的半周期，导通以角度 $θ = \sin^{−1} V_D/V_s$ 开始并终止 在 $(π – θ)$ 处，总传导角为 $(π –2θ)$。  

(b) $v_O$ 的平均值（直流分量）为 $V_O\simeq (1/π)V_s -V_D/2$。

(c) 峰值二极管电流为 $V_s -V_D /R$。

在 12V (rms) 正弦输入、$V_D\simeq 0.7V$ 和 $R = 100$ 的情况下，找出这些量的数值。 另外，给出 PIV 的值

答 (a) $θ = 2.4°$，导通角 = 175°；  (b) 5.05 伏；  (c) 163 毫安；  17 伏

#### 4.5.2 The Full-Wave Rectifier

全波整流器利用输入正弦波的两半，为了提供单极输出，它将正弦波的负半部分反转。 一种可能的实现如图 4.24(a) 所示。 在这里，变<u>压器的次级绕组是中心抽头的，以在具有指示极性的次级绕组的两半上提供两个相等的电压 $v_S$。</u> <u>**请注意，当输入线电压（为初级供电）为正时，标有 $v_S$ 的两个信号都将为正。**</u> 在这种情况下，$D_1$ 将导通，$D_2$ 将反向偏置。 通过 $D_1$ 的电流将流过 $R$ 并**返回到次级的中心抽头**。 然后，电路就像一个半波整流器，当 $D_1$ 导通时，**正半周期间的输出将与半波整流器产生的输出相同。**

现在，在交流线电压的负半周期期间，**标有 $v_S$ 的两个电压都将为负**。 **<u>因此 $D_1$ 将被切断，而 $D_2$ 将导通。</u>**  $D_2$ 传导的电流将流过 $R$ 并返回到中心抽头。 **因此，在 $D_2$ 导通的负半周期期间，==电路再次充当半波整流器。==** 然而，重要的一点是**==<u>通过 $R$ 的电流总是以相同的方向流动</u>==**，<u>因此 $v_O$ 将是单极的</u>，如图 4.24(c) 所示。 所示的输出波形是通过假设导通二极管具有恒定电压降 $V_D$ 获得的。 因此，全波整流器的传递特性如图 4.24(b) 所示。

显然，全波整流器产生的波形比半波整流器提供的波形更“有活力”。 在几乎所有整流器应用中，人们都会选择某种全波类型。

要找到全波整流器电路中二极管的 PIV，请考虑正半周期内的情况。 二极管 $D_1$ 导通，$D_2$ 截止，$D_2$ 的阴极电压为 $v_O$，其阳极电压为 $-v_S$，<u>因此 $D_2$ 两端的反向电压为 $(v_O +v_S)$</u>，当 $v_O$ 处于 $(V_s – V_D)$ 的峰值，$v_S$ 处于 $V_s$ 的峰值时，反向电压将达到最大值； 因此，
$$
PIV = 2V_s -V_D
$$
这大约是半波整流器的两倍。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220308155422488.png" alt="image-20220308155422488" style="zoom:67%;" />

图 4.24 使用带有中心抽头次级绕组的变压器的全波整流器： (a) 电路；  (b) 假设二极管采用恒定压降模型的传输特性；  (c) 输入和输出波形。

#### 4.5.3 The Bridge Rectifier

全波整流器的另一种实现如图 4.25(a) 所示，该电路被称为**桥式整流器**，因为其结构与惠斯通电桥相似，不需要中心抽头变压器，这与图 4.24 的全波整流器电路相比具有明显优势。 然而，桥式整流器需要四个二极管，而之前的电路中需要两个。 这并不是什么缺点，因为二极管很便宜，而且可以在一个封装中购买一个二极管桥。

桥式整流电路的工作原理如下：**<u>在输入电压的正半周期内，$v_S$ 为正，因此电流通过二极管 $D_1$、电阻器 $R$ 和二极管 $D_2$ 传导；同时，二极管 $D_3$ 和 $D_4$ 将反向偏置。 观察到在传导路径中有两个串联的二极管，==因此 $v_O$ 将比 $v_S$ 低两个二极管压降==（与前面讨论的电路中的一个压降相比）。</u>** 这在某种程度上是桥式整流器的缺点。

接下来，考虑输入电压负半周期的情况。 次级电压 $v_S$ 为负，因此 $-v_S$ 为正，迫使电流通过 $D_3$、$R$ 和 $D_4$。 同时，二极管 $D_1$ 和 $D_2$ 将反向偏置。 然而，需要注意的重要一点是，在两个半周期中，**电流以相同的方向（从右到左）流过 $R$**，因此 $v_O$ 将始终为正，如图 4.25(b) 所示。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220308160414081.png" alt="image-20220308160414081" style="zoom:80%;" />

要确定每个二极管的峰值反向电压 (PIV)，请考虑正半周期内的电路。  $D_3$ 两端的反向电压可以由 $D_3$、$R$ 和 $D_2$ 形成的回路确定为
$$
v_{D3}(反向) = v_O +v_{D2}(正向)
$$
因此 $v_{D3}$ 的最大值出现在 $v_O$ 的峰值处，由下式给出
$$
PIV = V_s -2V_D +V_D = V_s -V_D
$$
请注意，此处的 PIV 约为带有中心抽头变压器的全波整流器值的一半。 这是桥式整流器的另一个优点。

与使用中心抽头变压器相比，桥式整流器电路的另一个优点是变压器的次级绕组只需要大约一半的匝数。 另一种看待这一点的方法可以通过观察中心抽头变压器的次级绕组的每一半仅使用一半时间来获得。 这些优点使桥式整流器成为最流行的整流器电路配置。

#### EXERCISE

对于图 4.25(a) 的桥式整流器电路，使用恒压降二极管模型表明 (a) 输出电压的平均值（或直流分量）为 $V_O (2/π)V_s -  2V_D$ 和 (b) 峰值二极管电流为 $(V_s -2V_D)/R$。 求 (a) 和 (b) 中的量的数值以及 $v_S$ 为 12-V (rms) 正弦曲线、$V_D$ 为 0.7 V 且 $R = 100$ 的情况下的 PIV

#### 4.5.4 The Rectifier with a Filter Capacitor The Peak Rectifier

上述整流器电路产生的输出电压的脉动特性使其不适合作为电子电路的直流电源。 减少输出电压变化的一种简单方法是在负载电阻上放置一个电容器，我们将表明，该滤波电容器用于显着减少整流器输出电压的变化。

要了解带滤波电容器的整流电路如何工作，首先考虑图 4.26 所示的简单电路，让输入 $v_I$ 是一个峰值为 $V_p$ 的正弦波，并假设二极管是理想的。 随着 $v_I$ 变为正值，二极管导通并对电容器充电，因此 $v_O = v_I$ ，这种情况一直持续到 $v_I$ 达到其峰值 $V_p$，达到峰值以后，随着 $v_I$ 的降低，**二极管变为反向偏置，输出电压保持恒定在值 $V_p$**，事实上，**从理论上讲，电容器将无限期地保留其电荷并因此保留其电压，因为<u>电容器无法放电</u>（反向不导通）**。 因此，该电路提供的直流电压输出等于输入正弦波的峰值。 鉴于我们希望产生直流输出，这是一个非常令人鼓舞的结果。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220309140435608.png" alt="image-20220309140435608" style="zoom:80%;" />

接下来，我们考虑负载电阻 $R$ 跨接电容器 $C$ 的更实际情况，如图 4.27(a) 所示。 但是，我们将继续假设二极管是理想的。 如前所述，对于正弦输入，电容器充电至输入 $V_p$ 的峰值。 然后二极管截止，电容通过负载电阻 $R$ 放电。

电容器放电将持续几乎整个周期，直到 $v_I$ 超过电容器电压。 然后二极管再次开启并将电容器充电至峰值 $v_I$，并且该过程会重复。 请注意，为防止输出电压在电容器放电期间过度下降，**应选择一个 $C$ 值，以使时间常数 $CR$ 远大于放电间隔。**

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220309140942098.png" alt="image-20220309140942098" style="zoom:80%;" />

我们现在准备详细分析电路。 图 4.27(b) 显示了假设 $CR\gg T$ 下的稳态输入和输出电压波形，其中 $T$ 是输入正弦波的周期。 负载电流波形
$$
i_L = v_O/R\tag{4.23}
$$
和二极管电流（当它导通时）
...
如图 4.27(c) 所示，我们按照下面的顺序来观察整个电路：

1. 二极管在<u>输入正弦波峰值附近导通一个短暂的时间间隔 $t$</u>，并为电容器提供与在更长的放电时间间隔内损失的电荷相等的电荷，后者（指电容的放电时间）大约等于周期 $T$。

2. 假设一个理想二极管，二极管导通在时间 $t_1$ 开始，此时输入 $v_I$ 等于指数衰减的输出 $v_O$。 导通在 $v_I$ 峰值后不久的 $t_2$ 处停止；$t_2$ 的精确值可以通过在方程式 (4.25) 中设 $i_D =0$ 来确定**<u>（即此时由电容放电电势与正弦波的充电电势刚好相等，电流为 $0$ ，此后一小段时间内正弦波的充电电势上升，大于此时已经不断下降的电容放电电势，电容器开始充电）</u>**
3. 在二极管关断期间，电容器 $C$ 通过 $R$ 放电，因此 $v_O$ 随时间常数 $CR$ 呈指数衰减，**放电间隔从刚超过 $v_I$ 的峰值开始，在几乎持续整个周期 $T$ 的放电间隔结束时，$v_O = V_p – V_r$** ，其中 $V_r$ 是峰峰值纹波电压。 当 $CR\gg T$ 时，$V_r$ 的值很小。**<u>（要注意的是，在电容器放电时，电容本身可以看作是一个电源，因此左右两个电路是独立的；而在电容充电时，则不能把右侧的电路独立出来）</u>**

4. **$V_r$** 较小时，$v_O$ 几乎为常数，等于 $v_I$ 的峰值。 因此，直流输出电压大约等于 $V_p$。 类似地，电流 $i_L$ 几乎是恒定的，其直流分量 $I_L$ 由下式给出

$$
I_L = \frac{V_p} R\tag{4.26}
$$

如果需要，可以通过取 $v_O$ 极值的平均值来获得输出直流电压的更准确表达式，

$$
V_O = V_p - \frac1 2V_r\tag{4.27}
$$
有了这些观察结果，我们现在推导出 $V_r$ 以及二极管电流的平均值和峰值的表达式。 在二极管关断期间，$v_O$ 可以表示为
...
$$
V_p−V_r\simeq V_pe^{−T/CR}
$$
现在，由于 $CR\gg T$，我们可以使用近似 $e^{−T/CR}\simeq 1−T/CR$ 来获得
$$
V_r\simeq V_p \frac T {CR}\tag{4.28}
$$
我们观察到，要保持 $V_r$ 小，我们必须选择一个电容 $C$，使 $CR\gg T$，方程式中 (4.28) 的纹波电压 $V_r$ 可以用频率 $f =1/T$ 表示为
$$
V_r = \frac{V_p} {fCR}\tag{4.29a}
$$
使用方程式 (4.26) 我们可以用替代表达式来表示 $V_r$
$$
V_r = \frac{I_L} {fC}\tag{4.29b}
$$
请注意，对上述近似值的另一种解释是电容器通过恒定电流 $I_L = V_p/R$ 放电，只要 $V_r\ll V_p$，这个近似值就有效。

假设二极管导通几乎在 $v_I$ 的峰值处停止，我们可以确定导通间隔 $t$  
$$
V_p\cos(\omega\Delta t)=V_p-V_r
$$
其中 $ω = 2πf = 2π/T$ 是 $v_I$ 的角频率。 由于 $(ω\Delta t)$ 是一个小角度，我们可以采用近似 $\cos(ω\Delta t)\simeq 1− \frac1 2 (ω\Delta t)^ 2$ 来获得
$$
ω\Delta t \simeq 	 \sqrt{2V_r /V_p}\tag{4.30}
$$
我们注意到，当 $V_r\ll V_p$ 时，导通角 $ω\Delta t$ 会像假设的那样小。

为了确定导通期间的平均二极管电流 $i_{Dav}$，我们将二极管提供给电容器的电荷等同起来，
$$
...
使用方程 (4.30) 和 (4.29a)，
$$
i_{Dav} = I_L \left(1+π \sqrt{2V_p/V_r}\right)\tag{4.31}
$$
观察到当 $V_r\ll V_p$ 时，**二极管导通期间的平均电流远大于直流负载电流**，这并不奇怪，因为二极管导通的时间间隔很短，并且必须在更长的时间间隔内补充电容器损失的电荷，在该时间间隔内它由 $I_L$ 放电。

二极管电流的峰值 $i_{Dmax}$ 可以通过计算公式 (4.25) 中的表达式来确定。在二极管导通开始时，即在 $t = t_1 =−\Delta t$ （其中 $t = 0$ 处于峰值处），假设 $i_L$ 在方程式 (4.26) 给出的值上几乎是恒定的，我们得到
$$
i_{Dmax} = I_L\left( 1 + 2π \sqrt{2V_p / V_r}\right)\tag{4.32}
$$
从方程式（4.31）和（4.32），我们看到对于 $V_r\ll V_p，i_{Dmax}\simeq 2i_{Dav}$，这与 $i_D$ 的波形几乎是一个直角三角形的事实相关（见图 4.27c）。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220309144127974.png" alt="image-20220309144127974" style="zoom:80%;" />

图 4.27(a) 的电路称为半波峰值整流器。 图 1 的全波整流电路 4.24(a) 和 4.25(a) 可以通过在负载电阻两端包含一个电容器来转换为峰值整流器。 与半波情况一样，输出直流电压几乎等于输入正弦波的峰值（图 4.28）。 然而，纹波频率将是输入频率的两倍。 在这种情况下，峰峰值纹波电压可以使用与上述相同的程序导出，但放电周期 T 由 $T/2$ 代替，导致
$$
V_r=\frac{V_p}{2fCR}\tag{4.33}
$$
虽然二极管导通间隔 $t$ 仍由方程式给出 (4.30)，每个二极管中的平均和峰值电流将由下式给出
$$
i_{Dav} = I_L \left(1 + π \sqrt{V_p / 2V_r}\right)\tag{4.34}
$$

$$
i_{Dmax} = I_L \left(1 + 2π \sqrt{V_p / 2V_r}\right)\tag{4.35}
$$

将这些表达式与半波情况下的相应表达式进行比较，我们注意到对于相同的 $V_p、f、R$ 和 $V_r$ 值（因此相同的 $I_L$），我们需要一个大小为 半波整流器。 此外，全波整流器中每个二极管中的电流大约是半波电路二极管中流过的电流的一半。

上述分析假设理想二极管。 通过考虑二极管电压降可以提高结果的准确性，这可以通过将电容器充电的峰值电压 $V_p$ 替换为半波电路的 $(V_p – V_D)$ 和使用中心抽头变压器的全波电路以及电桥的 $(V_p – 2V_D)$ 来轻松完成 -整流器外壳。

我们通过指出峰值整流器电路在需要检测输入信号峰值的信号处理系统中找到应用来结束本节。 在这种情况下，该电路被称为峰值检测器。 峰值检测器的一个特别流行的应用是设计用于调幅 (AM) 信号的解调器。 我们不会在这里进一步讨论这个应用程序。

#### Example 4.8

考虑一个由峰值 $V_p = 100 V$ 的 $60 Hz$ 正弦波馈电的峰值整流器。设负载电阻 $R = 10 k$，找出将导致 $2 V$ 峰峰值纹波的电容 $C$ 的值，此外，计算二极管导通的周期分数以及二极管电流的平均值和峰值

**Solution**

从方程式 (4.29a) 我们得到 $C$ 的值
$$
C = \frac{V_p} {V_r f R} = \frac{100} {2×60×10×10^3} = 83.3 μF
$$
导通角 $ω\Delta t$ 由等式 (4.30) 求得为
$$
ω\Delta t = \sqrt{2×2/100} = 0.2 弧度
$$
因此，二极管在 $(0.2/2π)×100 = 3.18\%$ 的周期内导通。 平均二极管电流由方程式 (4.31) 获得，其中 $I_L = 100/10 = 10 mA$，即
$$
i_{Dav} = 10 \left(1 + π \sqrt{2 × 100/2}\right) = 324 mA
$$
使用方程式 (4.32) 找到峰值二极管电流,
$$
i_{Dmax} = 10 \left(1+2π \sqrt{2×100/2}\right) = 638 mA
$$
**最早的半导体二极管：**

猫须或晶体检测器是第一个商业化的电子二极管，作为无线电电话中使用的射频信号的包络检测器。 最早的二极管是由 Karl Ferdinand Braun 在德国发明的，它由一小块方铅矿（硫化铅）组成，通过可以调整的锋利弹簧线与其接触。 由于对早期无线电的这一贡献和其他贡献，布劳恩于 1909 年获得了诺贝尔物理学奖。后来改进和封装的硅基点接触二极管是二战期间雷达设备的重要固态部件。

#### 4.5.5 Precision Half-Wave Rectifier The Superdiode

迄今为止研究的整流器电路在信号路径中有一个或两个二极管压降。 因此，**<u>只有当要整流的信号远大于导通二极管的电压降（0.7 V 左右）时，这些电路才能正常工作</u>**。 在这种情况下，二极管正向特性的细节或二极管电压的精确值在决定电路性能方面并不起重要作用。 整流电路在电源设计中的应用确实如此。 然而，在其他应用中，待整流的信号很小（例如，大约 100 mV 左右），因此显然不足以打开二极管。 此外，在仪器仪表应用中，需要具有非常精确和可预测的传输特性的整流器电路。 对于这些应用，已经开发了一类电路，利用运算放大器（第 2 章）与二极管一起提供精密整流。 在下面的讨论中，我们研究了一种这样的电路。 网站上提供了对运算放大器二极管电路的全面研究。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220309200057444.png" alt="image-20220309200057444" style="zoom:80%;" />

图 4.29 (a) “超级二极管”精密半波整流器和 (b) 其几乎理想的传输特性。请注意，当 $v_I > 0$ 且二极管导通时，运算放大器提供负载电流，并且源可以方便地缓冲，这是一个额外的优势。 未显示运算放大器电源。

图 4.29(a) 显示了一个精密半波整流器电路，该电路**由放置在运算放大器负反馈路径中的二极管组成**，$R$ 为整流器负载电阻。 当然，运算放大器的运行需要电源。 为简单起见，这些未在电路图中显示。 该电路的工作原理如下：**如果 $v_I$ 变为正，则运放的输出电压 $v_A$ 将变为正，二极管将导通，从而在运放的输出端和负输入端之间建立一个闭合反馈路径。 这种负反馈路径会导致运算放大器的两个输入端之间出现虚拟短路。 因此，负输入端的电压，也是输出电压 $v_O$，将等于（在几毫伏范围内）正输入端的电压，即输入电压 $v_I$，**
$$
v_O = v_I\ \ \ \ \ \ v_I ≥ 0
$$
请注意，图 4.23 的简单半波整流器电路中显示的偏移电压 (0.7 V) 不再存在。 为了**<u>使运算放大器电路开始工作，$v_I$ 只需超过一个可以忽略不计的小电压，该电压等于二极管压降除以运算放大器的开环增益。</u>** 换言之，直线传递特性 $v_O-v_I$ 几乎通过原点。 这使得该电路适用于涉及非常小信号的应用。

现在考虑当 $v_i$ 变为负数的情况。 **<u>运算放大器的输出电压 $v_A$ 将趋于跟随并变为负值。 这将使二极管反向偏置，并且没有电流流过电阻 $R$，导致 $v_O$ 保持等于 0 V。因此，对于 $v_I < 0$，$v_O = 0$。由于在这种情况下二极管关闭，运算放大器将 以开环方式运行，其输出将处于负饱和水平。</u>**

该电路的传输特性如图 4.29(b) 所示，几乎与半波整流器的理想特性相同。 通过将二极管置于运算放大器的负反馈路径中，几乎完全掩盖了非理想二极管特性。 这是负反馈的另一个引人注目的应用，我们将在第 11 章正式研究这个主题。二极管和运算放大器的组合，如图 4.29(a) 中的虚线框所示，被恰当地称为“超级二极管”。

### 4.6 Limiting and Clamping Circuits

在本节中，我们将介绍二极管的其他非线性电路应用。

#### 4.6.1 Limiter Circuits

图 4.30 显示了限幅器电路的一般传输特性。 如图所示，对于一定范围内的输入，$L_−/K ≤ v_I ≤ L_+/K$，限幅器充当线性电路，提供与输入成比例的输出，$v_O = Kv_I$。 虽然通常 $K$ 可以大于 1，但本节讨论的电路具有 $K≤1$ 并且被称为无源限制器。  （有源限幅器的示例将在第 18 章中介绍。）**<u>==如果 $v_I$ 超过上限阈值 $L_+/K$，则输出电压被限制或钳位到上限电平 $L_+$。 另一方面，如果 $v_I$ 降低到低于下限阈值 $L_-/K$，则输出电压 $v_O$ 被限制在下限电平L-。==</u>**

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220309202441339.png" alt="image-20220309202441339" style="zoom:80%;" />

图 4.30 的一般传输特性描述了一个双限幅器——即，一个限幅器同时作用于输入波形的正峰值和负峰值。 当然，单个限制器是存在的。 最后，请注意，如果将图 4.31 所示的输入波形馈送到双限幅器，则其两个峰值将被剪掉。 因此，限制器有时被称为限幅器。

其特性如图 4.30 所示的限幅器被描述为硬限幅器。软限制的特点是线性区域和饱和区域之间的过渡更平滑，饱和区域的斜率大于零，如图 4.32 所示。 取决于应用，硬限制或软限制可能是优选的。

限制器在各种信号处理系统中都有应用。 它们<u>最简单的应用之一是将运算放大器的两个输入端子之间的电压限制为低于构成运算放大器电路输入级的晶体管的击穿电压的值。</u> 在本书的后面部分，我们将更多地谈论这个和其他限制器应用程序。

二极管可以与电阻器结合使用，以提供限制器功能的简单实现。 图 4.33 中描述了一些示例。 在图中的每个部分都给出了电路及其传输特性。 传输特性是使用恒定压降 $(V_D = 0.7 V)$ 二极管模型获得的，但假设传输特性的线性和饱和区域之间的过渡是平滑的。

图 4.33(a) 中的电路是半波整流器的电路，只是这里的输出是通过二极管获取的。 对于 $v_I < 0.5 V$，二极管截止，没有电流流过，$R$ 两端的电压降为零； 因此 $v_O = v_I$。 当 $v_I$ 超过 0.5 V 时，二极管开启，最终将 $v_O$ 限制为一个二极管压降 (0.7 V)。 图 4.33(b) 的电路与图 4.33(a) 中的电路相似，只是二极管是反向的。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220311111651365.png" alt="image-20220311111651365" style="zoom:80%;" />

可以通过**并联两个极性相反的二极管来实现双重限制，**如图 4.33(c) 所示。 这里特性的线性区域是在 $-0.5V≤v_I ≤0.5 V$ 时获得的。对于这个 $v_I$ 范围，两个二极管都关闭并且 $v_O =v_I$。 当 $v_I$ 超过 0.5 V 时，$D_1$ 开启并最终将 $v_O$ 限制为 +0.7 V。类似地，当 $v_I$ 变得比 –0.5 V 更负时，$D_2$ 开启并最终将 $v_O$ 限制为 –0.7 V。

**<u>二极管限幅器的阈值和饱和水平可以通过使用二极管串和/或通过将直流电压与二极管串联来控制</u>**。 后一种想法如图 4.33(d) 所示。 最后，我们可以使用两个串联的齐纳二极管，而不是二极管串，如图 4.33(e) 所示。 在该电路中，正向限制发生在电压 $V_{Z2} +0.7$ 时，其中 0.7 V 表示**<u>正向导通时齐纳二极管 Z1 两端的电压降</u>**。 对于负输入，$Z_1$ 充当齐纳二极管，而 $Z_2$ 正向导通。 应该提到的是，串联连接的齐纳二极管对在商业上可用于这种类型的应用，名称为双阳极齐纳二极管。

如果运算放大器与二极管和电阻器结合使用，则可以实现更灵活的限幅器电路。 第 18 章讨论了此类电路的示例。

#### 4.6.2 The Clamped Capacitor or DC Restorer

如果在基本的峰值整流器电路中，输出是通过二极管而不是电容器，这是一个具有重要应用结果的有趣电路。 该电路称为**直流恢复器**，如图 4.34 所示，由方波馈电。 由于二极管连接的极性，**==电容器将充电至电压 $v_C$，其极性如图 4.34 所示，并且等于输入信号的最负峰值的幅度。 随后，二极管关闭，电容器无限期地保持其电压。==** 例如，如果输入方波具有任意电平 –6 V 和 +4 V，则 $v_C$ 将等于 6 V。现在，由于输出电压 $v_O$ 由下式给出

$$
v_O=v_I+v_C
$$
因此，输出波形将与输入波形相同，只是它向上移动了 $v_C$ 伏特。 因此，在我们的示例中，输出将是电平为 0 V 和 +10 V 的方波。

可视化图 4.34 中电路操作的另一种方法是注意，由于二极管以所示极性连接在输出端，它可以防止输出电压低于 0 V（通过对电容器进行导通和充电，因此导致输出上升到 0 V），但这种连接不会限制 $v_O$ 的正偏移。 因此，输出波形的最低峰值将被钳位到 0 V，这就是该电路被称为钳位电容器的原因。 很明显，反转二极管极性将提供一个输出波形，其最高峰值被钳位到 0 V。在任何一种情况下，输出波形都将具有有限的平均值或直流分量。 该直流分量与输入波形的平均值完全无关。 作为一个应用，考虑通过电容耦合或交流耦合系统传输的脉冲信号。 电容耦合将导致脉冲序列失去它最初拥有的任何直流分量。 将产生的脉冲波形馈送到钳位电路为其提供确定的直流分量，这一过程称为直流恢复。 这就是为什么该电路也称为直流恢复器的原因。

恢复直流很有用，因为脉冲波形的直流分量或平均值是其占空比的有效量度。脉冲波形的**占空比**可以被调制（在称为脉冲宽度调制的过程中）并用于携带信息。 在这样的系统中，只需将接收到的脉冲波形馈送到直流恢复器，然后使用简单的 RC 低通滤波器将输出波形的平均值与叠加脉冲分离，即可实现检测或解调。

> 脉冲波形的占空比是脉冲在每个周期中所占的比例。 换句话说，它是脉冲宽度，表示为脉冲周期的一部分

当负载电阻 $R$ 跨接在钳位电路中的二极管上时，如图 4.35 所示，情况会发生显着变化，**==当输出高于地时，电流必须流入 $R$。==**由于此时二极管关闭，该电流显然来自电容器，<u>**从而导致电容器放电并且输出电压下降**</u>。 图 4.35 显示了方波输入的情况。 在 $t_0$ 到 $t_1$ 期间，输出电压随时间常数 CR 呈指数下降。 在 $t_1$ 时，输入减少 $V_a$ 伏，输出尝试跟随。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220311114151720.png" alt="image-20220311114151720" style="zoom:80%;" />

这会导致二极管大量导通并快速为电容器充电。 在时间间隔 $t_1$ 到 $t_2$ 结束时，输出电压通常为负数十分之几伏（例如，–0.5 V）。 然后，随着输入上升 $V_a$ 伏特（在 $t_2$ 时），输出随之而来，并且循环自身重复。 在稳态中，电容器在 $t_0$ 到 $t_1$ 期间损失的电荷在 $t_1$ 到 $t_2$ 期间恢复。 这种电荷平衡使我们能够计算平均二极管电流以及输出波形的细节。

#### 4.6.3 The Voltage Doubler

图 4.36(a) 显示了一个由两部分级联组成的电路：一个由 $C_1$ 和 $D_1$ 组成的钳位电容器，以及一个由 $D_2$ 和 $C_2$ 组成的峰值整流器。 当被一个幅度为 $V_p$ 的正弦波激励时，钳位部分提供如图 4.36(b) 所示的电压波形 $v_{D1}$，假设为理想二极管。 请注意，虽然正峰值被钳位到 0 V，但负峰值达到 $–2V_p$。 响应该波形，峰值检测器部分在电容器 $C_2$ 两端提供一个直流电压，该电压等于 $v_{D1}$ 的负峰值，即 -$2V_p$。

由于输出电压是输入峰值的两倍，因此该电路称为倍压器，该技术可以扩展到提供高于 $V_p$ 倍数的输出直流电压。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220311115607837.png" alt="image-20220311115607837" style="zoom: 67%;" />

### 4.7 Special Diode Types

在本节中，我们将简要讨论一些重要的特殊类型的二极管。

#### 4.7.1 The Schottky-Barrier Diode (SBD)

肖特基势垒二极管 (SBD) 是通过使金属与中等掺杂的 n 型半导体材料接触而形成的。 由此产生的金属-半导体结表现得像一个二极管，在一个方向（从金属阳极到半导体阴极）传导电流并在另一个方向上充当开路，并且被称为肖特基势垒二极管或简称为肖特基二极管 . 事实上，SBD 的电流-电压特性与 pn 结二极管非常相似，但有两个重要的例外：

1. 在 SBD 中，电流由多数载流子（电子）传导。 因此，SBD 不会表现出在正向偏置 pn 结中发现的少数载流子电荷存储效应。 因此，肖特基二极管可以从开启切换到关闭，反之亦然，这比使用 pn 结二极管要快得多。
2. 导通SBD的正向压降低于pn结二极管。 例如，由硅制成的 SBD 的正向电压降为 0.3 V 至 0.5 V，而硅 pn 结二极管的正向电压降为 0.6 V 至 0.8 V。  SBD 也可以由砷化镓 (GaAs) 制成，事实上，它在 GaAs 电路的设计中发挥着重要作用。砷化镓 SBD 的正向压降约为 0.7 V。

除了 GaAs 电路之外，肖特基二极管还可以用于设计一种特殊形式的双极晶体管逻辑电路，称为肖特基-TTL，其中 TTL 代表晶体管-晶体管逻辑。

 在离开肖特基势垒二极管的主题之前，重要的是要注意并非每个金属-半导体触点都是二极管。 事实上，金属通常沉积在半导体表面上，以便为半导体器件制作端子并连接集成电路芯片中的不同器件。 这种金属-半导体触点被称为欧姆触点，以将它们与导致 SBD 的整流触点区分开来。 欧姆接触通常是通过在重掺杂（因此是低电阻率）半导体区域上沉积金属制成的。  （回想一下，SBD 使用中度掺杂的材料。）

#### 4.7.2 Varactors

在第 3 章中，我们了解到反向偏置 pn 结会表现出电荷存储效应，该效应可以用耗尽层或结电容 $C_j$ 建模。 如方程式 (3.49) 表明，$C_j$ 是反向偏置电压 $V_R$ 的函数。 事实证明，这种依赖性在许多应用中都很有用，例如无线电接收器的自动调谐。 因此，制造了特殊的二极管以用作称为变容二极管的电压可变电容器。 这些器件经过优化，通过安排分级系数 m 为 3 或 4，使电容成为电压的强函数。

#### 4.7.3 Photodiodes

如果反向偏压的 pn 结被照亮（即暴露于入射光），撞击该结的光子会导致共价键断裂，从而在耗尽层中产生电子-空穴对。 然后耗尽区中的电场将释放的电子扫到 n 侧，将空穴扫到 p 侧，从而在结上产生反向电流。 这种电流称为光电流，与入射光的强度成正比。 这种称为光电二极管的二极管可用于将光信号转换为电信号。

光电二极管通常使用化合物半导体制造，例如砷化镓。 光电二极管是越来越多的称为光电子学或光子学的电路系列的重要组成部分。 顾名思义，这种电路利用电子和光学的最佳组合进行信号处理、存储和传输。

> 元素半导体（例如硅）使用元素周期表第 IV 列的元素，而化合物半导体则使用 III 和 V 或 II 和 VI 列的元素的组合。例如，GaAs 由镓（III 列）和砷（V 列）组成，因此被称为 III-V 化合物

通常，电子器件信号处理的首选方式，而光学器件最适合传输和存储。 示例包括电话和电视信号的光纤传输以及 CD-ROM 计算机光盘中光存储的使用。 光传输提供非常宽的带宽和低信号衰减。 光存储允许在狭小的空间内可靠地存储大量数据。

最后，我们应该注意，在没有反向偏置的情况下，照明光电二极管可以用作太阳能电池。 太阳能电池通常由低成本的硅制成，可将光能转化为电能。

#### 4.7.4 Light-Emitting Diodes (LEDs)

发光二极管（LED）执行与光电二极管相反的功能； 它将正向电流转换为光。 读者会记得在第 3 章中，在正向偏置的 pn 结中，少数载流子通过结注入并扩散到 p 和 n 区域。 然后扩散的少数载流子与多数载流子重新组合。 可以进行这种重组以产生光发射。 这可以通过使用称为直接带隙材料类型的半导体制造 pn 结来完成。砷化镓属于这一类，因此可用于制造发光二极管。

LED 发出的光与发生的重组次数成正比，而重组次数又与二极管中的正向电流成正比。LED 是非常流行的器件。 它们可用于设计多种类型的显示器，包括数字电压表等实验室仪器的显示器。 它们可以产生各种颜色的光。 此外，可以设计 LED 以产生具有非常窄带宽的相干光。 由此产生的设备是激光二极管。激光二极管在光通信系统和 DVD 播放器等方面都有应用。

将 LED 与光电二极管组合在同一个封装中会产生一种称为光隔离器的器件。  LED 将施加到光隔离器的电信号转换为光，光电二极管检测到光并将其转换回光隔离器输出端的电信号。 使用光隔离器可在连接到隔离器输入的电路和连接到其输出的电路之间提供完全的电气隔离。 这种隔离可用于减少电气干扰对系统内信号传输的影响，因此光隔离器经常用于数字系统的设计。 它们还可以用于医疗器械的设计，以降低患者触电的风险。

请注意，LED 和光电二极管之间的光耦合不需要在小封装内完成。 实际上，它可以使用光纤在长距离上实现，就像在光纤通信链路中所做的那样。

**从指示到照明：**

曾经仅用作低功率状态指示灯的发光二极管 (LED) 现在正在照亮我们的道路！ 汽车照明越来越多地使用 LED； 同样，越来越大功率的 LED 灯泡正在取代家庭和办公室中的白炽灯和荧光灯。 白炽灯泡将电转化为光的效率只有 5%，而另外 95% 则以热量的形式消散。 然而，LED 的光转换效率为 60%。此外，LED 的使用寿命是白炽灯泡的 25 倍（25,000 小时），是荧光灯的 3 倍。
...
<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210531114508940.png" alt="image-20210531114508940" style="zoom:67%;" />
...
图 6.7 显示了 npn BJT 的更真实（但仍然简化）的横截面。 请注意，集电极实际上围绕发射区，因此注入薄基极的电子难以逃脱被收集。 这样，得到的 $α$ 接近于1，并且 $β$ 很大，此外，请注意该器件是不对称的，因此发射极和集电极不能互换，有关实际器件物理结构的更多详细信息，请参阅附录 A。

> 如果发射极和集电极反向，即 CBJ 正向偏置而 EBJ 反向偏置，则器件以称为“反向激活模式”的模式运行。 得到的 $α$ 和 $β$ 值，表示为 $α_R$ 和 $β_R$（R 表示反向），分别比在上面讨论的“前向”主动模式中获得的 $α$ 和 $β$ 的值低得多。 因此，反向激活模式没有实际应用。 另一方面，MOSFET 是一个完美对称的器件，在其漏极和源极端子互换的情况下也可以同样良好地工作。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220313120349075.png" alt="image-20220313120349075" style="zoom:67%;" />

图 6.7 中的结构也表明 CBJ 的面积比 EBJ 大得多。因此，CB 二极管 $D_C$ 的饱和电流 $I_{SC}$ 远大于 EB 二极管 $D_E$ 的饱和电流。 通常，$I_{SC}$ 比 $I_{SE}$ 大 10 到 100 倍（回想一下 $I_{SE} = I_S/α \simeq IS$）。

#### 6.1.4 Operation in the Saturation Mode

> 饱和在 BJT 和 MOSFET 中意味着完全不同的东西。  BJT 的饱和工作模式类似于 MOSFET 的三极管工作区域。 另一方面，MOSFET 的饱和工作区对应于 BJT 工作的有源模式

如上所述，要使 BJT 工作在有源模式，CBJ 必须反向偏置。到目前为止，我们已经将 npn 晶体管的这个条件规定为 $v_{CB} ≥ 0$。但是，我们知道 pn 结在其正向电压超过大约 0.4 V 之前不会有效地变为正向偏置。**也就是说，事实上我们可以使得 $V_{CB}\simeq-0.4V$ 而不使其脱离 Active Mode，**这在图 6.8 中进行了说明，它是在恒定发射极电流 $I_E$ 下的 npn 晶体管的 $i_C$ 与 $v_{CB}$ 的示意图。 正如预期的那样，**在 Active Mode 下，$i_C$ 与 $v_{CB}$ 无关，这种情况会持续到 $v_{CB}$ 变为负值至大约 -0.4 V。低于这个 $v_{CB}$ 值，CBJ 开始充分导通**，晶体管离开活动模式并进入饱和状态操作模式，其中 $i_C$ 降低。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220313124157519.png" alt="image-20220313124157519" style="zoom:80%;" />

![image-20220313124837393](C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220313124837393.png)

图 6.9 通过在图 6.5(c) 的模型中增加一个**正向导通二极管 $D_C$ 来模拟 npn 晶体管在饱和状态下的操作（因为此时 CBJ 正偏而导通）。** 请注意，**==<u>通过 $D_C$ 的电流会增加 $i_B$ 并降低 $i_C$（因为 集电极电流大小 $i_E$ 不变）。</u>==**

要了解 $i_C$ 在进入饱和模式时下降的原因，我们可以为饱和 npn 晶体管构建一个模型，如下所示。 我们用**正向导通的 CBJ 二极管 $D_C$** 扩充了图 6.5(c) 的模型，如图 6.9 所示。 观察到电流 $i_{BC}$ 将从受控源电流中减去，从而降低集电极电流 $i_C$，由下式给出
$$
i_C = I_Se^{v_{BE} /V_T} -I_{SC} e^{v_{BC} /V_T}\tag{6.14}
$$
其中 $I_{SC}$ 是 $D_C$ 的饱和电流，与 $I_S$ 相关，与 CBJ 和 EBJ 的面积之比有关。 等式 (6.14) 中的第二项将随着 $v_{BC}$ 超过 0.4 V 左右而发挥越来越大的作用，导致 $i_C$ 降低并最终达到零。

图 6.9 还表明，在饱和状态下，基极电流将增加到 
$$
i_B = (I_S / β) e^{v_{BE} / V_T} + I_{SC} e^{v_{BC} / V_T}\tag{6.15}
$$
等式 (6.14) 和 (6.15) 可以结合以获得饱和晶体管的 $i_C/i_B$ 比值。

我们观察到这个比率将低于 $β$ 的值。 此外，该比率将随着 $v_{BC}$ 的增加而降低，并且晶体管被驱动到更深的饱和状态。 因为饱和晶体管的 $i_C/i_B$ 可以通过调整 $v_{BC}$ 设置为低于 $β$ 的任何所需值，所以这个比率称为强制 $β$ 并表示为 $β_{forced}$ 
$$
β_{forced} = \frac{i_C} {i_B}\mid_{saturation} ≤ β\tag{6.16}
$$
如后所述，在分析电路时，我们可以通过以下两个测试之一来确定 BJT 是否处于饱和模式：

1. CBJ 正向偏置是否超过 0.4 V？
2. $i_C/i_B$的比值是否低于$β$？

饱和晶体管的集电极到发射极电压 $v_{CE}$ 可以从图 6.9 中找到，作为 EBJ 和 CBJ 的正向偏置电压之差，
$$
V_{CE_{sat}} = V_{BE} -V_{BC}\tag{6.17}
$$
回想 CBJ 的面积比 EBJ 大得多，$V_{BC}$ 将比 $V_{BE}$ 小 0.1 到 0.3 V。因此，
$$
V_{CE_{sat}}\simeq0.1-0.3V
$$
通常我们会假设处于饱和边缘的晶体管的 $V_{CE_{sat}}$ = 0.3 V，而处于饱和深度的晶体管的 $V_{CE_{sat}}$ = 0.2 V。

#### 6.1.5 The pnp Transistor

pnp晶体管以类似于上述npn器件的方式操作。图 6.10 显示了一个 pnp 晶体管偏置以在活动模式下工作。 **这里电压 $V_{EB}$ 导致 p 型发射极的电位高于 n 型基极，从而正向偏置发射极-基极结。 集电极 - 基极结由电压 $V_{BC}$ 反向偏置**，这使 **==p 型集电极的电位低于 n 型基极==**

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220313132524772.png" alt="image-20220313132524772" style="zoom:80%;" />

与 npn 晶体管不同，**<u>pnp 器件中的电流主要由由于正向偏置电压 $V_{EB}$ 从发射极注入到基极的空穴传导。</u>** 由于从基极注入到发射极的电子所贡献的发射极电流分量通过使用轻掺杂基极而保持较小，因此大部分发射极电流将归因于空穴。 **从基极注入到发射极的电子产生基极电流的第一个分量 $i_{B1}$。 此外，注入基极的许多空穴将与基极中的多数载流子（电子）重新结合，因此消失的基极电子将不得不从外部电路中被替换，从而产生基极电流的第二个分量 $i_{B2}$。 成功到达集电极-基极结耗尽区边界的空穴将被集电极上的负电压吸引。 因此，这些空穴将扫过耗尽区进入集电极并作为集电极电流出现。**

从上面的描述可以很容易地看出，pnp 晶体管的电流-电压关系将与 npn 晶体管的电流-电压关系相同，只是 $v_{BE}$ 必须由 $v_{EB}$ 代替。 此外，pnp 晶体管的大信号、有源模式操作可以通过与图 6.5 中的 npn 晶体管类似的四个等效电路中的任何一个来建模。 这四个电路中的两个如图 6.11 所示。 最后，我们注意到 pnp 晶体管可以以类似于 npn 器件所描述的方式在饱和模式下工作。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220313132923806.png" alt="image-20220313132923806" style="zoom:80%;" />

**THE INVENTION OF THE BJT:**

1947 年末，约翰·巴丁和沃尔特·布拉顿在贝尔实验室演示了第一个工作晶体管，他们是威廉·肖克利领导的团队的一员。 该器件由锗制成，被称为点接触晶体管，并根据场效应原理工作。 然而，在几周之内，肖克利就完成了双极结型晶体管 (BJT) 的完整描述，并申请了题为“利用半导体材料的电路元件”的美国专利。 从 1950 年代初到 1970 年代中期，BJT 主导了电子世界，当时 MOSFET 占据了主导地位。  1956 年，肖克利、巴丁和布拉顿因发现晶体管效应而分享了诺贝尔物理学奖。
...
<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210531142109806.png" alt="image-20210531142109806" style="zoom:80%;" />

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210531143954357.png" alt="image-20210531143954357" style="zoom:80%;" />
...
<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210531145147969.png" alt="image-20210531145147969" style="zoom:67%;" />

考虑如上图所示的共射电路，其中$V_{BE}$和$V_{CE}$可调节，我们在给定的$V_{BE}$处调节$V_{CE}$并测量此时的集电极电流$i_c$，可得到如下图像

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210531145754476.png" alt="image-20210531145754476" style="zoom:67%;" />
...
<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210531153151661.png" alt="image-20210531153151661" style="zoom:67%;" />
...
i_C=\beta_Qi_B
$$
但这次我们采用了不大一样的方法，在上图中，我们确定的工作点 $Q$ 为
$$
...
<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210531154145103.png" alt="image-20210531154145103" style="zoom:67%;" />
...
<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210531154601893.png" alt="image-20210531154601893" style="zoom:67%;" />

饱和 BJT 的集电极到发射极的电阻很小，这是因为我们现在在 C 和 E 之间有两个串联的正向传导二极管（另请参见图 6.9）。

饱和 BJT 的一个简单模型如图 6.21 所示。 这里假定 $V_{BE}$ 为常数（约 0.7 V），$V_{CE}$ 也假定为常数，$V_{CE_{sat}}$ 为 0.2 V。也就是说，我们忽略了小的饱和电阻 $R_{CE_{sat}}$，以便使模型便于手动计算。

### 6.3 BJT Circuits at DC

我们现在准备分析只施加直流电压的 BJT 电路。 在以下示例中，我们将使用简单模型，其中 导通晶体管的$|V_{BE} |$ 为 0.7 V 饱和晶体管的 $|V_{CE} |$为 0.2 V，我们将忽略厄利效应。 这些模型如表 6.3 所示。 当然，更好的模型可以用来获得更准确的结果。 然而，这通常是以牺牲分析速度为代价的。 更重要的是，随之而来的复杂性可能会阻碍电路设计人员深入了解电路行为的能力。 使用带有 SPICE 的电路仿真可以获得使用精细模型的准确结果。 这几乎总是在设计的最后阶段完成，当然在电路制造之前。 然而，计算机模拟并不能替代快速的纸笔电路分析，这是有抱负的电路设计师必须掌握的一项基本能力。 以下一系列示例是朝这个方向迈出的一步。
...
考虑图 6.23(a) 所示的电路，该电路在图 6.23(b) 中重新绘制，以提醒读者本书通篇采用的用于指示与直流电源的连接的约定。 我们希望分析该电路以确定所有节点电压和支路电流。 我们将假设 $β$ 为 100。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220313135426269.png" alt="image-20220313135426269" style="zoom:80%;" />

图 6.23 例 6.4 的电路分析： (a) 电路；  (b) 重新绘制电路以提醒读者本书中使用的惯例来显示与直流电源的连接；  (c) 分析步骤编号。

**Solution**

看一下图 6.23(a) 中的电路，我们注意到基极连接到 +4 V，发射极通过电阻 RE 接地。 因此，有理由得出结论，基极-发射极结将正向偏置。 假设是这种情况并假设 $V_{BE}$ 约为 0.7 V，则发射极电压将为
$$
V_E = 4−V_{BE}\simeq 4−0.7 = 3.3 V
$$
我们知道 $R_E$ 两端的电压，因此可以确定通过它的电流 $I_E$，
$$
I_E = \frac{V_E -0} {R_E} = \frac{3.3} {3.3} =1mA
$$
由于集电极通过 $R_C$ 连接到 +10V 电源，因此集电极电压可能会高于基极电压，这意味着有源模式操作。 **==假设是这种情况==**，我们可以计算集电极电流为
$$
I_C=\alpha I_E
$$
$α$ 的值由下式获得
$$
α = \frac{β} {β +1} = \frac{100} {101} \simeq 0.99mA
$$
我们现在可以使用欧姆定律来确定集电极电压 $V_C$，
$$
V_C = 10−I_CR_C = 10−0.99×4.7\simeq +5.3V
$$
由于基极为 +4 V，集电极-基极结反向偏置 1.3 V，晶体管**确实处于假设的有源模式**。现在剩下要做的的只是确定基极电流 $I_B$，如下：
$$
I_B = \frac{I_E} {β +1} = \frac1 {101}\simeq 0.01 mA
$$
在离开这个例子之前，我们想强调直接在电路图上进行分析的价值。 只有这样，才能在合理的时间内分析复杂的电路。 图 6.23(c) 说明了上述对电路图的分析，分析步骤的顺序由带圆圈的数字表示。

#### Example 6.5

我们希望分析图 6.24(a) 的电路以确定所有节点的电压和通过所有分支的电流。 请注意，该电路与图 6.23 的电路相同，只是基极电压现在为 +6 V。假设晶体管 $β$ 指定为至少 50。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220313141001549.png" alt="image-20220313141001549" style="zoom: 67%;" />

**Solution**

在基极 +6 V 时，基极 - 发射极结将正向偏置； 因此，
$$
V_E =+6−V_{BE}\simeq 6−0.7 = 5.3 V
$$
以及
$$
I_E = \frac{5.3} {3.3} = 1.6 毫安
$$
现在，假设三极管处于 Active Mode，$I_C = αI_E\simeq I_E$； 因此，
$$
V_C = + 10−4.7 × I_C\simeq 10−7.52 = 2.48 V
$$
图 6.24(b) 说明了上述分析的细节。

由于计算的集电极电压比基极电压小 3.52 V，**因此我们最初的三极管处于 Active Mode 的假设是不正确的**。 事实上，晶体管必须处于饱和模式。假设是这种情况，$V_E$ 和 $I_E$ 的值将保持不变。 然而，集电极电压变为 
$$
V_C = V_E + V_{CE_{sat}}\simeq 5.3 + 0.2 = + 5.5 V
$$
从中我们可以确定 $I_C$ 为
$$
I_C = \frac{10−5.5} {4.7} = 0.96 mA
$$
现在可以计算 $I_B$
$$
I_B = I_E -I_C = 1.6-0.96 = 0.64 mA
$$
因此，晶体管在强制 $β$ 下工作
$$
β_{forced} = \frac{I_C} {I_B} = \frac{0.96} {0.64} = 1.5
$$
由于 $β_{forced}$ 小于 $β$ 的最小规定值，因此晶体管确实饱和。 我们应该在这里强调，在测试饱和度时，应该使用 $β$ 的最小值。 同理，如果我们正在设计一个晶体管要饱和的电路，则设计应基于指定的最小 $β$。 显然，如果具有此最小 $β$ 的晶体管饱和，则具有较高 $β$ 值的晶体管也将饱和。 分析的细节如图 6.24(c) 所示，其中所用步骤的顺序由带圆圈的数字表示。

#### Example 6.6

我们希望分析图 6.25(a) 中的电路，以确定所有节点的电压和通过所有分支的电流。 请注意，该电路与示例 6.4 和示例 6.5 中考虑的电路相同，只是现在基极电压为零。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220313145212708.png" alt="image-20220313145212708" style="zoom:80%;" />

**Solution**

由于基极电压为零，发射极通过 $R_E$ 接地，因此基极-发射极结不能导通，发射极电流为零。 此外，集电极-基极结不能导通，因为 n 型集电极通过 $R_C$ 连接到正电源，而 p 型基极接地。 因此，集电极电流将为零。 基极电流也必须为零，晶体管处于截止工作模式。

发射极电压为零，而集电极电压将等于 +10 V，因为 $R_E$ 和 $R_C$ 两端的电压降为零。 图 6.25(b) 显示了分析细节。

#### Example 6.7

我们想分析图 6.26(a) 的电路以确定所有节点的电压和通过所有分支的电流。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220313145422093.png" alt="image-20220313145422093" style="zoom: 80%;" />

**Solution**

该 pnp 晶体管的基极接地，而发射极通过 $R_E$ 连接到正电源 ($V^+=+10 V$)。 因此，发射极 - 基极结将正向偏置
$$
V_E = V_{EB}\simeq 0.7 V
$$
因此发射极电流将由下式给出
$$
I_E = \frac{V^+ -V_E} {R_E} = \frac{10-0.7} 2 = 4.65 mA
$$
由于集电极通过 $R_C$ 连接到负电源（比基极电压更负），因此该晶体管**==可能==**在活动模式下工作。 **==假设是这种情况==**，我们得到
$$
I_C=\alpha I_E
$$
由于没有给出 $β$ 的值，我们将假设 $β=100$，这导致 $α = 0.99$。 由于 $β$ 的较大变化会导致 $α$ 的微小差异，因此就确定 $I_C$ 的值而言，该假设并不重要。 因此，
$$
I_C = 0.99×4.65 = 4.6 毫安
$$
集电极电压为
$$
V_C = V^− +I_CR_C =−10+4.6×1=−5.4V
$$
因此，集电极-基极结反向偏置 5.4 V，晶体管确实处于有源模式，这支持了我们最初的假设，剩下的只是计算基极电流，
$$
I_B = \frac{I_E} {β +1} = \frac{4.65} {101}\simeq 0.05 mA
$$
显然，$β$ 的值会严重影响基极电流的大小，但是请注意，在该电路中，$β$ 的值对晶体管的工作模式没有影响。 由于 $β$ 通常是一个错误指定的参数，因此该电路代表了一个好的设计。 作为一项规则，**人们应该努力设计电路，使其性能对 $β$ 的值尽可能不敏感。** 分析细节如图 6.26(b) 所示。

#### Example 6.8

我们要分析图 6.27(a) 中的电路，以确定所有节点的电压和所有支路的电流。 假设 $β = 100$。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220313150950068.png" alt="image-20220313150950068" style="zoom:80%;" />

**Solution**

基极-发射极结明显正向偏置。 因此，
$$
I_B = \frac{+5−V_{BE}} {R_B}\simeq \frac{5−0.7} {100} = 0.043 mA
$$
假设晶体管工作在有源模式，我们现在可以得到
$$
I_C = βI_B = 100×0.043 = 4.3 mA
$$
集电极电压现在可以确定为
$$
V_C = 10−I_CR_C = 10−4.3×2=+1.4V
$$
由于基极电压 $V_B$ 是
$$
V_B = V_{BE}\simeq +0.7 V
$$
因此，集电极 - 基极结反向偏置 0.7 V，晶体管确实处于 Active Mode。 发射极电流将由下式给出
$$
I_E = (β + 1) I_B = 101 × 0.043\simeq 4.3 mA
$$
我们从这个例子中注意到，**==集电极和发射极电流严重依赖于 $β$ 的值==**。 事实上，如果 $β$ 高出 10%，晶体管将离开活动模式并进入饱和状态。 因此，这显然是一个糟糕的设计。 分析细节如图 6.27(b) 所示。

#### Example 6.9

我们想分析图 6.28(a) 的电路以确定所有节点的电压和通过所有分支的电流。  $β$ 的最小值指定为 30。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220313151620852.png" alt="image-20220313151620852" style="zoom:80%;" />

**Solution**

快速浏览一下这个电路就会发现晶体管要么处于 Active Mode，要么处于饱和状态。 假**==设三极管处于 Active Mode 并忽略基极电流，我们看到基极电压将接近零伏，发射极电压将接近 +0.7 V，发射极电流将接近 4.3 mA。 由于集电极在晶体管保持 Active Mode 时可以支持的最大电流约为 0.5 mA，因此晶体管肯定是饱和的。==**

假设晶体管饱和并用 $V_B$ 表示基极电压（参见图 6.28b），则可以得出
$$
\begin{aligned}&V_E = V_B +V_{EB}\simeq V_B +0.7\\&V_C = V_E -V_{EC_{sat}}\simeq V_B +0.7-0.2 = V_B +0.5\\&I_E = \frac{+5-V_E} 1 = \frac{5-V_B -0.7} 1 = 4.3-V_B \ mA\\&I_B = \frac{V_B} {10} = 0.1V_B\ mA\\&I_C = \frac{V_C -(-5)} {10} = \frac{V_B +0.5+5} {10 }= 0.1V_B +0.55\  mA\end{aligned}
$$
使用关系 $I_E = I_B +I_C$，我们得到
$$
4.3−V_B = 0.1V_B +0.1V_B +0.55
$$
得到结果
$$
V_B = \frac{3.75} {1.2}\simeq 3.13 V
$$
代入上述等式，我们得到
$$
\begin{aligned}&V_E = 3.83 V\\& V_C = 3.63 V\\& I_E = 1.17 mA\\& I_C = 0.86 mA\\& I_B = 0.31 mA\end{aligned}
$$
从中我们看到晶体管已经饱和，因为强制 $β$ 的值是
$$
β_{forced }= \frac{0.86} {0.31} \simeq 2.8
$$
远小于指定的最小值 $β$

#### Example 6.10

我们想分析图 6.29(a) 的电路以确定所有节点的电压和通过所有分支的电流，假设 $β = 100$。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220313153335397.png" alt="image-20220313153335397" style="zoom:80%;" />

**Solution**

分析的第一步是使用 Th'evenin 定理简化基本电路。 结果如图 6.29(b) 所示，其中
$$
\begin{aligned}&V_{BB} =+15 \frac{R_{B2} }{R_{B1} +R_{B2}} = 15 \frac{50 }{100+50} =+5 V \\&R_{BB} = R_{B1}|| R_{B2} =100 ||50= 33.3 k\Omega\end{aligned}
$$
为了计算基极或发射极电流，我们必须围绕图 6.29(b) 中标记为 $L$ 的回路编写一个回路方程。 但是请注意，通过 $R_{BB}$ 的电流与通过 $R_E$ 的电流不同，回路方程将是
$$
V_{BB} = I_BR_{BB} + V_{BE} + I_ER_E
$$
现在，假设 Active Mode ，我们将 $I_B$ 替换为
$$
I_B=\frac{I_E}{\beta+1}
$$
并重新排列方程，得到
$$
I_E = \frac{V_{BB} -V_{BE}} {R_E + [R_{BB} / (β +1)]}
$$
对于给定的数值，我们有
$$
I_E = \frac{5−0.7} {3+ (33.3 / 101)} = 1.29 mA
$$
基极电流为
$$
I_B = \frac{1.29} {101} = 0.0128 毫安
$$
基极电压由下式给出
$$
\begin{aligned}V_B& = V_{BE} + I_ER_E \\&= 0.7 + 1.29 × 3 = 4.57 V\end{aligned}
$$
我们可以将集电极电流计算为
$$
I_C = αI_E = 0.99 × 1.29 = 1.28 mA
$$
集电极电压现在可以计算为
$$
V_C=+15−I_CR_C=15−1.28×5=8.6V
$$
由此可见，集电极的电位比基极高 4.03 V，这意味着晶体管处于活动模式，正如所假设的那样。 分析结果如图 6.29(c,d) 所示。

#### Example 6.11

我们希望分析图 6.30(a) 中的电路，以确定所有节点的电压和通过所有分支的电流

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220313161101759.png" alt="image-20220313161101759" style="zoom:80%;" />

**Solution**

我们首先认识到该电路的一部分与我们在示例 6.10 中分析的电路相同——即图 6.29(a) 的电路。 当然，不同之处在于，在新电路中，**我们有一个额外的晶体管 $Q_2$ 及其相关的电阻器 $R_{E2}$ 和 $R_{C2}$。 假设 $Q_1$ 仍处于 Active Mode，**以下值将与上一个示例中获得的值相同：
$$
\begin{aligned}&V_{B1} =+4.57 V\ \ \ \  I_{E1} = 1.29 mA\\ &I_{B1} = 0.0128 mA\ \ \ \  I_{C1} = 1.28 mA\end{aligned}
$$
然而，**<u>集电极电压将与先前计算的不同</u>**，因为**==部分集电极电流 $I_{C1}$ 将流入 $Q_2$ ($I_{B2}$) 的基极引线==**。 作为第一个近似值，我们可以假设 $I_{B2}$ 远小于 $I_{C1}$； 也就是说，我们可以假设通过 $R_{C1}$ 的电流几乎等于 $I_{C1}$。 这将使我们能够计算 $V_{C1}$：
$$
\begin{aligned}&V_{C1} \simeq+15−I_{C1}R_{C1}\\& = 15−1.28×5=+8.6 V\end{aligned}
$$
因此，正如所假设的那样，$Q_1$ 处于 Active Mode。

就 $Q_2$ 而言，我们注意到其发射极通过 $R_{E2}$ 连接到 +15 V。 因此可以安全地假设 $Q_2$ 的发射极 - 基极结将被正向偏置。 因此，$Q_2$ 的发射极将处于由下式给出的电压 $V_{E2}$
$$
V_{E2} = V_{C1} + V_{EB}|_ {Q_2}\simeq 8.6+0.7=+9.3V
$$
$Q_2$ 的发射极电流现在可以计算为
$$
I_{E2} = \frac{+15−V_{E2}} {R_{E2}} = 15−9.3 2 = 2.85 mA
$$
由于 $Q_2$ 的集电极通过 $R_{C2}$ 接地，因此 $Q_2$ 可能在活动模式下运行，假设是这种情况。 我们现在发现 $I_{C2}$ 为 
$$
\begin{aligned}I_{C2} &= α_2I_{E2} = 0.99×2.85 \\&= 2.82 mA (假设 β_2 = 100)\end{aligned}
$$
$Q_2$ 的集电极电压为
$$
V_{C2}=I_{C2}R_{C2}=2.82×2.7=7.62V
$$
它比 $V_{B2}$ 低 0.98 V。因此，假设 $Q_2$ 处于活动模式是正确的。

在这个阶段，通过假设 $I_{B2}$ 可以忽略不计，找出我们计算中产生的误差幅度很重要, $I_{B2}$ 的值由下式给出
$$
I_{B2} = \frac{I_{E2}} {β_2 +1} = \frac{2.85} {101} = 0.028 mA
$$
这确实比 $I_{C1} (1.28 mA)$ 小得多。 如果需要，我们可以通过**<u>再迭代一次</u>**来获得更准确的结果，假设 $I_{B2}$ 为 0.028 mA。 新值将是
$$
\begin{aligned}&R_{C1} = I_{C1} -I_{B2} = 1.28-0.028 = 1.252 mA \\&V_{C1} = 15-5 × 1.252 = 8.74 V \\&V_{E2} = 8.74 + 0.7 = 9.44 V \\&I_{E2} = \frac{15-9.44} 2 = 2.78 mA\\& I_{C2} = 0.99 × 2.78 = 2.75 mA\\& V_{C2} = 2.75 × 2.7 = 7.43 V \\&I_{B2} = \frac{2.78} {101} = 0.0275 mA\end{aligned}
$$
请注意，$I_{B2}$ 的新值非常接近我们迭代中使用的值，不需要进一步的迭代。 最终结果如图 6.30(b) 所示。

读者有理由想知道在解决线性（或线性化）问题时使用迭代方案的必要性。 事实上，我们可以通过编写适当的方程来获得精确的解决方案（如果我们可以用一阶模型精确地调用我们所做的任何事情！）。 鼓励读者找到这个解决方案，然后将结果与上面获得的结果进行比较。 然而，重要的是要强调，在大多数此类问题中，只要我们能够快速且当然正确地获得近似解，就足以获得近似解。

在上述示例中，我们经常使用精确的 $α$ 值来计算集电极电流。 由于 $α\simeq 1$，如果假设 $α = 1$ 且 $I_C = I_E$，此类计算中的误差将非常小。 因此，除了严重依赖于 $α$ 值的计算（例如，基极电流的计算）外，通常假设 $α\simeq 1$。

####  Example 6.12

我们希望计算图 6.31(a) 电路中所有节点的电压和通过所有分支的电流。 假设 $β = 100$。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220313182552324.png" alt="image-20220313182552324" style="zoom:80%;" />

**Solution**

通过检查电路，我们得出结论，两个晶体管 $Q_1$ 和 $Q_2$ 不能同时导通。 因此，如果 $Q_1$ 开启，$Q_2$ 将关闭，反之亦然。 假设 $Q_2$ 开启。 因此，电流将从地通过 1-k 电阻器流入 $Q_2$ 的发射极。 因此，$Q_2$ 的基极将处于负电压，基极电流将通过 10k 电阻从基极流出并流入 +5V 电源。 这是不可能的，因为如果基极为负，则 10k 电阻器中的电流将不得不流入基极。 因此，我们得出结论，我们最初的假设——$Q_2$ 开启——是不正确的。 因此，$Q_2$ 将关闭，$Q_1$ 将打开。

现在的问题是 $Q_1$ 是处于 Active Mode 的还是饱和状态的，这种情况下的答案很明显：由于基极由 +5V 电源供电，并且由于基极电流流入 $Q_1$ 的基极，因此 $Q_1$ 的基极电压将低于 +5V。因此 $Q_1$ 的集电极-基极结反向偏置，$Q_1$ 处于活动模式。 剩下的只是使用已经详细描述的技术来确定电流和电压。 结果如图 6.31(b) 所示。
...
我们在本章结束时简要讨论了 BJT 中的两个重要的非理想效应：电压击穿，以及 $β$ 对 $I_C$ 和温度的依赖性

#### 6.4.1 Transistor Breakdown  

可以施加到 BJT 的最大电压受到 EBJ 和 CBJ 击穿效应的限制，这些击穿效应遵循第 3.5.3 节中描述的雪崩倍增机制。首先考虑共基电路（图 6.32(a)）。 图 6.32(b) 中的 $i_C-v_{CB}$ 特性表明，对于 $i_E =0$（即发射极开路），集电极-基极结在 $BV_{CBO}$ 表示的电压下击穿。 对于 $i_E >0$，击穿发生在小于 $BV_{CBO}$ 的电压下。 通常，对于分立 BJT，$BV_{CBO}$ 大于 50 V  <img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220314141450443.png" alt="image-20220314141450443" style="zoom:67%;" />  <img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220314141503522.png" alt="image-20220314141503522" style="zoom:67%;" />  接下来考虑图 6.33 的共发射极特性，它显示了在电压 $BV_{CEO}$ 下发生的击穿。 在这里，虽然击穿仍然是雪崩类型，但对特性的影响比共基配置更复杂。 我们不会详细解释这些； 指出通常 $BV_{CEO}$ 约为 $BV_{CBO}$ 的一半就足够了。 在晶体管数据表中，$BV_{CEO}$ 有时被称为维持电压 $LV_{CEO}$。

**<u>只要器件的功耗保持在安全范围内，CBJ 在共基极或共发射极配置中的击穿都不会造成破坏。</u>**然而，发射极-基极结的击穿情况并非如此。  EBJ 在电压 $BV_{EBO}$ 远小于 $BV_{CBO}$ 时以雪崩方式击穿。 通常，$BV_{EBO}$ 在 6 V 到 8 V 的范围内，**==并且从晶体管的 $β$ 永久降低的意义上说，击穿是破坏性的。==** 这并不妨碍在 IC 设计中使用 EBJ 作为齐纳二极管来生成参考电压。 在这样的应用中，人们不关心 $β$-降解效应。 第 13 章将讨论防止 IC 放大器中 EBJ 击穿的电路布置。晶体管击穿和最大允许功耗是功率放大器设计中的重要参数（第 12 章）。
...
<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210601130839668.png" alt="image-20210601130839668" style="zoom:67%;" />
...
晶体管（MOSFET或BJT）在放大器设计中的**<u>应用基础是当器件==工作在 Active Mode 时==，实现==压控电流源==。</u>** 具体来说，当 MOSFET 工作在饱和区或夹断区（本章也称为有源区）时，栅极和源极之间的电压 $v_{GS}$ 根据平方律关系控制漏极电流 $i_D$， 对于 NMOS 晶体管，表示为
$$
i_D = \frac1 2 k_n(v_{GS} -V_{tn})^ 2\tag{7.1}
$$
我们注意到，在这种 MOSFET 操作的一阶模型中，漏极电流 $i_D$ 不取决于漏极电压 $v_{DS}$，因为沟道在漏极端被夹断，从而“隔离”了漏极。

类似地，当 BJT 工作在有源区时，基极-发射极电压 $v_{BE}$ 根据指数关系控制集电极电流 $i_C$，对于 npn 晶体管，该指数关系表示为
$$
i_C = I_S e^{v_{BE}/V_T}\tag{7.2}
$$
在这里，这个 BJT 操作的一阶模型表明**<u>集电极电流 $i_C$ 不依赖于集电极电压 $v_{CE}$，因为集电极 - 基极结是反向偏置的，因此“隔离”了集电极。</u>**

图 7.1 显示了一个 NMOS 晶体管和一个在活动模式下工作的 npn 晶体管。观察到对于 NMOS 晶体管，夹断条件是通过保持 $v_{DS} ≥ v_{OV}$ 来确保的。由于过驱动电压 $v_{OV} =v_{GS} -V_{tn}$，这个条件意味着 $v_{GD} ≤V_{tn}$，这确实确保了漏端的沟道夹断

类似地，对于图 7.1(b) 中的 npn 晶体管，通过保持 $v_{CE} ≥ 0.3 V$ 来确保 CBJ 反向偏置条件。由于 $v_{BE}$ 通常在 0.7 V 附近，因此 $v_{BC}$ 保持小于 0.4 V，这足以防止这种相对大面积的结导通。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220314142559553.png" alt="image-20220314142559553" style="zoom:67%;" />

虽然我们使用 NMOS 和 npn 晶体管来说明有源模式操作的条件，但类似的条件也适用于 PMOS 和 pnp 晶体管，分别在第 5 章和第 6 章中进行了研究。

最后，我们注意到方程中的控制关系，即，(7.1) 和 (7.2) 是非线性的。尽管如此，我们很快将设计一种技术，从这些基本非线性的函数中获得几乎呈现为线性的放大。

#### 7.1.2 Obtaining a Voltage Amplifier

从上面我们看到晶体管基本上是一个**跨导放大器**：即**==输入信号为电压，输出信号为电流==**的放大器。 然而，更常见的是，人们对电压放大器感兴趣。 将跨导放大器转换为电压放大器的一种简单方法是将**<u>输出电流通过电阻器并将电阻器两端的电压作为输出。</u>** 对 MOSFET 执行此操作会产生如图 7.2(a) 所示的简单放大器电路。 这里 $v_{GS}$ 是输入电压，**$R_D$（称为==负载电阻==）<u>将漏极电流 $i_D$ 转换为电压 ($i_DR_D$)</u>**，$V_{DD}$ 是为放大器供电的电源电压，并与 $R_D$ 一起在有源区建立操作 , 很快就会显示。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220314143230861.png" alt="image-20220314143230861" style="zoom:67%;" />

在图 7.2(a) 的放大器电路中，输出电压取自漏极和地之间，而不是简单地跨过 $R_D$。 这样做是因为需要在输入和输出之间保持公共接地参考。 输出电压 $v_{DS}$ 由下式给出
$$
v_{DS} = V_{DD} -i_DR_D\tag{7.3}
$$
因此，**它是 $i_DR_D$ 的反相版本**（注意减号），偏移了电源电压 $V_{DD}$ 的恒定值。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220314145548454.png" alt="image-20220314145548454" style="zoom:67%;" />

如图 7.2(c) 所示，BJT 放大器采用了完全相同的布置，这里的输出电压 $v_{CE}$ 由下式给出
$$
v_{CE} = V_{CC} -i_C\tag{7.4}
$$

#### 7.1.3 The Voltage-Transfer Characteristic (VTC)

深入了解放大器电路操作的一个有用工具是其**==电压传输特性==** (VTC)。 这只是输出电压与输入电压的关系图（或清晰标记的草图）。 对于图 7.2(a) 中的 MOS 放大器，这是图 7.2(b) 所示的 $v_{DS}$ 与 $v_{GS}$ 的关系图。

观察到对于 $v_{GS} < V_t$，晶体管被切断，$i_D = 0$，并且从方程式 (7.3)，$v_{DS} = V_{DD}$。当 $v_{GS}$ 超过 $V_t$ 时，晶体管导通并且 $v_{DS}$ 降低。 然而，由于最初的 $v_{DS}$ 仍然很高，MOSFET 将在饱和区或有源区工作。 这会随着 $v_{GS}$ 的增加而继续，直到达到 $v_{GS}$ 的值，导致 $v_{DS}$ 比 $v_{GS}$ 低 $V_t$ 伏特[图 7.2(b) 中 VTC 上的 B 点]。 当 $v_{GS}$ 大于 B 点时，晶体管工作在三极管区域，$v_{DS}$ 下降得更慢。

图 7.2(b) 中的 VTC 表明**<u>最大斜率</u>**的部分（因此可能是最大的放大器增益）是标记为 AB 的部分，它对应于有源区的操作（**即，在这一区域，只要稍稍改变 $V_{GS}$ 的值就能得到很大的 $V_{DS}$ 变化）。**

当 MOSFET 作为放大器工作时，其工作点始终局限于 AB 段。 段 AB 的表达式可以通过代入等式（7.3）中的 $i_D$ 来获得，联立等式 （7.1）得到
$$
v_{DS} = V_{DD} - \frac1 2 k_nR_D (v_{GS} -V_t)^ 2\tag{7.5}
$$
这显然是非线性关系。 然而，可以通过使用 MOSFET 偏置技术获得线性（或几乎线性）放大。 然而，在考虑偏置之前，确定点 $B$ 的坐标是有用的，该点位于饱和区和三极管工作区域之间的边界。 这些可以通过代入方程式 (7.5) 获得，即，$v_{GS} = V_{GS}|_ B$ 和 $v_{DS} = V_{DS}|_ B = V_{GS}|_ B -V_t$，结果是
$$
V_{GS}|_ B = V_t + \frac{\sqrt{2k_nR_DV_{DD} +1} -1} {k_nR_D}\tag{7.6}
$$
B 点也可以通过过驱动电压来表征
$$
V_{OV}|_ B ≡ V_{GS}|_ B -V_t = \frac{\sqrt{2k_nR_DV_{DD} +1} -1} {k_nR_D}\tag{7.7}
$$
以及
$$
V_{DS}|_ B = V_{OV}|_ B\tag{7.8}
$$
可以用同样的方法来处理 BJT 时的情况：这在图 7.2(c) 和 (d) 中进行了说明。 在这种情况下，在有源区或放大器段 AB 上，输出电压 $v_{CE}$ 与输入电压 $v_{BE}$ 的关系为
$$
v_{CE} = V_{CC} -R_CI_S e^{v_{BE}/V_T}\tag{7.9}
$$
在这里，输入-输出关系也是非线性的，然而，线性（或几乎线性）放大可以通过使用下面讨论的偏置技术来获得。

#### 7.1.4 Obtaining Linear Amplification by Biasing the Transistor

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220314163636773.png" alt="image-20220314163636773" style="zoom:67%;" />

偏置使我们能够从 MOSFET 和 BJT 获得几乎线性的放大。 图 7.3(a) 中针对 MOSFET 的情况说明了该技术。 选择直流电压 $V_{GS}$ 以在 VTC 的 AB 段上的点 $Q$ 处获得该放大，稍后将讨论如何为偏置点 $Q$ 选择合适的位置。 暂时，观察 $Q$ 的坐标是直流电压 $V_{GS}$ 和 $V_{DS}$，它们的关系为
$$
V_{DS} = V_{DD} - \frac1 2 k_nR_D (V_{GS} -V_t)^ 2\tag{7.10}
$$
**$Q$ 点称为==<u>偏置点</u>==或==<u>直流工作点</u>==**。 此外，由于在 $Q$ 处不存在信号分量，因此它也被称为静止点（它是符号 $Q$ 的原点）。

接下来，要放大的信号 $v_{gs}$ 是时间 $t$ 的函数，**叠加在偏置电压 $V_{GS}$ 上**，如图 7.4(a) 所示。 因此 $v_{GS}$ 的总瞬时值变为
$$
v_{GS} (t) = V_{GS} + v_{gs} (t)
$$
可以通过将 $v_{GS}(t)$ 代入等式 (7.5) 来获得所得的 $v_{DS}(t)$，从图形上看，我们可以使用 VTC 逐点获得 $v_{DS}(t)$，如图 7.4(b) 所示。 在这里，我们展示了 $v_{gs}$ 是“小”幅度的三角波的情况。 具体来说，**<u>$v_{gs}$ 的幅度足够小，可以将瞬时工作点的偏移限制在偏置点 $Q$ 周围的 VTC 的一个短的、几乎线性的段</u>**。**==段越短，实现的线性度越高，并且三角波输出端的信号分量 $v_{ds}$ 将越接近理想情况==**，这是从非线性MOSFET获得线性放大的本质。

在离开图 7.4(b) 之前，我们希望引起读者注意增加信号幅度 $v_{gs}$ 的后果，由于瞬时工作点将不再局限于 VTC 的几乎线性段，输出信号 $v_{ds}$ 将偏离其理想的三角形形状； 也就是说，**它会表现出非线性失真**。 更糟糕的是，如果输入信号幅度变得足够大，瞬时工作点可能会完全离开 AB 段。 如果**这种情况发生在 $v_{gs}$ 的负峰值处，晶体管将在周期的一部分内截止**，并且 $v_{ds}$ 的**正峰值将被“截断”**。 **<u>如果它出现在 $v_{gs}$ 的正峰值处，三极管会在一部分周期内进入三极管区，$v_{ds}$ 的负峰值会变平。</u>** 由此可见，偏置点 $Q$ 位置的选择会对 $v_{ds}$ 的最大允许幅度产生深远影响，称为输出端的允许信号摆幅。 关于这一重要点，我们稍后会说更多。

BJT 放大器可以应用完全平行的开发。 事实上，我们需要做的只是更换图 2 中的 NMOS 晶体管。  7.3 和 7.4 带有一个 npn 晶体管，并将电压和电流符号更改为对应的 BJT。 产生的偏置点 $Q$ 将由直流电压 $V_{BE}$ 和 $V_{CE}$ 表征，其关系如下
$$
V_{CE} = V_{CC} -R_CI_S e^{V_{BE}/V_T}\tag{7.11}
$$
直流电流 $I_C$
$$
I_C = I_S e^{V_{BE}/V_T}\tag{7.12}
$$
此外，在直流偏置电压 $V_{BE}$ 上叠加一个小信号 $v_{be}$ 会导致
$$
v_{BE}(t) = V_{BE} +v_{be}(t)
$$
<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220314164049862.png" alt="image-20220314164049862" style="zoom:67%;" />

可以代入方程式 (7.9) 得到输出电压 $v_{CE}(t)$ 的总瞬时值，同样，通过保持 $v_{be}$ 足够小以将瞬时工作点的偏移限制在偏置点 $Q$ 附近的 VTC 的一个短的、几乎线性的段，从而获得几乎线性操作，并且同样，输出信号的摆幅问题与场效应管中的讨论一样。

#### 7.1.5 The Small-Signal Voltage Gain

**The MOSFET Case：**考虑图 7.4(a) 中的 MOSFET 放大器，如果输入信号 $v_{gs}$ 保持较小，则输出上的相应信号 $v_{ds}$ 将几乎与 $v_{gs}$ 成比例，**比例常数是 VTC 在 $Q$ 附近的几乎线性段的斜率**。这是放大器的电压增益 ，**==其值可以通过计算在偏置点 $Q$ 处 VTC 的切线斜率来确定，==**
$$
A_v = \frac{dv_{DS}} {dv_{GS}}\bigg|_{v_{GS}} = V_{GS}\tag{7.13}
$$
利用方程式 (7.5) 我们得到
$$
A_v = -k_n (V_{GS} -V_t) R_D\tag{7.14}
$$
可以用偏置点处的过驱动电压 $V_{OV}$ 表示为
$$
A_v =-k_nV_{OV}R_D\tag{7.15}
$$
我们对电压增益的这个表达式进行以下观察。

1. 增益为负，表示放大器正在**反相**； 也就是说，**==输入和输出之间存在 180° 相移==**。 这种倒置在图 7.4(b) 中是显而易见的，应该可以从方程式 (7.5) 中预测到。
2. 增益与负载电阻 $R_D$、晶体管跨导参数 $k_n$ 和过驱动电压 $V_{OV}$ 成正比。 这一切都很直观。

回想一下，偏置点处漏极中的直流电流与 $V_{OV}$ 的关系为
$$
I_D = \frac1 2 k_nV^2 _{OV}
$$
该等式可以与等式 (7.15) 结合，得到
$$
A_v = - \frac{I_DR_D} {V_{OV} / 2}\tag{7.16}
$$
也就是说，增益只是负载电阻 $R_D$ 上的直流电压降与 $V_{OV} /2$ 的比值，可以用另一种形式表示
$$
A_v= -\frac{V_{DD} -V_{DS}} {V_{OV} / 2}\tag{7.17}
$$
由于图 7.4(b) 中 VTC 的最大斜率出现在 $B$ 点，因此最大增益幅度 $|A_{vmax} |$ 通过在 B 点偏置晶体管获得，
$$
|A_{vmax} | = \frac{V_{DD} −V_{DS}|_ B} {V_{OV}|_B /2}\tag{7.18}
$$
由于 $V_{DS}|_ B = V_{OV}|_ B$，
$$
|Avmax | = \frac{V_{DD} −V_{OV}|_B}   {V_{OV} |_ B/2}\tag{7.18}
$$
其中 $V_{OV}|_ B$ 由方程式 (7.7)给出，当然，该结果仅具有理论上的重要性，<u>因为 B 处的偏置不会为输出端的负信号摆幅留下空间</u>。 然而，方程式 （7.18）中的结果很有价值，因为**它提供了这个基本放大器电路可实现的电压增益幅度的上限。** 例如，对于以 $V_{DD} = 5 V$ 和 $V_{OV}|_ B = 0.5 V$ 运行的分立电路放大器，可实现的最大增益为 $18 V/V$。 采用现代亚微米 MOSFET 的集成电路放大器在 $V_{DD} = 1.3 V$ 和 $V_{OV}|_ B = 0.2 V$ 下运行，可实现 11 V/V 的最大增益。

最后，请注意，**为了最大化增益，偏置点 $Q$ 应尽可能靠近点 B**，与输出端所需的信号摆幅一致。 这一点将在本章末尾的问题中进一步探讨

**The BJT Case**

类似的开发可用于获得 BJT 放大器的小信号电压增益，如图 7.6 所示
$$
A_v = \frac{dv_{CE} }{dv_{BE}}\bigg| _{v_{BE}= V_{BE}}\tag{7.19}
$$
<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220314191813390.png" alt="image-20220314191813390" style="zoom:67%;" />

图 7.6 BJT 放大器偏置在 $Q$ 点，小电压信号 $v_{be}$ 叠加在直流偏置电压 $V_{BE}$ 上，产生的输出信号 $v_{ce}$ 叠加在直流集电极电压 $V_{CE}$ 上，$v_{ce}$ 的幅度比 $v_{be}$ 的幅度大了电压增益 $A_v$ 

利用方程式（7.9）连同等式 (7.12)，我们得到
$$
A_v = -\left( \frac{I_C} {V_T}\right) R_C\tag{7.20}
$$
我们对电压增益的这个表达式进行以下观察： 

1. 增益为负，表示放大器正在反相； 也就是说，输入和输出之间存在 180° 相移。 这种反转应该是从方程式 (7.9) 中预料到的。

  2. **增益与集电极偏置电流 $I_C$ 和负载电阻 $R_C$ 成正比。**

可以通过将方程式 (7.20) 改写为下面的形式来进一步理解电压增益 $A_v$ 
$$
A_v = -\frac{I_CR_C} {V_T}\tag{7.21}
$$
也就是说，**<u>==增益是负载电阻 $R_C$ 上的直流电压降与物理常数 $V_T$ 之比==</u>（回想一下，室温下的热电压 $V_T$ 为 25 mV）**。 这种关系在形式上类似于 MOSFET（方程式 7.16），只是这里的分母是物理常数 ($V_T$) 而不是设计参数 ($V_{OV} /2$)。 通常，$V_{OV} /2$ 大于 ($V_T$)，因此我们可以从 BJT 放大器获得比 MOSFET 放大器更高的电压增益。 **这应该不足为奇，因为指数 $i_C-v_{BE}$ 关系比平方律关系 $i_D-v_{GS}$ 陡峭得多。**

等式  (7.21) 中的增益 $A_v$ 交替表示为
$$
A_v = -\frac{V_{CC} -V_{CE}} {V_T}\tag{7.22}
$$
从中我们看到，**当 $V_{CE}$ 处于其最小值约 0.3 V 时，实现了最大增益，**
$$
|A_{vmax} |  = \frac{V_{CC} -0.3} {V_T}\tag{7.23}
$$
同样，这只是一个**理论上的最大值**，因为在**<u>饱和边缘偏置 BJT 不会为输出端的负信号摆幅留下空间。</u>** 然而，**等式 (7.23) 提供了基本 BJT 放大器可实现的电压增益的上限**，例如，对于 $V_{CC} = 5 V$，最大增益为 188 V/V，远大于 MOSFET 的情况。 对于现代低压技术，1.3 V 的 $V_{CC}$ 提供 40 V/V 的增益，同样比 MOSFET 的情况大得多。 然而，读者不应轻易得出结论，在现代集成电路放大器的设计中，BJT 优于 MOSFET。 事实上，情况正好相反，正如我们将在第 8 章及以后看到的那样。

最后，我们从等式 (7.22) 得出结论，**==如果要最大化 $|A_v |$ ，晶体管应偏置在与输出端负信号摆幅所需值一致的尽可能低的 $V_{CE}$。==**

#### Example 7.2

考虑使用 $I_S = 10^{−15} A$、集电极电阻 $R_C = 6.8 k$ 和电源 $V_{CC} = 10 V$ 的 BJT 的放大器电路。

- (a) 确定在 $V_{CE} =3.2 V$ 时晶体管工作所需的偏置电压 $V_{BE}$ 的值。$I_C$ 的对应值是多少？
- (b) 求该偏置点的电压增益 $A_v$。 如果峰值幅度为 5mV 的输入正弦波信号叠加在 $V_{BE}$ 上，求输出正弦波信号的幅度（假设线性操作）。
- (c) 找出驱动晶体管达到饱和边缘的 $v_{BE}$（高于 $V_{BE}$）的正增量，其中 $v_{CE} = 0.3 V$。
- (d) 找出 $v_{BE}$ 中的负增量，该增量将晶体管驱动到截止的 1% 以内（即，到 $v_{CE} = 0.99V_{CC}$）。

**Solution**

(a)
$$
\begin{aligned}I_C&= \frac{V_{CC} -V_{CE}} {R_C} \\&= \frac{10 -3.2} {6.8} = 1mA\end{aligned}
$$
$V_{BE}$ 的值可由下式确定
$$
1×10^{−3} = 10^{−15} e^{V_{BE} /V_T}
$$
这得到
$$
V_{BE} = 690.8 毫伏
...
(b)
$$
\begin{aligned}A_v &= -\frac{V_{CC} -V_{CE}} {V_T}\\& = \frac{10-3.2} {0.025} = -272 V / V \\&\hat v_{ce} = 272 × 0.005 = 1.36 V\end{aligned}
$$
(c)
...
$$
要将 $i_C$ 从 1 mA 增加到 1.617 mA，$v_{BE}$ 必须增加
$$
\begin{aligned}\Delta v_{BE}& = V_T \ln \left(\frac{1.617} 1\right) \\&= 12 mV\end{aligned}
$$
(d) 

对于 $v_{CE} = 0.99V_{CC} = 9.9 V$，
$$
i_C = \frac{10−9.9} {6.8} = 0.0147 mA
$$
要将 $i_C$ 从 1 mA 降低到 0.0147 mA，$v_{BE}$ 必须改变
$$
\begin{aligned}\Delta v_{BE} &= V_T \ln \left(\frac{0.0147} 1\right) \\&=−105.5 mV\end{aligned}
$$

#### 7.1.6 Determining the VTC by Graphical Analysis  

图 7.7 显示了确定图 7.4(a) 的放大器的 VTC 的图形方法，虽然晶体管电路的图形分析在实践中很少使用，但在这个阶段对我们更深入地了解电路操作很有用，特别是在回答偏置点 $Q$ 的位置问题时。

   <img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220315134654732.png" alt="image-20220315134654732" style="zoom:67%;" />  图形分析基于以下观察：**对于 $v_{GS}$ 的每个值（该值取决于其直流偏置电路，在电路布置好时已经确定），电路将在对应于 $v_{GS}$ 的特定值的 $i_D -v_{DS}$ 曲线与表示方程式 (7.3) 的直线的交点处运行**，该方程可改写为 $$ i_D = \frac{V_{DD}} {R_D} − \frac1 {R_D} v_{DS}\tag{7.24} $$ ，表示这种关系的直线叠加在图 7.7 中的 $i_D -v_{DS}$ 特性曲线上，它在 $v_{DS} = V_{DD}$ 处与水平轴相交，斜率为 $-1/R_D$。 **==由于这条直线实际上代表负载电阻 $R_D$，因此称为负载线==**。 然后逐点确定 VTC。 请注意，我们标记了四个重要点：**<u>A 点，$v_{GS} = V_t$，Q 点，MOSFET 可以偏置以用于放大器操作（$v_{GS} = V_{GS}$ 和 $v_{DS} = V_{DS}$），B 点 MOSFET 离开饱和并进入 三极管区域，以及 C 点，它位于三极管区域的深处，$v_{GS} = V_{DD}$。</u>** 如果将 MOSFET 用作开关，则工作点 A 和 C 适用：在 A 处，晶体管关闭（开路），**在 C 处，晶体管作为低阻值电阻 $r_{DS}$ 工作，并且电压降很小 （闭路）**。  C 点的增量电阻也称为闭合电阻。  MOSFET 作为开关的操作如图 7.8 所示。 第 14 章详细研究了 MOSFET 作为开关的应用，涉及 CMOS 数字逻辑电路。
   <img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220315135342807.png" alt="image-20220315135342807" style="zoom:67%;" />  

图 7.8 图 7.4(a) 中 MOSFET 作为开关的操作： (a) 开路，对应图 7.7 中的 A 点；  (b) 闭路，对应于图 7.7 中的 C 点，闭合电阻大约等于 $r_{DS}$，因为 $V_{DS}$ 通常非常小  

上述图形分析方法**可用于确定图 7.2(c) 中 BJT 放大器的 VTC**。 这里，图 **<u>==7.2(d) 中的 A 点对应于 BJT 刚刚开启 ($v_{BE} \simeq0.5 V$)，B 点对应于 BJT 离开有源区并进入饱和区==</u>**。 如果将 BJT 作为开关操作，则两种操作模式是截止（开路）和饱和（闭路）。 如第 6.2 节所述，**在饱和状态下，BJT 具有较小的闭合电阻 $R_{CE_{sat}}$ 以及偏移电压**。更严重的是，将 BJT 切换出其饱和区可能需要相对较长的延迟时间，以确保移除存储在 BJT 基区中的电荷。 相对于 MOSFET，这种现象使得 BJT 在数字逻辑应用中的吸引力大大降低  

#### 7.1.7 Deciding on a Location for the Bias Point Q  

对于 MOSFET 放大器，偏置点 $Q$ 由 $V_{GS}$ 的值和负载电阻 $R_D$ 的值决定，决定 $Q$ 位置的两个重要考虑因素是所需的增益和输出端所需的信号摆幅。 为了说明，考虑图 7.4(b) 所示的 VTC。 这里 $R_D$ 的值是固定的，剩下的唯一变量是 $V_{GS}$ 的值。 **由于随着我们靠近 B 点斜率增加，我们通过将 $Q$ 定位在尽可能靠近 B 的位置来获得更高的增益。** **<u>然而，$Q$ 越靠近边界点 B，负信号摆幅的允许幅度越小。</u>** 因此，正如工程设计中经常发生的那样，我们遇到需要权衡的情况。 这里的答案相对简单：对于给定的 $R_D$，将 $Q$ 定位在**尽可能靠近三极管区域（B 点）的位置以获得高增益，但<u>距离足够远以允许所需的负信号摆幅</u>。**  <img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220315140243898.png" alt="image-20220315140243898" style="zoom: 67%;" />  

在决定 $R_D$ 的值时，参考 $i_D -v_{DS}$ 平面很有用。 图 7.9 显示了导致两个极端偏置点的两条负载线： **$Q_1$ 点太靠近 $V_{DD}$，导致对 $v_{ds}$ 的正信号摆幅的严重限制，超过允许的正最大值会导致信号的正峰值被截断，**因为 MOSFET 将在每个周期接近正峰值的部分关闭。 我们通过说电路没有足够的“净空”来谈论这种情况。 同样，$Q_2$ 点距离三极管区域的边界太近，从而严重限制了$v_{ds}$ 的允许负信号摆幅。 超过此限制将导致晶体管在负峰值附近的每个周期的一部分进入三极管区域，从而导致输出信号失真。 在这种情况下，我们说电路没有足够的“腿部空间”。 我们将在第 7.4 节中对偏置设计进行更多说明。

最后，我们注意到完全相同的考虑适用于 BJT 放大器的情况
...
等式右侧的第一项。可以识别为直流偏置电流 $I_D$（方程 7.25），**第二项表示与输入信号 $v_{gs}$ 成正比的电流分量（注意，他是线性的）**，第三项是与输入信号的**平方成正比**的电流分量。 最后一个分量是不受欢迎的，**==因为它代表非线性失真==**。 为了减少 MOSFET 引入的非线性失真**（使线性量远远大于非线性量）**，输入信号应保持较小，以便
...
如果满足这个小信号条件，我们可以**忽略方程中的最后一项**，并将 $i_D$ 表示为
...
与 $i_d$ 和 $v_{gs}$ 相关的参数是 MOSFET 跨导 $g_m$ （**即其系数**），
...
由前面的分析可知，**在小信号近似下，信号量叠加在直流量上**。 例如，总漏极电流 $i_D$ 等于直流电流 $I_D$ 加上信号电流 $i_d$ ，总漏极电压 $v_{DS} = V_{DS} +v_{ds}$，依此类推。 **==因此，通过将直流或偏置计算与小信号计算分开，可以大大简化分析和设计==**。

也就是说，**<u>==一旦建立了稳定的直流工作点并计算了所有直流量，我们就可以执行信号分析而忽略直流量。==</u>**
...
我们现在将仔细研究由方程 (7.32) 给出的 MOSFET 跨导。  ，我们用 $k_n = k _n'(W/L)$ 重写如下
...
图 7.16(a) 显示了上面研究的没有 $r_o$ 的等效电路。 在图 7.16(b) 中，我们添加了与原始受控源串联的第二个 $g_mv_{gs}$ 电流源，这种增加显然不会改变终端电流，因此是允许的。 新创建的电路节点，标记为 $X$，连接到图 7.16(c) 中的栅极端子 $G$， **观察到栅极电流没有改变（根据基尔霍夫电流定律）——也就是说，它保持为零——因此这种连接不会改变端子特性，**我们现在注意到我们**有一个==受控电流源== $g_mv_{gs}$ 连接在其控制电压 $v_{gs}$ 上**。

我们可以**用电阻代替这个受控源，只要该==电阻吸收与源相同的电流==**（参见附录 D 中的源吸收定理。）我们不难从图中得到该节点两侧的电势差和电流大小，因此
...
在开发 T 模型时，我们没有包括 $r_o$，如果需要，这可以通过**在图 7.16(d) 的电路中加入漏极和源极之间的电阻 $r_o$ 来完成**，如图 7.17(a) 所示。  T 模型的另一种表示，其中电压控制电流源被电流控制电流源代替，如图 7.17（b）所示。
...
最后，我们应该注意，为了将图 7.13(b) 的模型与等效的 T 模型区分开来，前者有时被称为 $hybrid -π$ 模型，这是双极晶体管文献的结转。 稍后将解释这个名称的由来。

#### 7.2.2 The BJT Case

我们接下来考虑 BJT 的小信号操作，并开发小信号等效电路模型来表示其在给定偏置点的操作。 以下开发与我们用于 MOSFET 的开发类似，只是这里有一个额外的复杂性：BJT 消耗有限的基极电流。 正如将很快看到的那样，**这种现象（有限 $β$）本身表现为一个有限的输入电阻，（从 BJT 的基极看入（与无限的输入电阻看着 MOSFET 的栅极相比））**。

考虑图 7.20(a) 所示的概念放大器电路。 这里基极-发射极结由直流电压 $V_{BE}$ 正向偏置。 集电极 - 基极结的反向偏置是通过将集电极通过电阻器 $R_C$ 连接到另一个电压为 $V_{CC}$ 的电源来建立的，待放大的输入信号由叠加在 $V_{BE}$ 上的电压源 $v_{be}$ 表示。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220316184548443.png" alt="image-20220316184548443" style="zoom:67%;" />

**The DC Bias Point**

我们首先通过将信号 $v_{be}$ 设置为零来考虑直流偏置条件，电路简化为图 7.20(b) 中的电路，我们可以写出直流电流和电压的以下关系：
$$
\begin{aligned}&I_C = I_Se^{V_{BE} /V_T} \\& I_E = I_C/α\\&I_B = I_C/β\\&  V_{CE} = V_{CC} -I_CR_C\end{aligned}
$$
对于 Active Mode，$V_{CE}$ 应大于 $(V_{BE} -0.4)$ ，以允许集电极处所需的负信号摆幅。

**The Collector Current and the Transconductance**

如果施加如图 7.20(a) 所示的信号 $v_{be}$，则总瞬时基极-发射极电压 $v_{BE}$ 变为
$$
v_{BE} = V_{BE} + v_{be}
$$
相应地，集电极电流变为
$$
\begin{aligned}i_C &= I_Se^{v_{BE} / V_T} = I_Se ^{(V_{BE} + v_{be}) / V_T} \\&= I_Se^{V_{BE} / V_T} e^{v_{be} / V_T}\end{aligned}
$$
使用方程式 (7.52) 得到
$$
i_C = I_Ce^{v_{be} /V_T}\tag{7.56}
$$
现在，如果 $v_{be}\ll V_T$ ，我们可以近似等式 (7.56) 为
$$
i_C\simeq I_C \left(1+ \frac{v_{be}} {V_T}\right)\tag{7.57}
$$
在这里，我们利用了泰勒展开进行线性近似，但前提是
$$
v_{be}\ll V_T\tag{7.58}
$$
这样我们就可以忽略指数级数展开中的高阶项，方程式中的条件 (7.58) 是 BJT 的小信号近似值，对应于等式 (7.29) 用于 MOSFET 的情况。  BJT 的小信号近似仅对 $v_{be}$ 小于 5 mV 或最多 10 mV 有效。 在这种近似下，总集电极电流由方程式 (7.57) 给出，并且可以重写为
$$
i_C = I_C + \frac{I_C} {V_T} v_{be}\tag{7.59}
$$
因此集电极电流由直流偏置值 $I_C$ 和信号分量 $i_c$ 组成，
$$
i_c = \frac{I_C} {V_T} v_{be}\tag{7.60}
$$
该方程将集电极中的信号电流与相应的基极-发射极信号电压联系起来，它可以重写为
$$
i_c = g_mv_{be}\tag{7.61}
$$
其中 $g_m$ 是跨导，从方程式 (7.60) 可以得知，它的值由下式给出
$$
g_m = \frac{I_C} {V_T}\tag{7.62}
$$
我们观察到 **BJT 的跨导与集电极偏置电流 $I_C$ 成正比**。 因此，**要获得 $g_m$ 的恒定可预测值，我们需要一个恒定的可预测 $I_C$，**此外，我们注意到与 MOSFET 相比，BJT 具有相对较高的跨导：例如，在 $I_C = 1 mA$ 时，$g_m$ 为 40 mA/V。 最后，与 $g_m$ 取决于器件尺寸（$W$ 和 $L$）的 MOSFET 不同，**BJT 的 $g_m$ 仅取决于偏置工作时的直流集电极电流。**

图 7.21 给出了 $g_m$ 的图形解释，其中显示 **$g_m$ 等于 $i_C = I_C$ 处（即偏置点 $Q$）处 $i_C-v_{BE}$ 特性曲线的切线斜率**，因此，
$$
g_m = \frac{∂i_C} {∂v_{BE}}\bigg| _{i_C =I_C}\tag{7.63}
$$
小信号近似意味着保持信号幅度足够小，以至于操作仅限于 $i_C-v_{BE}$ 指数曲线的几乎线性段。 增加信号幅度将导致集电极电流具有与 $v_{be}$ 非线性相关的分量。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220316191027344.png" alt="image-20220316191027344" style="zoom:67%;" />

图 7.21 小信号条件下晶体管的线性工作：一个三角波的小信号 $v_{be}$ 叠加在直流电压 $V_{BE}$ 上。 它会产生一个集电极信号电流 $i_c$，也是三角波形，叠加在直流电流 $I_C$ 上，这里，$i_c = g_mv_{be}$，其中 $g_m$ 是 $i_C-v_{BE}$ 曲线在偏置点 $Q$ 处的斜率。

**The Base Current and the Input Resistance at the Base**

为了确定从 $v_{be}$ 看到的电阻，我们首先使用方程式 (7.59) 计算总基极电流 $i_B$ ，如下：
$$
i_B = \frac{i_C} β = \frac{I_C} β + \frac1 β \frac{I_C} {V_T} v_{be}
$$
因此
$$
i_B = I_B + i_b\tag{7.64}
$$
其中 $I_B$ 等于 $I_C/β$ 并且信号分量 $i_b$ 由下式给出
$$
i_b = \frac1 β \frac{I_C} {V_T} v_{be}\tag{7.65}
$$
用 $g_m$ 代替 $I_C/V_T$ 给出
$$
i_b = \frac{g_m} β v_{be}\tag{7.66}
$$
**基极和发射极之间的小信号输入电阻，从基极看，用 $r_π$ 表示**，定义为
$$
r_π ≡ \frac{v_{be} }{i_b}\tag{7.67}
$$
使用方程式 (7.66) 给出
$$
r_π = \fracβ {g_m}\tag{7.68}
$$
因此 $r_π$ 直接取决于 $β$ 并且与偏置电流 $I_C$ 成反比，代入方程式 (7.68) 中的 $g_m$ 并将 $I_C/β$ 替换为 $I_B$ 给出了 $r_π$ 的替代表达式，
$$
r_π = \frac{V_T} {I_B}\tag{7.69}
$$
在这里，我们回想一下，因为 MOSFET 的栅极电流为零（在直流和低频下），所以栅极的输入电阻是无穷大的； 也就是说，在 MOSFET 中没有 $r_π$ 的对应物

**The Emitter Current and the Input Resistance at the Emitter**

总发射极电流 $i_E$ 可以使用方程式 (7.59) 确定为
$$
i_E = \frac{i_C} α = \frac{I_C} α + \frac{i_c} α
$$
因此
$$
i_E = I_E + i_e\tag{7.70}
$$
其中 $I_E$ 等于 $I_C/α$ 并且信号电流 $i_e$ 由下式给出
$$
i_e = \frac{i_c} α = \frac{I_C} {αV_T} v_{be} = \frac{I_E} {V_T} v_{be}\tag{7.71}
$$
如果我们**用 $r_e$ 表示基极和发射极之间的小信号电阻**，它可以定义为
$$
r_e ≡ \frac{v_{be}} {i_e}\tag{7.72}
$$
使用方程式 (7.71) 我们发现称为发射极电阻的 $r_e$ 由下式给出
$$
r_e = \frac{V_T} {I_E}\tag{7.73}
$$
与等式 (7.62) 比较，可以发现
$$
r_e = \fracα {g_m} \simeq \frac1 {g_m}\tag{7.74}
$$
$r_π$ 和 $r_e$ 之间的关系可以通过结合方程式  (7.67) 和 (7.72)  中各自的定义来找到，为
$$
v_{be} = i_br_π = i_er_e
$$
因此
...
r_π = (β +1) r_e\tag{7.75}
$$
图 7.22 说明了 $r_π$ 和 $r_e$ 的定义。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220316203535532.png" alt="image-20220316203535532" style="zoom:67%;" />

最后，与 MOSFET 进行比较会很有用：对于 MOSFET，$α = 1$ 并且向源极观察的电阻仅为 $1/g_m$。

**SHOCKLEY AND SILICON VALLEY:**

1956 年，威廉·布拉德福德·肖克利 (William Bradford Shockley) 在加利福尼亚州山景城（他的出生地斯坦福附近）创办了一家新公司，肖克利半导体实验室。 在贝尔实验室时，他与 John Bardeen 和 Walter Brattain 一起发明了 BJT。 在 Shockley，最初的重点是开发半导体器件，尤其是新的四层二极管。 但是，肖克利的科学天才以及挑选和吸引优秀团队成员的能力首先在贝尔实验室得到证明，但并没有与之相媲美的管理人才。 因此，1957 年，他的八名团队成员（所谓的叛徒八人，包括戈登·摩尔和罗伯特·诺伊斯）离开了肖克利，创立了仙童半导体。 这是一个好时机：第一颗人造卫星在一个月后发射，随后的太空竞赛加速了对固态电路的需求。 几十年过去了，2002 年，一群自 1956 年以来与硅谷有联系的大约 30 人在斯坦福大学会面，回忆肖克利对信息技术时代的贡献，他们一致认为肖克利是把硅带到硅谷的人！

**The Voltage Gain**

总集电极电压 $v_{CE}$ 为
$$
\begin{aligned}v_{CE} &= V_{CC} -i_CR_C \\&= V_{CC} -(I_C +i_c)R_C  \\&=(V_{CC} -I_CR_C)-i_cR_C \\&= V_{CE} -i_cR_C\end{aligned}\tag{7.76}
$$
因此，**叠加在集电极偏置电压 $V_{CE}$ 上，我们得到信号电压 $v_{ce}$**，由下式给出
$$
\begin{aligned}v_{ce} &=−i_cR_C =−g_mv_{be}R_C \\& = (−g_mR_C)v_{be}\end{aligned}\tag{7.77}
$$
从中我们发现该放大器的**电压增益 $A_v$** 为
$$
A_v≡ \frac{v_{ce}} {v_{be}}  = -g_mR_C\tag{7.78}
$$
这里我们再次注意到，因为 $g_m$ 与集电极偏置电流成正比，所以增益将与集电极偏置电流一样稳定。 用方程式 (7.62) 代替 $g_m$，使我们能够将增益表示为
$$
A_v = -\frac{I_CR_C} {V_T}\tag{7.79}
$$
这与我们在第 7.1 节（方程 7.21）中推导出的表达式相同。 最后，我们注意到方程式 (7.78) 中的增益表达式的形式与 MOSFET 放大器的形式相同（即 $-g_mR_D$）。

**Separating the Signal and the DC Quantities**

上述分析表明，图 7.20(a) 的放大器电路中的**每个电流和电压都由两个分量组成：直流分量和信号分量**。 例如，$v_{BE} = V_{BE} +v_{be}，I_C = I_C +i_c$，等等。 直流分量由图 7.20(b) 中给出的直流电路和晶体管施加的关系确定（方程 7.52 到 7.54）。 另一方面，**BJT 的信号操作表示可以通过消除直流源获得**，如图 7.23 所示。 请注意，**由于理想直流电源的电压不变，其两端的信号电压将为零。** 出于这个原因，**我们用短路代替了 $V_{CC}$ 和 $V_{BE}$**。 如果电路**包含==理想的直流电流源==，这些将被==开路==取代**。 但是请注意，图 7.23 的电路仅在它显示各种信号电流和电压时才有用。 它不是一个实际的放大器电路，因为没有显示直流偏置电路。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220316205535107.png" alt="image-20220316205535107" style="zoom:67%;" />

图 7.23 图 7.20(a) 的放大器电路，去掉了直流电源（$V_{BE}$ 和 $V_{CC}$）（短路）。 因此**仅存在信号分量**。 请注意，**这是 BJT 信号操作的表示，而不是实际的放大器电路。**

图 7.23 还显示了施加小信号 $v_{be}$ 时获得的电流增量（$i_c$、$i_b$ 和 $i_e$）的表达式。 这些关系可以用一个电路来表示。 这样的电路应该有三个端子——C、B 和 E——并且应该产生与图 7.23 所示相同的端子电流。 就小信号操作而言，所得电路等效于晶体管，因此可以将其视为等效小信号电路模型。

**The Hybrid-$\pi$ Model**

BJT 的等效电路模型如图 7.24(a) 所示。**该模型将 BJT 表示为==电压控制的电流源==**，并明确包括输入到基极的输入电阻 $r_π$。 该模型显然产生 $i_c =g_mv_{be}$ 和 $i_b =v_{be}/r_π$ ，然而，不那么明显的是，该模型也得到了 $i_e$ 的正确表达式，如下面的步骤所示：在射级节点，我们有
$$
\begin{aligned}i_e& = \frac{v_{be}} {r_π} +g_mv_{be} = \frac{v_{be}} {r_π} \left(1+g_mr_π\right) \\ &= \frac{v_{be}} {r_π} (1+β) = v_{be} \bigg/ \left(\frac{r_π} {1+β} \right) \\&= v_{be}/r_e\end{aligned}
$$
通过用基极电流 $i_b$ 表示受控源的电流 ($g_mv_{be}$)，可以获得稍微不同的等效电路模型，如下所示：
$$
\begin{aligned}g_mv_{be} &= g_m \left(i_br_π\right)  \\&= \left(g_mr_π\right)  i_b = βi_b\end{aligned}
$$
这导致了如图 7.24(b) 所示的替代等效电路模型，这里晶体管表示为电流控制的电流源，控制电流为 $i_b$。

正如我们在 MOSFET 的小信号模型中所做的那样，我们可以通过**在集电极和发射极之间添加电阻 $r_o = V_A/I_C$ 来解释厄利效应**（由于基极宽度调制，$i_C$ 对 $v_{CE}$ 的轻微依赖性），如图 7.25 所示。 请注意，为了符合文献，我们将 **$v_{be}$ 重命名为 $v_π$** ，图 7.25 的两个模型是混合 π 模型的版本，是 BJT 中使用最广泛的模型。 图 7.25(a) 的等效电路对应于 MOSFET [图7.13(b)] 除了 $r_π$ ，它解释了 BJT 的有限基极电流（或有限 β）然而，图 7.25(b) 的等效电路没有 MOS 对应物

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220316214155366.png" alt="image-20220316214155366" style="zoom:67%;" />

图 7.24 用于 BJT 小信号操作的混合 $π$ 模型的两个略有不同的版本，(a) 中的等效电路将 BJT 表示为**压控电流源**（跨导放大器），（b）中的等效电路将 BJT 表示为**电流控制电流源**（电流放大器）。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220316220816628.png" alt="image-20220316220816628" style="zoom:67%;" />

值得注意的是，图 7.25 的小信号等效电路模拟了 BJT 在给定偏置点的操作。 从模型参数 $g_m$、$r_π$ 和 $r_o$ 取决于直流偏置电流 $I_C$ 的值这一事实可以看出这一点，如图 7.25 所示。 也就是说，这些等效电路模拟了 BJT 在偏置点附近的增量操作。

与 MOSFET 放大器的情况一样，在 BJT 模型中包含 $r_o$ 会导致图 7.20(a) 的概念放大器的电压增益变为
$$
\frac{v_o} {v_{be}} = -g_m (R_C|| r_o)\tag{7.80}
$$
因此，增益的幅度有所减小。

**The T Model**

尽管混合 $π$ 模型（在图 7.24 所示的两个变体之一）可用于对任何晶体管电路进行小信号分析，但在某些情况下，如图 7.26 所示的替代模型是 方便多了。 这个模型，与 MOSFET 一样，称为 T 模型，在图 7.26 中有两个版本。

图 7.26(a) 的模型将 BJT 表示为**==电压控制电流源==**，控制电压为 $v_{be}$。 然而，这里**明确显示了==基极和发射极之间==的电阻**，观察发射极，从图 7.26(a) 我们清楚地看到，该模型同样可以得到 $i_c$ 和 $i_e$ 的正确表达式，也可以证明它得到了 $i_b$ 的正确表达式（参见下一页的练习 7.18）。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220317152502442.png" alt="image-20220317152502442" style="zoom:67%;" />

图 7.26 BJT 的 T 模型的两个略有不同的版本。  (a) 中的电路是**电压控制的电流源**表示，(b) 中的电路是**电流控制的电流源**表示，这些模型明确显示了发射极电阻 $r_e$ 而不是混合 $π$ 模型中的基极电阻 $r_π$。

如果在图 7.26(a) 的模型中，受控源的电流用发射极电流表示为

$$
\begin{aligned}g_mv_{be}& = g_m(i_er_e)\\& = (g_mr_e)i_e = αi_e\end{aligned}
$$
我们得到了如图 7.26(b) 所示的替代 T 模型。 这里 BJT 表示为电流控制的电流源，但控制信号为 $i_e$。

最后，T 模型可以通过 $r_o$ 进行扩充，以说明 $i_c$ 对 $v_{ce}$ 的依赖性（厄利效应），从而获得图 7.27 所示的等效电路。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220317153147717.png" alt="image-20220317153147717" style="zoom:67%;" />

**Small-Signal Models of the pnp Transistor**

尽管图 7.25 和 7.27  中的小信号模型，是针对 npn 晶体管的情况开发的，它们同样适用于没有极性变化的 pnp 晶体管。

#### Example 7.5

我们希望分析图 7.28(a) 所示的晶体管放大器，以确定其电压增益 $v_o/v_i$，假设 $β = 100$ 并忽略厄利效应

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220317153547028.png" alt="image-20220317153547028" style="zoom:67%;" />



<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220317155151622.png" alt="image-20220317155151622" style="zoom:67%;" />

**Solution**

我们将遵循五个步骤：

1. **分析的第一步是确定静态工作点**，为此，我们**假设 $v_i$ =0**，从而获得图 7.28(b) 所示的直流电路。 直流基极电流为

$$
\begin{aligned}I_B &= \frac{V_{BB} -V_{BE}} {R_{BB}}\\&\simeq \frac{3-0.7} {100} = 0.023 mA\end{aligned}
$$

直流集电极电流为
$$
I_C = βI_B = 100×0.023 = 2.3 mA
$$
集电极的直流电压为
$$
\begin{aligned}V_C &= V_{CC}−I_CR_C \\&=+10−2.3×3=+3.1V\end{aligned}
$$
由于 +0.7 V 时的 $V_B$ 小于 $V_C$，因此在静态条件下，晶体管将在活动模式下工作，直流分析如图 7.28(b) 所示。

2. **<u>确定了工作点后，我们现在可以继续确定小信号模型参数</u>**

$$
\begin{aligned}&r_e = \frac{V_T} {I_E} = \frac{25 mV} {(2.3/0.99) mA} = 10.8\Omega\\& g_m = \frac{I_C} {V_T} = \frac{2.3 mA} {25 mV }= 92 mA/V\\& r_π = \frac{β} {g_m} = \frac{100} {92} = 1.09 k\Omega\end{aligned}
$$

3. 用短路代替 $V_{BB}$ 和 $V_{CC}$ 得到图7.28(c) 中的电路。
4. 为了进行小信号分析，使用图 7.24 的两个混合 $π$ 等效电路模型中的任何一个来代替图 7.28(c) 电路中的晶体管同样方便，在图 7.28(d) 中给出的放大器等效电路中使用了第一种形式。
5. 图 7.28(d)中的等效电路分析如下：**（上图采用的是电压控制电流源模型，先用串联分压电路计算出基极-发射极电压以确定集电极电流）**

$$
\begin{aligned}v_{be}& = v_i \frac{r_π} {r_π + R_{BB}}\\& = v_i \frac{1.09} {101.09} = 0.011v_i\end{aligned}\tag{7.81}
$$

输出电压 $v_o$ 由下式给出

$$
\begin{aligned}v_o &= -g_mv_{be}R_C \\&= -92 × 0.011v_i × 3 = -3.04v_i\end{aligned}
$$
因此电压增益将为

$$
A_v = \frac{v_o} {v_i} = -3.04 V / V\tag{7.82}
$$

#### Example 7.6

为了更深入地了解晶体管放大器的工作状态，我们希望考虑前面示例中分析的电路中各个点的波形。 为此，假设 $v_i$ 具有三角波形，首先确定 $v_i$ 允许具有的最大幅度。 然后，将 $v_i$ 的幅度设置为该值，给出总量 $i_B(t)、v_{BE}(t)、i_C(t)$ 和 $v_C(t)$ 的波形。

**Solution**

对信号幅度的一个约束是小信号近似，它规定 $v_{be}$ 不应超过约 10 mV。 如果我们将三角波形 $v_{be}$ 取为 20 mV **峰峰值**并向回代，则方程式 (7.81) 可用于确定 $v_i$ 的最大可能峰值，
$$
\hat{v_i} = \frac{\hat{v_{be}}} {0.011} = \frac{10} {0.011} = 0.91 V
$$
为了检查晶体管是否保持在激活模式，$v_i$ 的峰值为 $\hat{v_i} = 0.91V$，我们计算评估集电极电压，**集电极上的电压将由叠加在直流值 $V_C = 3.1 V$ 上的三角波 $v_o$ 组成**。三角波的峰值电压为
$$
\hat{v_o} = \hat{v_i} × 增益 = 0.91 × 3.04 = 2.77 V
$$
因此，当输出摆动为负时，集电极电压达到最小值 3.1−2.77 = 0.33 V，比基极电压低不到 0.4 V。因此晶体管将保持在活动模式，$v_i$ 有一个峰值 0.91 V。不过，为了安全起见，我们将使用稍低的约 0.8 V 的 $\hat{v_i}$ 值，如图 7.29(a) 所示，并利用等效电路完成对这个问题的分析，如图 7.28(d)，基极中的信号电流将是三角形的，峰值 $\hat{i_b}$ 为
$$
\hat{i_b} = \frac{\hat{v_i}}{R_{BB} + r_π} = \frac{0.8 }{100 + 1.09 }= 0.008 mA
$$
该三角波电流将叠加在静态基极电流 $I_B$ 上，如图 7.29(b) 所示，基极-发射极电压由叠加在直流 $V_{BE}$ 上的三角波分量组成，约为 0.7 V，三角波的峰值为
$$
\hat{v_{be}} = \hat{v_i} \frac{r_π} {r_π +R_{BB}} = 0.8 \frac{1.09} {100+1.09} = 8.6 mV
$$
总的 $v_{BE}$ 如图 7.29(c) 所示。

集电极中的信号电流将呈三角形波形，峰值 $i_c$ 由下式给出
$$
\hat{i_c} = β\hat{i_b} = 100 × 0.008 = 0.8 mA
$$
该电流将**叠加在静态集电极电流 $I_C$ (=2.3 mA) 上**，如图 7.29(d) 所示。

将 $v_i$ 乘以电压增益即可得到集电极的信号电压； 那是，
$$
\hat{v_o} = 3.04 × 0.8 = 2.43 V
$$
图 7.29(e) 显示了总集电极电压 $v_C$ 与时间的关系图，注意**输入信号 $v_i$ 和输出信号 $v_o$ 之间的相位反转**。

最后，我们观察到每个总量**都是直流量（从图 7.28b 中的直流电路中找到）和信号量（从图 7.28d 中的电路中找到）的总和**。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220317163425156.png" alt="image-20220317163425156" style="zoom: 67%;" />

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220317163529444.png" alt="image-20220317163529444" style="zoom:67%;" />

#### Example 7.7

我们需要分析图 7.30(a) 的电路以确定电压增益和各个点的信号波形。 电容器 $C_{C1}$是一个耦合电容器，其目的是将信号 $v_i$ 耦合到发射极，同时阻断直流，这样，**当信号 $v_i$ 连接时，由 $V^+$ 和 $V^-$ 以及 $R_E$ 和 $R_C$ 建立的直流偏置将不会受到干扰**，出于本示例的目的，将假设 $C_{C1}$ 非常大，以便在感兴趣的信号频率处充当完美的短路，类似地，另一个非常大的电容器 $C_{C2}$ 用于将输出信号 $v_o$ 耦合到系统的其他部分，忽略厄利效应。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220317163840770.png" alt="image-20220317163840770" style="zoom:67%;" />

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220317163909292.png" alt="image-20220317163909292" style="zoom:67%;" />

在这里，我们将再次遵循五个步骤：

1. 图 7.30(b) 显示了**去掉了信号源和耦合电容**的电路，直流工作点可以确定如下：

$$
I_E = \frac{+10−V_E} {R_E}\simeq \frac{+10−0.7} {10} = 0.93 mA
$$

假设 $β=100$，则 $α=0.99$，并且
$$
\begin{aligned}&I_C = 0.99I_E = 0.92 mA\\& V_C =−10+I_CR_C \\&\ \  \ \ \ =−10+0.92×5=−5.4 V\end{aligned}
$$
因此晶体管处于活动模式。
...
$$
\begin{aligned}&g_m = \frac{I_C} {V_T} = \frac{0.92} {0.025} = 36.8 mA/V\\ &r_e = \frac{V_T} {I_E} = \frac{0.025} {0.92} = 27.2 \Omega\\& β = 100 \\&α = 0.99 \\&r_π = \frac{β} {g_m} = \frac{100} {36.8} = 2.72 k\Omega\end{aligned}
$$

3. 为了准备小信号分析电路，我们用短路代替直流源。 得到的电路如图 7.30(c) 所示。 请注意，我们还消除了两个耦合电容器，因为它们被假定为完美的短路。
4. 我们现在准备用图 7.24 和 7.26 中的四个等效电路模型之一来替换 BJT，**虽然这四个中的任何一个都可以工作，但图 7.26 的 T 模型会更方便，因为电路是接地的，** 选择图 7.26(b) 中的版本会得到图 7.30(d) 所示的放大器等效电路。
5. 分析图 7.30(d) 中的电路以确定输出电压 $v_o$，因此电压增益 $v_o/v_i$ 很简单，如图所示。 结果是

$$
A_v = \frac{v_o} {v_i} = \frac{αR_C} {r_e} = \frac{0.99× 5} {0.0272} = 182 V / V
$$

请注意，**电压增益为正，表示输出与输入信号同相**，**该属性是由于输入==<u>信号被施加到发射器而不是基极</u>==**，如例 7.5 中所做的那样，我们应该强调的是，正增益与这个例子中使用的晶体管是 pnp 类型的事实无关。

回到允许信号幅度的问题，我们从图 7.30(d) 观察到 $v_{eb} =v_i$，因此，如果需要小信号操作（线性），则 $v_i$ 的峰值应限制在大约 10 mV。 将 $\hat{V_i}$ 设置为该值，如图 7.31 中的正弦波输入所示，集电极处的峰值幅度 $\hat{V_o}$ 将为 
$$
\hat{V_ o} = 182 × 0.01 = 1.82 V
$$
<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220317165427583.png" alt="image-20220317165427583" style="zoom:67%;" />

图 7.31 图 7.30 电路的输入和输出波形。 观察该放大器是同相的，这是接地基配置的一个属性

**Performing Small-Signal Analysis Directly on the Circuit Diagram**

在大多数情况下，我们应该用其小信号模型明确替换每个 BJT 并分析生成的电路，就像我们在上面的示例中所做的那样。 这种系统的程序特别推荐给初学者；然而，**经验丰富的电路设计人员通常直接对电路进行一阶分析**。 图 7.32 说明了我们在示例 7.5 和 7.7 中分析的两个电路的这个过程。 敦促读者遵循此直接分析程序（步骤已编号），**观察到隐含地使用了等效电路模型； 我们只省去了绘制电路的步骤，将 BJT 替换为其模型。 然而，直接分析还有一个非常重要的好处：==它可以深入了解通过电路的信号传输==，** 这种洞察力在设计中可以证明是无价的，特别是在选择适合给定应用的电路配置阶段。 直接分析也可用于 MOS 放大器电路。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220317165629018.png" alt="image-20220317165629018" style="zoom:80%;" />

图 7.32 直接在电路图上执行信号分析，隐式采用 BJT 小信号模型： (a) 示例 7.5 的电路；  (b) 例 7.7 的电路；

#### EXERCISE

7.20 

图 E7.20 中的晶体管由恒流源 $I = 1 mA$ 偏置，$β = 100$ 和 $V_A = 100 V$。

(a) 忽略厄利效应，求基极、发射极和集电极的直流电压。

(b) 求 $g_m、r_π$ 和 $r_o$。

(c) 如果端子 $Z$ 接地，$X$ 连接到源电阻 $R_{sig} = 2 k\Omega$ 的信号源 $v_{sig}$，$Y$ 连接到 $8k\Omega$ 负载电阻，则使用前面所示的混合 $π$ 模型（图 7.25  ) 画出放大器的小信号等效电路（注意电流源 $I$ 应该用开路代替。）计算总电压增益 $v_y/v_{sig}$，如果忽略 $r_o$，估计增益幅度的误差是多少？  （注意：**无限电容用于表示电容足够大，以至于在所有感兴趣的信号频率下都起到短路的作用**。但是，电容器仍会阻断直流。）

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220317191905980.png" alt="image-20220317191905980" style="zoom:67%;" />

Ans. (a) $–0.1 V, –0.8 V, +2.1 V$; (b) $40 mA/V, 2.5 k\Omega, 100 k\Omega$; (c) $–77 V/V, +3.9%$

#### 7.2.3 Summary Tables

我们通过提供三个有用的汇总表来结束本节： 表 7.1 列出了分析 MOSFET 或 BJT 放大器电路时要遵循的五个步骤。 表 7.2 给出了 MOSFET 小信号等效电路模型，以及模型参数值的计算公式。 最后，表 7.3 提供了 BJT 的相应数据。

| 表 7.1 晶体管放大器电路分析的系统程序                        |
| ------------------------------------------------------------ |
| 1. 消除信号源，确定晶体管的直流工作点。<br/>2.计算小信号模型的参数值。<br/>3. 通过用短路代替每个直流电压源和用开路代替每个直流电流源来消除直流电源。<br/>4. 用其中一种小信号等效电路模型替换晶体管。 尽管可以使用任何模型，但对于正在分析的特定电路，一个模型可能比其他模型更方便。 这一点将在下一节中更清楚地说明。<br/>5. 分析得到的电路以确定所需的量（例如，电压增益、输入电阻）。 |

| 表 7.2 MOSFET 的小信号模型                                   |
| ------------------------------------------------------------ |
| 小信号参数<br/>NMOS晶体管<br/>跨导：$g = μ_n C_{ox} \frac{W}L V_{OV} = \sqrt{2μ_n C_{OX} \frac WL I_D} = \frac{2I_D} {V_{OV}}$<br/>输出电阻：$r_o = V _A/I_D = 1/λI_D$<br/>PMOS 晶体管<br/>公式与 NMOS 相同，但使用  $|V_{OV}|,|V_A|,|\lambda|$ 以及用 $\mu_n$ 来代替 $\mu_p$<br/>小信号、等效电路模型<br/><img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220317192924341.png" alt="image-20220317192924341" style="zoom:67%;" /> |

| 表 7.3 BJT 的小信号模型                                      |
| ------------------------------------------------------------ |
| **Hybrid-π Model**<br/><img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220317193011412.png" alt="image-20220317193011412" style="zoom:67%;" /><br/>**T Model**<br/><img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220317193051464.png" alt="image-20220317193051464" style="zoom:67%;" /><br/>直流偏置电流模型参数<br/>![image-20220317193150900](C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220317193150900.png)<br/> 以 $g_m$ 表示<br/>![image-20220317194424583](C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220317194424583.png)<br/>以 $r_e$ 表示<br/>![image-20220317194528102](C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220317194528102.png)<br/>$α$ 和 $β$ 之间的关系<br/>![image-20220317194603459](C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220317194603459.png) |

### 7.3 Basic Configurations

在第 7.1 节中，我们研究了 MOSFET 和 BJT 作为放大器的应用的基本原理。 在那里，我们发现通过在晶体管工作的有效区域的适当点对晶体管施加直流偏置，并保持输入信号（$v_{gs}$ 或 $v_{be}$）较小，可以获得几乎线性的放大。然后，在第 7.2 节中，我们开发了电路模型代表两种晶体管类型（表 7.2 和 7.3）中每一种的小信号操作，从而为晶体管放大器的分析提供了系统程序（表 7.1）。
...
连接MOSFET或BJT作为放大器有三种基本配置。这些配置中的每一个都是**通过==将其中一个设备端子连接到接地==来获得的，从而==创建一个双端口网络==**，其中接地端子与输入和输出端口共用。图7.33（a–c）显示了MOSFET的配置，图7.33（d–f）显示了BJT的配置。

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210606152347187.png" alt="image-20210606152347187" style="zoom:67%;" />1
...
图 7.34(a) 显示了一个放大器，其信号源具有开路电压 $v_{sig}$ 和内阻 $R_{sig}$。 这些可以是实际信号源的参数，或者在级联放大器中，是所研究的放大器之前的另一个放大器级的输出电路的 Th´evenin 等效值。 放大器显示为负载电阻 $R_L$ 连接到输出端子。 这里，$R_L$ 可以是实际负载电阻或级联放大器中后续放大器级的输入电阻
...
而放大器的放大倍数，正如我们此前所提到的，是在其**负载开路**的情况下测得的（**开路电压增益**）
...
在三种基本晶体管放大器配置中，共源极（共发射极，用于 BJT）是最广泛使用的。 **==通常，在由多个增益级级联形成的放大器中，大部分电压增益是通过级联使用一个或多个共源极（或共发射极，对于 BJT）级获得的==**

**Characteristic Parameters of the CS Amplifie**

图7.35（a）所示为共源放大器（省略偏置布置），该放大器由具有源极电阻 $R_{sig}$ 的信号源 $v_{sig}$ 馈电。我们希望分析这个电路来确定 $R_{in}、A_{vo}$ 和 $R_o$，为此，我们假设 $R_D$ 是放大器的一部分；因此，如果负载电阻 $R_L$ 连接到放大器输出，则 $R_L$ 与 $R_D$ 以并联的形式出现。在这种情况下，我们还希望确定 $A_v$ 和 $G_v$。
...
以npn晶体管为例，由于其CBJ反偏，因此**基极到集电极的电路无法导通，换言之，三极管的内部路径仅有基极到发射极一条**，因此其输入电阻不难得到，即为从基极看向发射极的等效电阻 $r_{\pi}$，其等效电路如下图所示
...
这是一个非常重要的结果。 它指出，**==从基极看的输入电阻是发射极总电阻的 $(β +1)$ 倍==**，这被称为电阻反射规则。 **因子 $(β +1)$ 的出现是因为基极电流是发射极电流的 $1/(β +1)$ 倍**，**上式清楚地表明，在发射极中包含电阻 $R_e$ 可以显著增加 $R_{in}$**，这是一个非常理想的结果。 事实上，$R_{in}$ 的值是按比例增加的
...
总而言之，在 CE 放大器的发射极中包含一个电阻 $R_e$ 会导致以下特性
...
3.CS和CE放大器增益的高频限制，以及增益衰减和 3db 频率 $f_H$ 的上限主要由漏极和栅极（集电极和基极）之间的小电容决定。
...
在第10.1节分立放大电路的低频响应分析中，我们将注意力放在频率fL的确定上，它定义了中频的下限。通常而言，$f_L$ 被定义为增益大小比电路处在中频的值低3分贝的频率。要注意的是，集成电路放大器不使用耦合和旁路电容器，因此其中频的下限在零频率（dc）处，如图10.2所示。
...
在某些电路中，如不久前讨论的共射放大器的电路中，电容器存在相互作用，很难确定 $f_L$ 频率，然而，幸运的是，有一种简单的方法可以获得 $f_L$ 的估计值，而不需要确定极点的频率。尽管该方法是基于其中一个极点占主导地位的假设，虽然该假设的前提并非精确，但其仍然为 $f_L$ 给出了一个良好的近似。方法如下
...
除了简单之外，该方法还有一个非常重要的作用：方程式指出了每个电容器对 $f_L$ 值的相对贡献。具体来说，与特定电容器相关的时间常数值越低，该电容器对fL的贡献就越大。如下文所示，这一观察结果具有重要的设计意义。

短路时间常数法的应用将在下一节中说明，其中它用于确定 CE 放大器的 $f_L$。
...
图10.9（a）显示了使用classic偏置的分立电路共射放大器，以及耦合电容器$C_{C1}$和$C_{C2}$以及旁路电容器$C_E$，我们希望能计算出该电路的 $f_L$，并且同时，我们需要确定 $C_{C1}$、$C_E$ 和 $C_{C2}$ 的值，以确保 $f_L$ 的大小符合实际应用要求，同时最小化所需的总电容值。

为了分析CE放大器的低频增益，我们使用图10.9（b）所示的等效电路。这种等效电路是通过短接 $V_{CC}$，用T-model代替 BJT，而忽略 $r_o$ 得到的。忽略 $r_o$ 的原因在于晶体管的输出电阻对分立电路放大器的增益影响很小，并且由于它的存在会导致分析相当复杂。从图10.9（b）中的电路中，我们观察到BJT基极输入电流有限导致 $C_{C1}$ 和 $C_{C2}$ 相互作用。也就是说，与CS放大器的情况不同，这里由$C_{C1}$和$C_{C2}$引起的两极中的每一个都将以一种复杂的方式依赖于这两个电容值，这妨碍了对设计的理解。
...
$$
当在这个表达式中代入数值时，很快就会发现三个电容器中哪个对fL的贡献最大。显然，这个贡献最大的电容器是时间常数最小的电容器。在射级放大电路中，通常是$C_E$，这是因为相关的电阻$R_{CE}$通常很小。知道哪种电容器有可能主导$f_L$具有重要的设计意义，如下所示。

##### Selecting Values for CC1, CE, and CC2

我们现在讨论为$C_{C1}$、$C_E$和$C_{C2}$选择适当值的设计问题。设计目标是将较低的3-dB频率$f_L$放置在指定位置，使电路中的总电容值最小。由于如上所述，$C_E$通常是三个电阻中最低的，因此通过选择$C_E$使总电容最小化，从而使其对$f_L$的贡献占主导地位。也就是说，通过参考上式，我们可以选择某个电容值$C_S$，使得$1/(C_ER_E)$，比方说，是80%的$\omega_L=2\pi f_L$，让其他每个电容器贡献10%的$ω_L$。例10.2应该有助于说明这个过程。

##### Example 10.2

我们希望为共射放大器选择一个合适的$C_{c1}$,$C_{C2}$,$C_E$,其中放大器电路参数为$R_B=100k\Omega$,$R_C=8k\Omega$,$R_L=5k\Omega$,$R_{sig}=5k\Omega,R_E=5k\Omega,\beta=100,g_m=40mA/V$,以及$r_\pi=2.5k\Omega$.设计要求为$f_L=100Hz$
...
$$
其中，$C_{\mu0}$是 CBJ 零偏时的大小；$V_{CB}$ 是CBJ反向偏置电压的大小，$V_{0c}$是CBJ内置电压（通常为0.75 V），m是其分级系数（通常为0.2–0.5）。
...
<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210516153916804.png" alt="image-20210516153916804" style="zoom:67%;" />
...
其中 $ω_{p1}$，$ω_{p2}$......$ω_{pn}$ 是指n个极点的频率，$ω_{Z2}$，······$ω_{Zn}$是表示n个零频率点的频率。在高频响应中，零频率点和极点的频率根据定义大于中频率，我们从等式（10.71）中可以看出，当 s 接近中间带频率时，$F_h(s)$大小将逐渐接近1，也使得放大电路的增益大小逐渐接近$A_M$
...
这是我们在第10.3节分析的共射极放大器的情况。根据经验法则，如果频率最低的极点与最近的极点或零频率点至少有四倍频的距离，则存在主极点，如果主极不存在，则 3-dB 频率 $ω_H$ 可由 $| F_H(jω)|$ 的图像确定。或采用如下的近似公式，为简单起见，仅考虑在高频带中具有两个极点和两个零频率点的电路的情况
...
<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210518193038701.png" alt="image-20210518193038701"  />
...
在图11.11（a）的串联-并联反馈放大器中，运算放大器输入电阻为$R_{in}$，开路电压增益$μ$和输出电阻$r_o$。 计算β的表达式，$A_f≡V_o$ / $V_s$的理想值和环路增益Aβ。
...
<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210524194928576.png" alt="image-20210524194928576" style="zoom:67%;" />
...
环路增益随频率变化的方式决定了反馈放大器的稳定与否。要了解这一事实，请考虑相位角 $φ(ω) =180°$ 的情况（换言之，通过该放大器以后输出和输入是反相的）. 在这个频率下，$ω_{180}$ 会使回路增益 $A(jω)β( jω)$ 变为负实数。因此，在这个频率的反馈将变为正反馈。如果在 $ω = ω_{180}$ 时的环路增益的大小小于 1，那么从等式（11.58）我们可以看出闭环增益 $A_f(jω)$ 将大于开环增益 $A(jω),$ 因为式（11.58）的分母将小于1。然而，反馈放大器将是稳定的。

另一方面，如果频率 $ω_{180}$环路增益的大小等于1，由式（11.58）可知，$A_f(jω)$ 将变为无穷大。这意味着放大器即使在输入为 0 的情况下仍然会有输出；这就是振荡器的定义。为了直观地观察这个反馈回路是如何振荡的，考虑图11.1的一般回路，外部输入$x_s$设置为零。电路中的任何干扰，如电源开关的闭合，将在放大器的输入处产生扰动信号 $x_i(t)$。这样的噪声信号通常包含很宽的频率范围，现在我们将集中讨论频率分量 $ω = ω_{180}$，即信号$X_i \sin(ω_{180})$。这个输入信号将产生如下的反馈信号
...
现在的问题是：如果 $ω_{180}$ 环路增益的大小大于1，将会发生什么？我们将不会从一般的角度来回答这个问题，而是我们在这里感兴趣的受限但非常重要的一类电路。答案（从公式（11.58）中不明显）是电路将振荡，并且振荡的幅度将增加，直到一些非线性的电流分量（总是以某种形式存在）将环路增益的幅度降低到精确的 1，此时将获得持续振荡。在第十八章的正弦波振荡器的设计中将采用这种机制，即通过使用环路增益大于 1 的正反馈来启动振荡，然后使用非线性来将环路增益减小到所需幅度（也就是1），这种机制将在正弦振荡器的设计中得到利用。不过现在恰恰相反，我们知道在负反馈放大器中如何发生振荡，我们希望找到防止振荡发生的方法。

##### 11.7.2 The Nyquist Plot（奈奎斯特图）

奈奎斯特图是基于上述讨论的用于测试稳定性的一种形式化方法。 它只是环路增益的极坐标图，其中频率用作参数。如下图所示

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210526200237730.png" alt="image-20210526200237730" style="zoom:67%;" />

==**（从上面的图可以很轻易看出，增益幅度（模长）在相角趋向于 $180°$ 时趋向于 $0$ ，这幅图很好地描绘出了增益与相角之间的关系）**==

请注意，径向距离为 $ |Aβ|$ 角度是相位角 $φ$，实线图表示正频率，由于环路增益（就此而言，物理网络的任何增益函数）都具有一个幅度，该幅度是频率的偶函数，而相位是频率的奇函数，因此 $Aβ$ 表示负频率（如图11.28所示）（如虚线）可以通过 Re 轴绘制镜像。

奈奎斯特图与负实轴在频率 $ω_{180}$ 处相交。 因此，如果此交点发生在点（–1，0）的左侧，我们知道该频率下环路增益的幅度大于 1，放大器将变得不稳定。 另一方面，如果相交出现在点（–1，0）的右侧，则放大器将保持稳定。 因此，如果奈奎斯特图围绕点（–1，0），则放大器将不稳定，但是，应该指出的是，该陈述是奈奎斯特准则的简化版本。 但是，它适用于我们感兴趣的所有电路。
...
<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210527110653675.png" alt="image-20210527110653675" style="zoom:67%;" />

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210527110707651.png" alt="image-20210527110707651" style="zoom:67%;" />
...
从第11.17和11.18节我们知道，可以通过检查其环路增益 $Aβ$ 作为频率的函数来确定反馈放大器是否稳定，最简单，最有效的方法之一是使用 $Aβ$ 的波特图，如图11.36所示。  （请注意，因为相位接近–360°，所以检查的电路是四阶电路。）图11.36中绘出了环路增益的反馈放大器将是稳定的，因为在180°的相移频率 $ω_{180}$处， 环路增益的幅度小于1（负dB）。  **==$ω_{180}$ 处的 $|A\beta|$ 值与单位增益的差值（注：是转换为分贝以后的差值）称为增益裕度==**，**<u>增益裕度表示在保持稳定性的同时可以增加环路增益的量。 反馈放大器通常设计为具有足够的增益裕度，以允许环路增益随温度，时间等不可避免地发生变化。</u>**
...
通过构建环路增益$Aβ$的Bode图来研究稳定性可能是一个冗长而耗时的过程，尤其是当我们必须研究给定放大器对于各种反馈网络的稳定性时。另一种更简单的方法是只构造开环增益$A(jω)$的Bode图。暂时假设β与频率无关，在这个波特图中同时画出 $20\lg(1/\beta)$ 和 $20\lg|A|$，两条曲线之差是
...
本章中的材料要求完全熟悉运算放大器电路应用。 因此，学习第 2 章是先决条件。(注：我们在分析电路的稳态响应时，将恒定电流源视作开路，恒定电压源视作短路)
...
观察到自然模位于半径为 $ω_p(1/\epsilon )^{ 1/N}$ 的圆上，并以 $π/N$ 的等角间隔，第一个模式与 $+jω$ 轴成 π/2N 角。 由于自然模都与原点的径向距离相等，因此它们都具有相同的频率 $ω_0 = ω_p(1/\epsilon )^{ 1/N}$ 。 要了解 N = 2、3 和 4 阶巴特沃斯滤波器的自然模，请分别参见图 17.10(b)、(c) 和 (d)。 一旦 N 个自然模 $p_1, p_2, .  .  .  ,p_N$ 已经找到，传递函数可以写为
...
<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210615202733067.png" alt="image-20210615202733067" style="zoom:67%;" />

观察到随着 $Q$ 的增加，带宽减少，带通滤波器变得更具选择性。

如果传输零点位于 $jω$ 轴上，在复共轭位置 $±jω_n$，则幅度响应在 $ω = ω_n$ 处呈现零传输。 因此，幅度响应中的陷波发生在 $ω = ω_n$ 处，而 $ω_n$ 被称为陷波频率。 二阶陷波滤波器有三种情况：常规陷波，当 $ω_n = ω_0$ 时获得 [图17.16(d)]； 低通陷波，当 $ω_n > ω_0$ [图17.16(e)]； 以及当 $ω_n < ω_0$ 时获得的高通陷波 [图17.16(f)]。我们敦促读者验证这些图中给出的响应细节（尽管这是一项相当乏味的任务！）。 请注意，在所有陷波情况下，dc 和 $s=∞$ 处的传输是有限的。 这是因为在 $s = 0$ 或 $s=∞$ 处没有传输零点。

最后一个感兴趣的特殊情况是全通 (AP) 滤波器，其特性如图 17.16(g) 所示。 这里两个传输零点位于 $s$ 平面的右半部分，在极点的镜像位置。  （对于任何阶的全通函数都是这种情况。）全通函数的幅度响应在所有频率上都是恒定的； 所谓平坦增益，在我们的例子中等于 $|a_2 |$。 全通函数的频率选择性在于其相位响应

### 17.5 The Second-Order LCR Resonator

在本节中，我们将研究图 17.17(a) 所示的二阶 LCR 谐振器。我们将使用该谐振器来推导各种二阶滤波器功能的电路实现。 下一节将说明，用使用运算放大器-RC 电路获得的模拟电感代替电感器 L 会产生运算放大器-RC 谐振器。

后者构成了将在 17.6 节中研究的一类重要的有源 RC 滤波器的基础。

##### 17.5.1 The Resonator Natural Modes

图 17.17(a) 并联谐振电路的自然模或极点可以通过施加不改变电路自然结构的激励来确定。

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210615203630929.png" alt="image-20210615203630929" style="zoom:67%;" />

图 17.17(b) 和 (c)显示了两种可能的电路激励方式 。 在图 17.17(b) 中，谐振器由并联的电流源 $I$ 激发。 由于就电路的自然响应而言，独立的理想电流源等效于开路，因此图 17.17(b) 的激励不会改变谐振器的自然结构。 因此，图 17.17(b) 中的电路可用于通过简单地找到响应函数的极点来确定谐振器的自然模。 例如，我们可以将谐振器两端的电压 $V_o$ 作为响应，从而获得响应函数 $V_o/I = Z$，其中 $Z$ 是并联谐振电路的阻抗。 但由于电路的并联结构，在导纳$Y$方面工作更方便； 因此，

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210615203608341.png" alt="image-20210615203608341" style="zoom:67%;" />

加入我们将传递函数的分母改写成标准形式，那么有

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210615203727253.png" alt="image-20210615203727253" style="zoom:67%;" />

因此

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210615203744076.png" alt="image-20210615203744076" style="zoom:67%;" />

从电路理论入门课程中对并联谐振电路的研究中，读者应该熟悉这些表达式。

图 17.17(c) 显示了另一种激发并联 $LCR$ 谐振器以确定其自然模的方法。 此处，电感器 $L$ 的节点 $x$ 已与地断开并连接到理想电压源 $V_i$。 现在，由于就电路的自然响应而言，理想的独立电压源等效于短路，图 17.17(c) 的激励不会改变谐振器的自然结构。因此我们可以使用图 17.17(c) 中的电路来确定谐振器的自然模。这些是任何响应函数的极点。 例如，我们可以选择 $V_o$ 作为响应变量并找到传递函数 $V_o/V_i$。 

##### 17.5.2 Realization of Transmission Zeros

选择了 LCR 谐振器的组件值来实现给定的一对复共轭自然模后，我们现在考虑使用谐振器来实现所需的滤波器类型（例如，LP、HP 等）。 具体来说，我们希望找出输入电压信号 $V_i$ 的注入位置，以便传递函数 $V_o/V_i$ 是所需的。 为此，请注意在图 17.17(a) 中的谐振器电路中，任何标记为 $x、y$ 或 $z$ 的节点都可以与地断开连接并连接到 $V_i$，而不会改变电路的自然模式。完成后，电路采用分压器的形式，如图 17.18(a) 所示。因此实现的传递函数为

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210615204309680.png" alt="image-20210615204309680" style="zoom:67%;" />

我们观察到传输零点是 $Z_2(s)$ 为零时的 $s$ 值，前提是 $Z_1(s)$ 不同时为零，或者 $Z_1(s)$ 处的 $s$ 值是无穷大，前提是 $Z_2(s)$ 是 不能同时无限。 因此我们可以从其中看出其物理意义：当 $Z_2(s)$ 表现为短路或 $Z_1(s)$ 表现为开路时，输出将为零。 如果存在 $Z_1$ 和 $Z_2$ 都为零的 $s$ 值，则 $V_o/V_i$ 将是有限的，并且不会获得传输零点。 类似地，如果存在 $Z_1$ 和 $Z_2$ 均为无穷大的 $s$ 值，则 $V_o/V_i$ 将是有限的，并且不会实现零传输。

##### 17.5.3 Realization of the Low-Pass Function

使用刚刚概述的方案，我们看到为了实现低通功能，节点 $x$ 与地断开并连接到 $V_i$，如图 17.18(b) 所示。 该电路的传输零点将位于串联阻抗变为无穷大（$sL$ 在 $s=∞$ 处变为无穷大）或者分流阻抗变为零时（$1/[sC +(1/R)]$ 在 $s=∞$ 处变为零）的 $s$ 值处

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210615204802547.png" alt="image-20210615204802547" style="zoom: 80%;" />

因此，该电路在 $s=∞$ 处有两个传输零点，正如二阶 LP 所假设的那样。 传递函数可以通过检查或使用分压器规律来编写。 按照后一种方法，我们得到

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616095841875.png" alt="image-20210616095841875" style="zoom: 67%;" />

##### 17.5.4 Realization of the High-Pass Function

为了实现二阶高通功能，节点 $y$ 与地断开并连接到 $V_i$，如图 17.18(c) 所示。 此处串联电容器在 $s = 0$ (dc) 处引入传输零点，并联电感器在 $s = 0$ (dc) 处引入另一个传输零点。因此，通过检查，传递函数可以写为 

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616100120551.png" alt="image-20210616100120551" style="zoom:67%;" />

其中 $ω_0$ 和 $Q$ 是自然模参数。$a_2$是高频传输。  $a_2$ 的值可以通过观察电路确定，随着 $s$ 接近$∞$，电容器接近短路，$V_o$ 接近 $V_i$，导致 $a_2 = 1$。

##### 17.5.5 Realization of the Bandpass Function

带通功能是通过将节点 $z$ 与地断开并将其连接到 $V_i$ 来实现的，如图 17.18（d）所示。 这里串联阻抗是电阻性的，因此不会引入任何传输零点。 它们的获得方式如下： $s = 0$ 处的一个零由并联电感器实现，而 $s=$$∞$ 处的一个零由并联电容器实现。 在中心频率 $ω_0$ 处，并联 LC 调谐电路呈现无限阻抗，因此电路中没有电流流动。 因此，在 $ω = ω_0$ 处，$V_o = V_i$。 换句话说，带通滤波器的中心频率增益为 1。 其传递函数可得如下：

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616102834135.png" alt="image-20210616102834135" style="zoom:67%;" />

##### 17.5.6 Realization of the Notch Functions

为了在 $jω$ 轴上获得一对传输零点，我们在串联中使用了一个并联谐振电路，如图 17.18(e) 所示。 观察该电路是通过将节点 $x$ 和 $y$ 与地断开并将它们连接到 $V_i$ 来获得的。  LC 电路的阻抗在 $ω =ω_0 =1/ \sqrt{LC}$ 处变为无穷大，因此在该频率下导致零传输。

分流阻抗是电阻性的，因此不会引入传输零点。图 17.18(e) 将实现陷波传递函数

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616103251876.png" alt="image-20210616103251876" style="zoom:67%;" />

从电路中可以看出高频增益$a_2$的值为1。为了获得陷波频率 $ω_n$ 可被任意设置的陷波滤波器实现，我们采用了上述方案的变体。 我们仍然在串联支路中使用并联 LC 电路，如图 17.18(f) 所示，其中选择 $L_1$ 和 $C_1$ 使得 

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616103536137.png" alt="image-20210616103536137" style="zoom:67%;" />

因此，$L_1C_1$ 谐振电路将在 $±jω_n$ 处引入一对传输零点，前提是 $L_2C_2$ 谐振电路在 $ω_n$ 处不谐振。 除此限制外，必须选择 $L_2$ 和 $C_2$ 的值以确保自然模没有改变； 因此

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616104617160.png" alt="image-20210616104617160" style="zoom:67%;" />

换句话说，当$V_i$被短路代替时，电路应该退化为原来的LCR谐振器。 考虑图 17.18(f) 电路的另一种方式是，它是通过将一部分 $L$ 和一部分 $C$ 抬离地并将它们连接到 $V_i$ 来从原始 LCR 谐振器获得的。

应该注意的是，在图 17.18(f) 的电路中，$L_2$ 在 $s = 0$ 时不会引入零点，因为在 $s = 0$ 时，$L_1C_1$ 电路也有一个零点。 事实上，在 $s = 0$ 时，电路简化为电感分压器，直流传输为 $L_2/(L_1 +L_2)$。 可以对 $C_2$ 以及它在 $s=∞$ 处不引入零以及在 $s=∞$ 处的传输为 $C_1/(C_1 +C_2)$ 作出类似的评论。

LPN 和 HPN 滤波器的实现是图 17.18(f) 中通用陷波电路的特例。 具体来说，对于 LPN，

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616105430161.png" alt="image-20210616105430161" style="zoom:67%;" />

因此

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616105509791.png" alt="image-20210616105509791" style="zoom:67%;" />

该条件可以通过消除 $L_2$ 来满足（即，$L_2 =∞$ 且 $L_1 = L$），从而产生图 17.18(g) 中的 LPN 电路。 传递函数可以通过检查写为

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616105633132.png" alt="image-20210616105633132" style="zoom:67%;" />

其中$ω^ 2_ n =1/LC_1，ω ^2_ 0 =1/L(C_1 +C_2)，ω_0/Q=1/CR，a_2$ 为高频增益。 从电路中我们看到，当 $s→∞$ 时，电路简化为图 17.18(h) 中的电路，其中

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616105809028.png" alt="image-20210616105809028" style="zoom:67%;" />

因此

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616105855850.png" alt="image-20210616105855850" style="zoom:67%;" />

为了获得 HPN 实现，我们从图 17.18(f) 的电路开始，并使用 $ω_n <ω_0$ 的事实来获得

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616105933201.png" alt="image-20210616105933201" style="zoom:67%;" />

选择$C_2 = 0$（即，$C_1 = C$）时可以满足。 这样我们就得到了图 17.18(i) 所示的简化电路。 观察到随着 $s→∞，V_o$ 接近 $V_i$，因此高频增益为 1。 因此，传递函数可以表示为

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616110105679.png" alt="image-20210616110105679" style="zoom:67%;" />

##### 17.5.7 Realization of the All-Pass Function

全通传递函数

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616135412625.png" alt="image-20210616135412625" style="zoom:67%;" />

可以写成

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616135442055.png" alt="image-20210616135442055" style="zoom: 67%;" />

右侧的第二项是中心频率增益为 2 的带通函数。我们已经有了一个带通电路（图 17.18d），但中心频率增益为 1。 因此，我们将尝试以 0.5 的平坦增益实现全通实现，即，

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616135522770.png" alt="image-20210616135522770" style="zoom:67%;" />

该功能可以使用传输比为 0.5 的分压器以及图 17.18(d) 的带通电路来实现。 为了实现减法，在分压器的输出端和带通滤波器的输出端之间取全通电路的输出，如图 17.19 所示。 不幸的是，该电路的缺点是在输入和输出之间缺少公共接地端子。 全通功能的运算放大器-RC 实现将在下一节中介绍。

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616135741751.png" alt="image-20210616135741751" style="zoom:67%;" />

### 17.6 Second-Order Active Filters Based on Inductor Replacement

在本节中，我们研究了一系列实现各种二阶滤波器功能的运算放大器 - RC 电路。 该电路基于一个运放-RC 谐振器，该谐振器通过将 LCR 谐振器中的电感 L 替换为具有电感输入阻抗的运放-RC 电路而获得。

##### 17.6.1 The Antoniou Inductance-Simulation Circuit

多年来，已经提出了许多运算放大器-RC 电路来模拟电感器的操作。 其中，A. Antoniou 发明的一种电路（参见 Antoniou，1969）已被证明是“最好的”。  “最佳”是指电路的运行非常能容忍运算放大器的非理想特性，尤其是它们的有限增益和带宽。 图 17.20(a) 显示了 Antoniou 电感仿真电路。 

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616140423242.png" alt="image-20210616140423242" style="zoom:67%;" />

如果电路在其输入端（节点 1）馈入电压源 $V_1$ 且输入电流表示为 $I_1$，则对于理想运算放大器，输入阻抗可表示为

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616140441933.png" alt="image-20210616140441933" style="zoom:67%;" />

它是由下式给出的电感 $L$

图 17.20(b) 显示了电路分析，假设运算放大器是理想的，因此每个运算放大器的两个输入端子之间出现虚拟短路，并且还假设运算放大器的输入电流为零。 分析从节点 1 开始，假设该节点由电压源 $V_1$ 供电，然后逐步进行，步骤的顺序由带圆圈的数字表示。 分析结果是输入电流 $I_1$ 的表达式，从中可以找到 $Z_{in}$。

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616140617900.png" alt="image-20210616140617900" style="zoom:67%;" />

该电路的设计通常基于选择$R_1 =R_2 =R_3 =R_5 =R$ 和$C_4 =C$，从而导致$L = CR_2。$ 然后为 $C$ 和 $R$ 选择方便的值以产生所需的电感值 $L$。有关该电路的更多详细信息以及运算放大器的非理想性对其性能的影响，请参见 Sedra 和 Brackett (1978)。

##### 17.6.2 The Op Amp–RC Resonator

图 17.21(a) 显示了我们在 17.5 节中详细研究的 LCR 谐振器。 用图 17.20(a) 的 Antoniou 电路实现的模拟电感替换电感器 L 产生图 17.21(b) 的运算放大器-RC 谐振器。  （暂时忽略用虚线绘制的附加放大器。）

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616140843212.png" alt="image-20210616140843212" style="zoom:67%;" />

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616140856121.png" alt="image-20210616140856121" style="zoom:67%;" />

图 17.21(b) 的电路是一个具有以下极点频率的二阶谐振器

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616140941400.png" alt="image-20210616140941400" style="zoom:67%;" />

我们使用了方程中给出的 $L$ 的表达式。极点 $Q$ 因子可以使用等式中的表达式获得。 $C = C_6$ 且 $R = R_6$； 因此，$Q = ω_0 C_6 R_6。$

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616141104097.png" alt="image-20210616141104097" style="zoom:67%;" />

通常选择 $C_4 = C_6 = C$ 和 $R_1 = R_2 = R_3 = R_5 = R$，结果为

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616141151507.png" alt="image-20210616141151507" style="zoom:67%;" />

因此，如果我们为 $C$ 选择一个实际方便的值，我们可以使用上面的等式确定 $R$ 的值以实现给定的 $ω_0$，然后经过同样的步骤确定 $R_6$ 的值以实现给定的 $Q$。

##### 17.6.3 Realization of the Various Filter Types

图 17.21(b) 的运算放大器-RC 谐振器可用于通过遵循第 17.5 节中详细描述的与 LCR 谐振器相关的方法来生成各种二阶滤波器功能的电路实现。 因此，为了获得带通函数，我们将节点 $z$ 与地断开并将其连接到信号源 $V_i$。 通过向节点 $y$ 注入 $V_i$ 获得高通函数。 为了使用 LCR 谐振器实现低通功能，电感端子 $x$ 与地断开连接并连接到 $V_i$。 有源谐振器中的相应节点是 $R_5$ 接地的节点，在图 17.21(b) 中标记为节点 $x$。 通过将 $V_i$ 馈送到节点 $x$ 和 $y$ 来获得常规陷波函数 ($ω_n = ω_0$)。 在所有情况下，输出都可以作为谐振电路两端的电压 $V_r$。

然而，这不是用作滤波器输出端的方便节点，因为在那里连接负载会改变滤波器特性。 使用缓冲放大器可以轻松解决该问题。 这是增益为 $K$ 的放大器，在图 17.21(b) 中用虚线画出。 图 17.21(c) 显示了如何使用以同相配置连接的运算放大器来简单地实现该放大器。 注意，放大器k不仅可以缓冲滤波器的输出，而且还允许设计者通过适当地选择$K$的值来将滤波器增益设置为任何所需的值。

图 17.22 显示了从图 17.21(b) 的谐振器获得的各种二阶滤波器电路。 这些电路的传递函数和设计方程在表 17.1 中给出。 请注意，传递函数可以类比为 LCR 谐振器的传递函数。 我们已经对图 17.22(a) 到 (d) 中给出的 LP、HP、BP 和常规陷波电路进行了评论。 图 17.22(e) 和 (f) 中的 LPN 和 HPN 电路分别通过直接类比图 17.18(g) 和 (i) 中的 LCR 对应电路而获得。 然而，图 17.22(g) 中的全通电路值得一些解释。

##### 17.6.4 The All-Pass Circuit

![image-20210616141625013](C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616141625013.png)

此前我们看到具有单位增益平坦的全通函数可以写成 $AP = 1−(BP$，中心频率增益为 $2)$ 传递函数以这种方式相关的两个电路（即 ，一个的传递函数等于1 减去另一个的传递函数）被称为互补电路（补码电路）。因此，具有单位平坦增益的全通电路是中心频率增益为 2 的带通电路的补码。 获得给定线性电路的补码的简单程序是：断开所有接地的电路节点，并将它们连接到 $V_i$，并断开所有连接到 $V_i$ 的节点并将它们接地。 也就是说，在线性电路中交换输入和接地会生成一个电路，其传递函数是原始电路的传递函数的补码。

回到手头的问题，我们首先使用图 17.22(c) 的电路，通过简单地选择 $K = 2$ 并使用图 17.21(c) 的电路实现缓冲放大器来实现增益为 2 的 BP  $r_1 = r_2$。 然后我们交换输入和接地，从而获得图 17.22(g) 的全通电路。

最后，除了设计简单之外，图 17.22 中的电路还表现出出色的性能。 它们可以单独使用实现二阶滤波器功能，也可以级联实现高阶滤波器

![image-20210616142045027](C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616142045027.png)

![image-20210616142106500](C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616142106500.png)

![image-20210616142122518](C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616142122518.png)

### 17.7 Second-Order Active Filters Based on the Two-Integrator-Loop Topology

在本节中，我们研究实现二阶滤波器功能的另一系列运算放大器 – RC 电路。 该电路基于在整个反馈回路中使用两个级联的积分器，因此被称为双积分器回路

##### 17.7.1 Derivation of the Two-Integrator-Loop Biquad

为了推导出双积分器回路双二次电路，或众所周知的双二次电路，考虑二阶高通传递函数

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616142327757.png" alt="image-20210616142327757" style="zoom:67%;" />

其中 $K$ 是高频增益。 将等式两侧交叉相乘，并将结果方程的两边除以 $s^2$（以将所有涉及 $s$ 的项变为 $1/s$ ，这是积分器的传递函数）给出

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616142529187.png" alt="image-20210616142529187" style="zoom:67%;" />

在这个方程中，我们观察到信号 $(ω_0/s)V_{hp}$ 可以通过使 $V_{hp}$ 通过时间常数等于 $1/ω_0$ 的积分器获得。 此外，将结果信号通过另一个相同的积分器导致第三个方程中涉及 $V_{hp}$的信号——即，$(ω^ 2_ 0/s_2 )V_{hp}$。 图 17.23(a) 显示了这种双积分器配置的框图。 请注意，考虑到使用反相运算放大器米勒积分器电路（第 2.5.2 节）来实现每个积分器，图 17.23(a) 中的积分器块已分配负号。

然而，问题仍然存在，即如何形成 $V_{hp}$，即馈送两个级联积分器的输入信号。 为此，我们重新排列方程，用单和双集成版本表示 Vhp，而 $V_i$ 表示为

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616144016496.png" alt="image-20210616144016496" style="zoom:67%;" />

这表明可以通过使用图 17.23(b) 的加权求和获得 $V_{hp}$。 现在应该很容易看出，通过将图 17.23(a) 的积分器块与图 17.23(b) 的求和器块组合，可以获得完整的框图实现，如图 17.23(c) 所示。

\在图 17.23(c) 的实现中，在加法器的输出处获得的 $V_{hp}$ 实现了方程 3 的高通传递函数 $T_{hp} ≡ V_{hp}/V_i$。第一个积分器输出端的信号为$–(ω_0/s)V_{hp}$，这是一个带通函数

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616144141160.png" alt="image-20210616144141160" style="zoom:67%;" />

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616144155517.png" alt="image-20210616144155517" style="zoom:67%;" />

因此，第一个积分器输出端的信号标记为 $V_{bp}$。 请注意，实现的带通滤波器的中心频率增益等于 $–KQ$。

以类似的方式，我们可以证明在第二个积分器的输出端实现的传递函数是低通函数， 

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616144301414.png" alt="image-20210616144301414" style="zoom:67%;" />

因此，第二个积分器的输出被标记为 $V_{lp}$。 请注意，实现的低通滤波器的直流增益等于 $K$。

我们得出结论，图 17.23(c) 中以框图形式显示的双积分器环路双二阶同时实现了三个基本的二阶滤波函数 LP、BP 和 HP。 这种多功能性使该电路非常受欢迎，并为其命名为通用有源滤波器。

##### 17.7.2 Circuit Implementation

为了获得图 17.23(c) 的双积分器环路双二阶运算放大器电路实现，我们用一个具有 $CR = 1/ω_0$ 的米勒积分器电路替换每个积分器，并且我们用一个运算器替换加法器块使其能够为其输入分配正负权重的放大器求和电路。 由此产生的电路，被称为 Kerwin-Huelsman-Newcomb 或 KHN biquad，以其发明者的名字命名，如图 17.24（a）所示。

给定 $ω_0、Q$ 和 $K$ 的值，电路的设计很简单：我们为积分器的组件 $C$ 和 $R$ 选择合适的实用值，以便 $CR=1/ω_0$。 为了确定与加法器相关的电阻值，我们首先使用叠加原理将加法器 $V_{hp}$ 的输出表示为其输入 $V_i、V_{bp}$ 和 $V_{lp}$ 为

![image-20210616144507391](C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616144507391.png)

![image-20210616184727437](C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616184727437.png)

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616184748761.png" alt="image-20210616184748761" style="zoom:67%;" />

代入 $V_{bp} =−(ω_0/s)V_{hp}$ 和 $V_{lp} = ω_0^ 2/s^2 V_{hp}$ ,给出

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616184958364.png" alt="image-20210616184958364" style="zoom:67%;" />

等式的最后一项。

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616185135543.png" alt="image-20210616185135543" style="zoom:67%;" />

这意味着我们可以为 $R_1$ 和 $R_f$ 选择任意但实际上方便的相等值。 然后，将方程右侧的倒数第二项等同起来。并设置 $R_1 = R_f$ 产生实现给定 $Q$ 所需的比率 $R_3/R_2$

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616185311735.png" alt="image-20210616185311735" style="zoom:67%;" />

因此，可以为 $R_2$ 或 $R_3$ 选择任意但方便的值，另一个电阻的值可以使用等式确定。  (17.63)。 最后，将方程中的 $V_i$ 系数相等。  (17.61) 和 (17.58) 并代入 $R_f = R_1$ 和来自等式的 R3/R2。  (17.63) 结果

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616185422119.png" alt="image-20210616185422119" style="zoom:67%;" />

因此增益参数 $K$ 固定为该值。

KHN 双二阶可用于通过对三个输出（LP、BP 和 HP）的加权版本求和来实现陷波和全通功能。 图 17.24(b) 显示了这样的运算放大器加法器；因此我们可以写 

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616185942434.png" alt="image-20210616185942434" style="zoom:67%;" />

## CHAPTER 18 Signal Generators and Waveform- Shaping Circuits

在电子系统的设计中，经常需要具有规定标准波形的信号，例如正弦、方波、三角波或脉冲。 需要标准信号的系统包括需要时钟脉冲的计算机和控制系统，其中包括定时； 以各种波形的信号作为信息载体的通信系统； 以及测试和测量系统，其中使用各种波形的信号来测试和表征电子设备和电路。 在本章中，我们研究信号发生器电路。

本章中研究的信号发生器或振荡器电路共同能够提供频率在赫兹到数百吉赫兹范围内的信号。有些可以在芯片上制造，有些则使用分立元件。 常见振荡器的例子包括微处理器时钟发生器（利用第 16.4.4 节中研究的环形振荡器在芯片上制造，频率在几千兆赫范围内）； 无线收发器中的载波波形发生器（片上，高达数百千兆赫范围）； 电子表中的振荡器以及电子实验室中的变频函数发生器（使用频率在赫兹到兆赫兹范围内的分立电路）。

有两种截然不同的生成正弦波的方法，这可能是最常用的标准波形。 第 18.1 至 18.3 节中研究的第一种方法采用由放大器和 RC 或 LC 频率选择网络组成的正反馈回路。 虽然生成的正弦波的频率由频率选择网络确定，但幅度是使用非线性机制设置的，通过单独的电路或使用放大设备本身的非线性来实现。 尽管如此，这些利用共振现象产生正弦波的电路被称为线性振荡器。 该名称清楚地将它们与通过第二种方法生成正弦波的电路区分开来。 在这些电路中，通过适当地整形三角波来获得正弦波。 我们在第 18.8 节中研究了三角波形发生器之后的波形整形电路。

产生方波、三角波、脉冲（等）波形的电路（称为非线性振荡器或函数发生器）采用称为多谐振荡器的电路构建块。

多谐振荡器分为三种类型：双稳态（第 18.4 节）、非稳态（第 18.5 节）和单稳态（第 18.6 节）。 本章介绍的多谐振荡器电路采用运算放大器，适用于精密模拟应用。 第 16 章研究了使用数字逻辑门的双稳态和单稳态多谐振荡器电路。

通过在反馈回路中连接双稳态多谐振荡器和运算放大器积分器（第 18.5 节），可以获得生成方波和三角波的通用方案。 使用商用通用 IC 芯片 555 定时器（第 18.7 节）可以获得类似的结果。

### 18.1 Basic Principles of Sinusoidal Oscillators

在本节中，我们研究线性正弦波振荡器设计的基本原理。 尽管名称为线性振荡器，但必须采用某种形式的非线性来控制输出正弦波的幅度。 事实上，所有的振荡器本质上都是非线性电路。 这使振荡器的分析和设计任务复杂化：不再能够直接应用变换（s 平面）方法。 尽管如此，已经开发了一些技术，通过这些技术可以分两个步骤执行正弦振荡器的设计：第一步是线性步骤，并且可以很容易地采用反馈电路分析的频域方法。随后，在步骤 2 中，可以提供幅度控制的非线性机制

##### 18.1.1 The Oscillator Feedback Loop

正弦振荡器的基本结构由一个放大器和一个连接在正反馈回路中的选频网络组成，如图 18.1 中的框图形式所示。 尽管在实际的振荡器电路中不会出现输入信号，但我们在此处包含了一个输入信号以帮助解释工作原理。 重要的是要注意，与图 11.1 的负反馈回路不同，这里的反馈信号 $x_f$ 与正号相加。 因此，反馈增益由下式给出

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616190942403.png" alt="image-20210616190942403" style="zoom:67%;" />

我们注意到分母中的负号。 环路增益 $L(s)$ 由下式给出

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616191014890.png" alt="image-20210616191014890" style="zoom:67%;" />

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616191035830.png" alt="image-20210616191035830" style="zoom:67%;" />

特征方程为

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616191104155.png" alt="image-20210616191104155" style="zoom:67%;" />

##### 18.1.2 The Oscillation Criterion

如果在特定频率 $f_0$ 处，环路增益 $Aβ$ 等于 1，则由不难得到 $A_f$ 将是无限的。 也就是说，在该频率下，对于零输入信号，电路将具有有限的输出。 根据定义，这样的电路是振荡器。 因此，图 18.1 的反馈回路提供频率为 $ω_0$ 的正弦振荡的条件是

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616191231286.png" alt="image-20210616191231286" style="zoom:67%;" />

也就是说，在 $ω_0$ 处，环路增益的相位应为零，环路增益的幅度应为 1。 这被称为巴克豪森准则。 请注意，对于以一种频率振荡的电路，振荡准则应仅在一种频率（即 $ω_0$）下满足； 否则产生的波形将不是简单的正弦曲线。

通过再次考虑图 18.1 的反馈回路，可以获得对巴克豪森准则的直观感觉。 为了使这个循环产生并维持一个没有输入的输出 $x_o (x_s = 0)$，反馈信号 $x_f$ ，

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616191549348.png" alt="image-20210616191549348" style="zoom:67%;" />

应该足够大，当乘以 $A$ 时，它会产生 $x_o$，即，

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616191629651.png" alt="image-20210616191629651" style="zoom:67%;" />

因此

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616191650952.png" alt="image-20210616191650952" style="zoom:67%;" />

这导致

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616191715588.png" alt="image-20210616191715588" style="zoom:67%;" />

需要注意的是，振荡频率$ω_0$ 仅由反馈回路的相位特性决定； 环路以相位为零的频率（或等效地为 $360°$）振荡。 因此，振荡频率的稳定性将由反馈回路的相位 $φ(ω)$ 随频率变化的方式决定。  “陡峭”函数 $φ(ω)$ 将导致更稳定的频率。 如果想象由于电路组件之一的变化而导致相位 $φ$ 的变化，则可以看出这一点。 如果 $dφ/dω$ 很大，那么 $ω_0$ 的变化就会很小，如图 18.2 所示。

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616191849016.png" alt="image-20210616191849016" style="zoom:67%;" />

研究振荡器电路的另一种方法包括检查电路极点，这是特征方程（方程 18.3）的根。 对于在频率 $ω_0$ 下产生持续振荡的电路，特征方程的根必须为 $s= ±jω_0$。 因此 $1−A(s)β(s)$ 应该具有形式为 $s^2+ω^ 2_ 0$ 的因子。

##### 18.1.3 Analysis of Oscillator Circuits

对给定振荡器电路进行分析，以确定振荡频率和振荡开始的条件，分三步进行：

1.断开反馈回路以确定回路增益 $A(s)β(s)$。 此步骤类似于第 11.2 节中分析负反馈放大器时使用的步骤。

2.振荡频率 $ω_0$ 为 $A(jω) β(jω)$ 的相位角为零或等效为 360° 的频率。

3.振荡开始的条件是从

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616192231881.png" alt="image-20210616192231881" style="zoom:67%;" />

请注意，使环路增益的幅度略大于 1 可确保振荡开始。

##### Example 18.1

图 18.3(a) 显示了通过在正增益放大器的反馈路径中放置二阶 LCR 带通滤波器 [见图 17.18(d)] 形成的正弦振荡器。 求振荡频率$ω_0$，以及振荡开始的条件。 假设运算放大器是理想的

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616192403212.png" alt="image-20210616192403212" style="zoom: 67%;" />

为了获得环路增益，我们在输入阻抗无穷大的运算放大器的正输入端断开正反馈环路，施加输入电压 $V_i$ 并找到返回电压 $V_r$ 。 这导致了图 18.3(b) 中的电路。 由于运算放大器是理想的，我们可以找到它的输出电压 $V_o$，因此 $A(s)$ 为

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616193349798.png" alt="image-20210616193349798" style="zoom:67%;" />

频率选择网络的传递函数 $β(s) ≡ V_r /V_o$ 可以使用公式 2 找到。

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616193444879.png" alt="image-20210616193444879" style="zoom:67%;" />

虽然我们发现分别确定 $A(s)$ 和 $β(s)$ 很容易，但实际上我们感兴趣的是它们的乘积，环路增益 $A(s) β(s)$，

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616193635343.png" alt="image-20210616193635343" style="zoom:67%;" />

代入 $s = jω$，

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616193719786.png" alt="image-20210616193719786" style="zoom:67%;" />

从这个表达式我们看到 $A(jω) β(jω)$ 的相位角在使分母实部为零的 $ω$ 值处为零，因此

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616193800553.png" alt="image-20210616193800553" style="zoom:67%;" />

在该频率下，环路增益的幅度由下式给出

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616193832365.png" alt="image-20210616193832365" style="zoom:67%;" />

因此振荡将开始

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616193855293.png" alt="image-20210616193855293" style="zoom:67%;" />

虽然 $r_2 = 0$ 理论上足以维持振荡，但通常设计 $r_2/r_1 > 0$ 以确保振荡开始。 这一点将在下一节中更详细地解释。

在离开这个例子之前，我们观察到可以通过检查图 18.3(a) 中的电路获得结果。 由于放大器增益的相位角为零，我们检查 LCR 带通电路以确定其相位为零的频率。 这又是 LC 谐振回路具有无限阻抗时的频率，即谐振频率 $ω_0 = 1/\sqrt{LC}$。 在该频率下，带通 LCR 电路的传输是统一的。 因此，要开始振荡，放大器增益必须大于或等于 1

**An Alternative Analysis Approach**

有一种简单的替代方法可用于分析振荡器电路，无需中断反馈回路。 该方法如下进行：我们假设电路正在振荡，因此具有振荡频率 $ω_0$ 的电压和电流信号。 我们以通常的方式分析电路，并根据单个电压或电流变量将方程简化为单个方程。 由于电压或电流量不为零（因为假设电路是振荡的），我们可以除以变量，从而消除它。 然后可以将方程处理为形式

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616194128982.png" alt="image-20210616194128982" style="zoom:67%;" />

其中 $D(s)$ 是 $s$ 中的多项式。 代入 $s = jω$，然后我们将 $D(jω)$ 的实部和虚部等同为零。 结果方程之一产生$ω_0$，另一个提供持续振荡的条件。 此方法将在下一节中多次使用。

##### 18.1.4 Nonlinear Amplitude Control

振荡条件，即第 18.1.2 节中讨论的巴克豪森准则，保证了数学意义上的持续振荡。 然而，众所周知，任何物理系统的参数都不能在任何时间长度内保持恒定。 换句话说，假设我们努力使$|Aβ|$在 $ω=ω_0$ 时$  =1$ ，但外界条件的改变，比如温度变化，可能使 $|Aβ|$ 变得略低于1。 显然，在这种情况下振荡将停止。 相反，如果$|Aβ|$ 超过1，振荡幅度将增长。 因此，我们需要一种能强制稳定 $|Aβ|$ 的机制。 在所需的输出幅度值处使其保持等于 1。 该任务是通过提供用于增益控制的非线性电路来完成的。

基本上，增益控制机制的功能如下：首先，为了确保振荡开始，设计电路使得$|Aβ|$ 略大于1。 这对应于设计电路以使极点位于 $s$ 平面的右半部分。 因此，当电源打开时，振荡的幅度会增加。 当幅度达到所需水平时，非线性网络开始起作用，并使环路增益减小到1。 换句话说，极点将被“拉回”到 $jω$ 轴。 这个动作将导致电路以所需的幅度维持振荡。 如果由于某种原因环路增益降低到低于 1，正弦波的幅度将减小。 这将被非线性网络检测到，这将导致环路增益增加到1。

正如将要看到的，非线性振幅稳定机制的实现有两种基本方法。 第一种方法使用限幅器电路（参见第 4.6 节）。 允许振荡增长，直到幅度达到限制器设置的水平。 当限幅器开始工作时，幅度保持恒定。

显然，限幅器应该是“软的”以最小化非线性失真。 然而，这种失真可以通过反馈回路中频率选择网络的滤波作用来减少。 事实上，在第 18.2 节中研究的振荡器电路之一中，正弦波是硬限制的，产生的方波被施加到反馈回路中的带通滤波器。 输出正弦波的“纯度”将是该滤波器选择性的函数。也就是说，滤波器的 $Q$ 值越高，正弦波输出的谐波含量越少。

另一种幅度控制机制利用一个元件，其电阻可由输出正弦波的幅度控制。 通过将此元件放置在反馈电路中，使其电阻决定环路增益，可以设计电路以确保环路增益在所需的输出幅度下达到统一。 在三极管区域中工作的二极管或 JFET 1 通常用于实现受控电阻元件。

##### 18.1.5 A Popular Limiter Circuit for Amplitude Control

我们通过介绍经常用于运算放大器振荡器的幅度控制以及各种其他应用的限幅器电路来结束本节。 该电路比第 4 章中介绍的电路更精确、更通用。

限幅电路如图 18.4(a) 所示，其传递特性如图 18.4(b) 所示。 

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616195434749.png" alt="image-20210616195434749" style="zoom:67%;" />

要了解如何获得传递特性，首先考虑小（接近零）输入信号 $v_I$ 和小输出电压 $v_O$ 的情况，因此 $v_A$ 为正，$v_B$ 为负。 很容易看出，二极管 $D_1$ 和 $D_2$ 都将关闭。 因此，所有输入电流 $v_I /R_1$ 都流经反馈电阻 $R_f$，输出电压由下式给出

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616195525619.png" alt="image-20210616195525619" style="zoom:67%;" />

这是图 18.4(b) 中限幅器传递特性的线性部分。 我们现在可以使用叠加来根据 $±V$ 和 $v_O$ 找到节点 A 和 B 处的电压为

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616195611360.png" alt="image-20210616195611360" style="zoom:67%;" />

随着 $v_I$ 变为正值，$v_O$ 变为负值，从方程可以看出。 并且 $v_B$ 将变得更负，从而保持 $D_2$ 关闭。 然而，等式表明 $v_A$ 变得不那么正了。 然后，如果我们继续增加 $v_I$，将达到 $v_O$ 的负值，此时 $v_A$ 变为 $-0.7 V$ 左右，二极管 $D_1$ 导通。 如果我们对 $D_1$ 使用恒压降模型并表示压降 $V_D$，则 $D_1$ 导通时的 $v_O$ 值可以从式（1）中找到。  (18.6)。 这是负限制水平，我们表示为 $L_−$，

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616200033604.png" alt="image-20210616200033604" style="zoom:67%;" />

可以通过将 $L_−$ 除以限制器增益 $−R_f /R_1$ 来找到 $v_I$ 的相应值。 如果 $v_I$ 增加到超过这个值，更多的电流被注入 $D_1$，并且 $v_A$ 保持在大约 $-V_D$。 因此流过 $R_2$ 的电流保持恒定，而额外的二极管电流流过 $R_3$。 因此，$R_3$ 与 $R_f$ 并联出现，增量增益（忽略二极管电阻）为 $-(R_f|| R_3)/R_1$。 为了使传输特性在极限区域内的斜率变小，应为 $R_3$ 选择一个较低的值。

可以以与刚刚采用的相同的方式找到负 $v_I$ 的传递特性。 很容易看出，对于负 $v_I$ ，二极管 $D_2$ 的作用与二极管 $D_1$ 为正 $v_I$ 发挥的作用相同。 我们可以使用方程。  (18.7) 找到正极限水平 $L_+$

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210616200557189.png" alt="image-20210616200557189" style="zoom:67%;" />

正极限区域的传递特性斜率为$-(R_f|| R_4)/R_1$。 因此，我们看到图 18.4(a) 的电路用作软限幅器，具有限幅电平 $L_+$ 和 $L_-$，并且限幅增益可通过选择适当的电阻值独立调节。

最后，我们注意到增加 $R_f$ 会导致线性区域中的更高增益，同时保持 $L_+$ 和 $L_−$ 不变。 在极限情况下，完全去除 $R_f$ 会导致图 18.4(c) 的传输特性，即比较器的传输特性。 也就是说，电路将 $v_I$ 与 $0 V$ 的比较器参考值进行比较：$v_I > 0$ 导致 $v_O\simeq L_-$，而 $v_I$ $<0$ 产生 $v_O\simeq  L+$。

### 18.2 Op Amp–RC Oscillator Circuits

在本节中，我们将研究一些使用运算放大器和 RC 网络的实用振荡器电路。 这些电路通常组装在印刷电路板上； 它们的工作频率从非常低的频率一直到最多 1 MHz。

##### 18.2.1 The Wien-Bridge Oscillator

最简单的振荡器电路之一是维恩电桥。 图 18.5 显示了一个没有非线性增益控制网络的维恩桥振荡器。 该电路由一个以同相配置连接的运算放大器组成，闭环增益为 $1+R_2/R_1$。

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210628201836553.png" alt="image-20210628201836553" style="zoom:67%;" />

在这个正增益放大器的反馈路径中，连接了一个 RC 网络。 通过将反馈网络的传递函数 $V_a(s)/V_o(s)$ 乘以放大器增益，可以很容易地获得环路增益，

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210628201934327.png" alt="image-20210628201934327" style="zoom:67%;" />

因此

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210628202011497.png" alt="image-20210628202011497" style="zoom:67%;" />

改换为物理频率，有

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210628202032625.png" alt="image-20210628202032625" style="zoom:67%;" />

环路增益将是一个实数（相位为零时），在一个频率上由下式给出

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210628202102189.png" alt="image-20210628202102189" style="zoom:67%;" />

如果环路增益至少为 1，则振荡将从该频率开始。 这可以通过选择开环增益的值来实现，具体而言，是改变其中的参数

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210628202216823.png" alt="image-20210628202216823" style="zoom:67%;" />

为了确保振荡会开始，我们一般选择使 $R_2/R_1$ 略大于 2。读者可以很容易地验证，如果 $R_2/R_1 = 2+δ$，其中 $δ$ 是一个小数，特征方程 $1−L(s  ) = 0$ 将在 s 平面的右半部分。

振荡幅度可以通过使用非线性控制网络来确定和稳定。 幅度控制功能的两种不同实现方式，如图  18.6 和图  18.7 所示。

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210628202418689.png" alt="image-20210628202418689" style="zoom:67%;" />

图 18.6 中的电路采用了第 18.1.4 节中研究的类型的对称反馈限制器。 它由二极管 $D_1$ 和 $D_2$ 以及电阻器 $R_3、R_4、R_5$ 和 $R_6$ 形成。 限幅器的工作方式如下：在输出电压 $v_O$ 的正峰值处，节点 b 处的电压将超过电压 $v_1$（约为 $1×3v_O$），二极管 $D_2$ 导通。 这会将正峰值钳位到由 $R_5、R_6$ 和负电源确定的值。 具体来说，可以通过设置$v_b =v_1 +V_{D2}$ 并在节点b 写一个节点方程同时忽略通过$D_2$ 的电流来计算正输出峰值的值。

同样，输出正弦波的负峰值将被钳位到导致二极管 $D_1$ 导通的值。 负峰值的值可以通过设置 $v_a=v_o-V_1$ 来确定，同时我们在节点 a 写一个方程，忽略通过 $D_1$ 的电流。 最后，请注意，为了获得对称的输出波形，选择 $R_3$ 等于 $R_6$，$R_4$ 等于 $R_5$。

图 18.7 的电路采用了幅度控制的参数变化机制的廉价实现。 调整电位器 $P$ 直到振荡刚开始增长。 随着振荡的增加，二极管开始导通，导致 a 和 b 之间的有效电阻减小。 将在导致环路增益完全一致的输出幅度处达到平衡。 可以通过调节电位器 P 来改变输出幅度。

如图 18.7 所示，输出取自 b 点而不是运算放大器输出端，因为 b 处的信号失真比 a 处的低。 要理解这一点，请注意 b 处的电压与运算放大器输入端子上的电压成正比，后者是节点 a 处电压的滤波（通过 RC 网络）版本。 然而，节点 b 是一个高阻抗节点，如果要连接负载，则需要一个缓冲器。

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210628202704539.png" alt="image-20210628202704539" style="zoom:67%;" />

##### 18.2.2 The Phase-Shift Oscillator

### 18.4 Bistable Multivibrators

在本节中，我们开始研究另一种类型的波形生成电路——非线性振荡器或函数发生器。 这些设备使用称为多谐振荡器的一类特殊电路。 如前所述，多谐振荡器分为三类：双稳态、单稳态和非稳态。 本节讨论第一个，即双稳态多谐振荡器。顾名思义，双稳态多谐振荡器有两种稳定状态。 电路可以无限期地保持在任一稳定状态，只有在适当触发时才移动到另一个稳定状态。

##### 18.4.1 The Feedback Loop

通过在环路增益大于 1 的正反馈环路中连接直流放大器，可以获得双稳态。 这种反馈回路如图 18.19 所示； 它由一个运算放大器和一个正反馈路径中的电阻分压器组成。 要了解如何获得双稳态，请考虑在接近地电位的运算放大器的正输入端子下运行。 这是一个合理的起点，因为电路没有外部激励。

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210628203234867.png" alt="image-20210628203234867" style="zoom:67%;" />

假设每个电子电路中不可避免地存在电噪声会导致电压 $v_+$ 出现一个小的正增量。 该增量信号将被运算放大器的大开环增益 $A$ 放大，因此运算放大器的输出电压 $v_O$ 中将出现更大的信号。 分压器 ($R_1$, $R_2$) 将输出信号的一小部分 $β≡R_1/(R_1 +R_2)$ 反馈到运算放大器的正输入端。 如果 $Aβ$ 大于 1，通常情况下，反馈信号将大于 $v_+$ 中的原始增量。 这种再生过程一直持续到运算放大器最终饱和，其输出电压处于正饱和电平 $L_+$。 发生这种情况时，正输入端子 $v_+$ 上的电压变为 $L_+R_1/(R_1 +R_2)$，为正，从而使运算放大器保持在正饱和状态。 这是电路的两种稳定状态之一。

在上面的描述中，我们假设当 $v_+$ 接近零伏时，$v_+$ 出现正增量。 如果我们假设同样可能出现负增量的情况，运算放大器最终会在负方向饱和，$v_O =L_−R_1/(R_1 +R_2)。$ 这是另一种稳定状态。

因此我们得出结论，图 18.19 的电路有两种稳定状态，一种是运放处于正饱和状态，另一种是运放处于负饱和状态。 电路可以无限期地以这两种状态中的任何一种存在。 我们还注意到，电路不能在 $v_+ = 0$ 和 $v_O = 0$ 的状态下存在任何时间长度。 这是一种不稳定平衡状态（也称为亚稳态）； 任何干扰，例如由电噪声引起的干扰，都会导致双稳态电路切换到其两个稳定状态之一。 这与反馈为负的情况形成鲜明对比，负反馈导致运算放大器的输入端子之间出现虚拟短路，并在遇到干扰时保持这种虚拟短路。

双稳态电路操作的物理类比如图 18.20 所示。

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210628203657939.png" alt="image-20210628203657939" style="zoom:67%;" />

##### 18.4.2 Transfer Characteristic of the Bistable Circuit

问题自然会出现，我们如何才能使图 18.19 的双稳态电路改变状态？为了帮助回答这个关键问题，我们推导出双稳态电路的传输特性。 参考图 18.19 表明接地的两个节点中的任何一个都可以作为输入端。 我们调查这两种可能性。

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210628203945251.png" alt="image-20210628203945251" style="zoom:67%;" />

图 18.21(a) 显示了将电压 $v_I$ 施加到运算放大器的反相输入端子的双稳态电路。 为了推导出传输特性 $v_O–v_I$，假设 $v_O$ 处于其两个可能的电平之一，比如 $L_+$，因此 $v_+ =βL_+$。 现在，当 $v_I$ 从 $0 V$ 增加时，我们可以从电路中看到，在 $v_I$ 达到等于 $v_+$（即 $βL_+$）的值之前什么都不会发生。

而当 $v_I$ 开始超过此值时，运算放大器的输入端子之间会产生净负电压。 该电压被运算放大器的开环增益放大，因此 $v_O$ 变为负值。 分压器反过来使 $v_+$ 变为负值，从而增加运算放大器的净负输入并保持再生过程继续进行。 这个过程最终导致运算放大器在负方向饱和：即 $v_O = L_−$，相应地，$v_+ = βL_−$。 很容易看出，进一步增加 $v_I$ 对双稳态电路的获得状态没有影响。 图 18.21(b) 显示了增加 $v_I$ 的传递特性。 观察到该特性是具有表示为 $V_{TH}$ 的阈值电压的比较器的特性，其中 $V_{TH} = βL_+$。

接下来考虑当 $v_I$ 减少时会发生什么。 由于现在 $v_+ =βL_−$，我们看到电路保持在负饱和状态，直到 $v_I$ 变为负值，达到等于 $βL_−$ 的点。 当 $v_I$ 低于该值时，运算放大器的输入端子之间会出现净正电压。 该电压被运算放大器增益放大，从而在运算放大器的输出端产生正电压。 然后正反馈回路的再生作用开始并导致电路最终进入其正饱和状态，其中 $v_O =L_+$ 和 $v_+ =βL_+$。 降低 $v_I$ 的传递特性如图 18.21(c) 所示。 在这里，我们再次观察到该特性是比较器的特性，但具有阈值电压 $V_{TL} =βL_-$。

图 18.21(a) 电路的完整传输特性 $v_O -v_I$ 可以通过组合图 18.21(b) 和 (c) 中的特性获得，如图 18.21(d) 所示。 如图所示，电路在不同的 $v_I$ 值处改变状态，这取决于 $v_I$ 是增加还是减少。 因此，该电路表现出滞后现象； 滞后的宽度是高阈值 $V_{TH}$ 和低阈值 $V_{TL}$ 之间的差值。 另请注意，双稳态电路实际上是一个具有迟滞的比较器。 正如稍后将展示的，在某些应用中为比较器的特性增加迟滞可能非常有益。 最后，请注意，由于图 18.21 的双稳态电路在 $v_I$ 增加超过正阈值 $V_{TH}$ 时从正状态 ($v_O = L_+$) 切换到负状态 ($v_O = L_-$)，因此该电路是反相的。 稍后将介绍具有同相传输特性的双稳态电路。

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210628204842786.png" alt="image-20210628204842786" style="zoom:67%;" />

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210628204853091.png" alt="image-20210628204853091" style="zoom:67%;" />

##### 18.4.3 Triggering the Bistable Circuit

现在回到如何使双稳态电路改变状态的问题，我们从图 18.21(d) 的转移特性中观察到，如果电路处于 $L_+$ 状态，则可以通过施加输入使 $v_I$ 的值大于 $V_{TH} ≡ βL_+$，从而将其切换到 $L_−$ 状态。 这种输入会导致运算放大器的输入端子之间出现净负电压，从而启动再生循环，最终电路切换到 $L_-$ 稳定状态。

这里重要的是要注意输入 $v_I$ 仅启动或触发再生。 因此，我们可以在不影响再生过程的情况下移除 $v_I$。 换句话说，vI 可以只是一个短持续时间的脉冲。 输入信号$v_1$因此被称为触发信号，或简称为触发。

图 18.21(d) 的特性还表明双稳态电路可以通过施加幅度大于负阈值 $V_{TL}$ 的负触发信号 $v_I$ 切换到正状态 ($v_O = L_+)$。

##### 18.4.4 The Bistable Circuit as a Memory Element

我们从图 18.21(d) 中观察到，对于 $V_{TL} < v_I < V_{TH}$ 范围内的输入电压，输出可以是 $L_+$ 或 $L_-$，这取决于电路已经处于的状态。因此，对于这个输入范围，输出由触发信号的前一个值（导致电路处于当前状态的触发信号）决定。 因此该电路表现出记忆力。 事实上，双稳态多谐振荡器是数字系统的基本存储元件，正如我们在第 16 章中看到的那样。 最后，请注意，在模拟电路应用中，例如本章我们关心的那些，双稳态电路也被称作施密特触发器。

##### 18.4.5 A Bistable Circuit with Noninverting Transfer Characteristic

通过将输入信号 vI（触发信号）施加到接地的 R1 端子，可以使用图 18.19 的基本双稳态反馈环路推导出具有同相传输特性的电路。 结果电路如图 18.22(a) 所示。 为了获得传递特性，我们首先对由 R1 和 R2 形成的线性电路进行叠加，从而将 v+ 用 vI 和 vO 表示为

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210628205744935.png" alt="image-20210628205744935" style="zoom:67%;" />

从这个方程我们看到，如果电路处于正稳定状态且 vO = L+，则 vI 的正值将不起作用。 要触发电路进入 L- 状态，必须使 vI 为负，并且其值必须使 v+ 减小到零以下。 因此，可以通过代入方程找到低阈值 VTL。 vO =L+，v+ =0，并且 vI =VTL。 结果是

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210628205917137.png" alt="image-20210628205917137" style="zoom:67%;" />

图 18.22(a) 电路的完整传输特性如图 18.22(b) 所示。

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210628210001870.png" alt="image-20210628210001870"  />

观察正触发信号 vI（值大于 VTH）导致电路切换到正状态（vO 从 L- 变为 L+）。 因此，该电路的传输特性是同相的。

##### 18.4.6 Application of the Bistable Circuit as a Comparator

比较器是一种模拟电路构建块，可用于各种应用，从检测相对于预设阈值的输入信号电平到模数 (A/D) 转换器的设计。 尽管人们通常认为比较器具有单个阈值 [见图 18.23(a)]，但在许多应用中为比较器特性添加迟滞是很有用的。 

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210628210140443.png" alt="image-20210628210140443"  />

如果这样做，比较器显示两个阈值，VTL 和 VTH，对称放置在所需的参考电平附近，如图 18.23（b）所示。 通常 VTH 和 VTL 之间有少量间隔，比如 100 mV。

为了证明滞后的必要性，我们考虑比较器的常见应用。假如需要设计一种检测和计数任意波形过零的电路。 可以使用阈值设置为 0 V 的比较器来实现这种功能。每次发生过零时，比较器都会在其输出端提供阶跃变化。 每个阶跃变化都可用于生成一个脉冲，并将这些脉冲馈送到计数器电路。

现在想象一下，如果正在处理的信号（通常确实有）叠加在其上的干扰，例如比信号频率高得多的频率，会发生什么。 因此，信号可能会在我们试图检测的每个零交叉点周围多次穿过零轴，如图 18.24 所示。 

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210628210437907.png" alt="image-20210628210437907" style="zoom:67%;" />

因此，比较器会在每次过零时多次更改状态，我们的计数显然会出错。 但是，如果我们知道干扰的预期峰峰值幅度，则可以通过在比较器特性中引入适当宽度的滞后来解决问题。 然后，如果输入信号的幅度增加，具有迟滞的比较器将保持低电平状态，直到输入电平超过高阈值 VTH。 随后，即使由于干扰信号降低到低于 VTH，比较器也将保持在高电平状态。 只有当输入信号降低到低于低阈值 VTL 时，比较器才会切换到低状态。

##### 18.4.7 Making the Output Levels More Precise

通过将运算放大器与限幅器电路级联，可以使双稳态电路的输出电平比运算放大器的饱和电压更精确（有关限幅器电路的讨论，请参见第 4.6 节）。 图 18.25 显示了两种这样的布置。

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210628210701885.png" alt="image-20210628210701885" style="zoom:67%;" />

### 18.5 Generation of Square and Triangular Waveforms Using Astable Multivibrators

通过安排一个双稳态多谐振荡器周期性地切换状态，可以产生方波。 这可以通过将双稳态多谐振荡器与反馈回路中的 RC 电路连接来实现，如图 18.26(a) 所示。 观察到双稳态多谐振荡器具有反相传输特性，因此可以使用图 18.21(a) 的电路实现。

这导致了图 18.26(b) 的电路。 我们将很快说明该电路没有稳定状态，因此被恰当地命名为非稳态多谐振荡器。

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210628210840759.png" alt="image-20210628210840759" style="zoom:67%;" />

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210628210909416.png" alt="image-20210628210909416"  />

在这一点上，我们想提醒读者一个重要的关系，我们将在以下几节中多次使用：电容器 C 通过一个电阻 R 充放电最终会得到一个电压 v（t）

<img src="C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210628211119198.png" alt="image-20210628211119198" style="zoom:67%;" />

其中 V0+ 是 t = 0+ 时的电压，τ = CR 是时间常数。

##### 18.5.1 Operation of the Astable Multivibrator

要了解非稳态多谐振荡器如何工作，请参阅图 18.26(b) 并让双稳态多谐振荡器的输出处于其两个可能的电平之一，例如 L+。 电容器 C 将通过电阻器 R 向该电平充电。因此，C 两端的电压（施加到运算放大器的负输入端子并因此表示为 v-）将以指数方式向 L+ 上升，时间常数 τ = CR。 同时，正输入端的电压
...
在整本书的各个点上，我们使用了一些不同的可能方法来表征线性双端口网络，本附录中提供了该主题的摘要

## C.1 Characterization of Linear Two-Port Networks

双端口网络（图 C.1）有四个端口变量：$V_1$、$I_1$、$V_2$ 和 $I_2$，如果双端口网络是线性的，我们可以使用**其中两个变量作为==激励变量==**，**另外两个作为==响应变量==**。 例如，网络可以被端口 1 的电压 $V_1$ 和端口 2 的电压 $V_2$ 激励，并且可以测量两个电流 $I_1$ 和 $I_2$ 来表示网络响应，<u>**在这种情况下，$V_1$ 和 $V_2$ 是==自变量==，$I_1$ 和 $I_2$ 是==因变量==，网络运行可以用两个方程来描述**</u>
$$
I_1 = y_{11}V_1 +y_{12}V_2\tag{C.1}
$$

$$
I_2 = y_{21}V_1 + y_{22}V_2\tag{C.2}
$$

这里，$y_{11}$、$y_{12}$、$y_{21}$ 和 $y_{22}$ 这四个参数是**==导纳==**<u>（即电阻的倒数，表示电流与电压的正比关系）</u>，它们的值完全表征了线性二端口网络。

根据四个端口变量中的哪两个用于表示网络激励，获得一组不同的方程（以及相应的一组不同的参数）来表征网络。 我们将介绍电子学中常用的四个参数集。

### C.1.1 y Parameters

短路导纳（或 $y$ 参数）表征基于以 $V_1$ 和 $V_2$ 为自变量的激发网络，如图 C.2(a) 所示，描述方程如 (C.1) 和 (C.2) 所示，四个导纳参数可以根据它们在方程式 (C.1) 和 (C.2) 中的作用来定义**（使用了叠加原理）**。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220426184314625.png" alt="image-20220426184314625" style="zoom:80%;" />

具体来说，从方程式 (C.1) 我们看到 $y_{11}$ 被定义为
$$
y_{11}=\frac{I_1}{V_1}\bigg|_{V_2=0}\tag{C.3}
$$
因此 $y_{11}$ 是端口 1 的**输入导纳**（此时假设端口 2 短路），这个定义如图 C.2(b) 所示，它还提供了一种测量输入短路导纳 $y_{11}$ 的概念方法。

$y_{12}$ 的定义可以从方程 (C.1) 得到。
$$
y_{12} = \frac{I_1} {V_2}\bigg| _{V_1=0}\tag{C.4}
$$
因此，$y_{12}$ 代表从端口 2 到端口 1 的传输，由于在放大器中，端口 1 代表输入端口，端口 2 代表输出端口，**因此 $y_{12}$ 代表网络中的内部反馈**，图 C.2(c) 说明了 $y_{12}$ 的定义和测量方法。

$y_{21}$ 的定义可以从方程 (C.2) 得到。
$$
y_{21} = \frac{I_2} {V_1}\bigg| _{V_2=0}\tag{C.5} 
$$
因此，$y_{21}$ 表示从端口 1 到端口 2 的传输，如果端口 1 是放大器的输入端口，端口 2 是输出端口，**则 $y_{21}$ 提供了正向增益或传输的度量，**图 C.2(d) 说明了 $y_{21}$ 的定义和测量方法。

参数 $y_{22}$ 可以根据等式 (C.2) 定义。
$$
y_{22} = \frac{I_2} {V_2}\bigg|_{V_1=0}\tag{C.6}
$$
因此 $y_{22}$ 是端口 1 短路时进入端口 2 的导纳。 对于放大器，$y_{22}$ 是输出短路导纳，图 C.2(e) 说明了 $y_{22}$ 的定义和测量方法。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220426185244468.png" alt="image-20220426185244468" style="zoom:67%;" />

### C.1.2 z

双端口网络的**开路阻抗**（或 $z$ 参数）表征基于 $I_1$ 和 $I_2$ 激励网络，如图 C.3(a) 所示。 描述方程是
$$
...
<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220426185548163.png" alt="image-20220426185548163" style="zoom:67%;" />

由于 $z$ 和 $y$ 参数表征之间的对偶性，我们将不详细讨论 $z$ 参数。 图 C.3 给出了四个 $z$ 参数的定义和测量方法。

### C.1.3 h

双端口网络的混合（或 $h$ 参数）表征基于 $I_1$ 和 $V_2$ 激励网络，如图 C.4(a) 所示（注意名称混合背后的原因），描述方程是
$$
...
从中可以得到四个 $h$ 参数的定义为
$$
h_{11} = \frac{V_1} {I_1}\bigg |_{V_2=0} \ ,\ h_{21} = \frac{I_2} {I_1} \bigg|_{V_2=0}
$$

$$
h_{12} = \frac{V_1} {V_2}\bigg| _{I_0=0}\ ,\  h_{22} = \frac{I_2} {V_2}\bigg|_ {I_1=0}
$$

因此，$h_{11}$ 是端口 1 的输入阻抗，此时端口 2 短路；参数 $h_{12}$ 代表网络的反向或反馈电压比，在输入端口开路时测量。 正向传输参数 $h_{21}$ 代表输出端口短路时网络的电流增益； 因此，$h_{21}$ 称为短路电流增益。 最后，$h_{22}$ 是输入端口开路的输出导纳。

$h$ 参数的定义和概念测量设置在图 C.4 中给出。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220426195038921.png" alt="image-20220426195038921" style="zoom:67%;" />

### C.1.4 g

双端口网络的逆混合（或 $g$ 参数）表征基于 $V_1$ 和 $I_2$ 对网络的激励，如图 C.5(a) 所示。 描述方程是

![image-20220426195120397](C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220426195120397.png)

图 C.5 给出了定义和概念测量设置。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220426195207596.png" alt="image-20220426195207596" style="zoom:67%;" />

### C.1.5 Equivalent-Circuit Representation

双端口网络可以由基于用于表征的参数集的等效电路来表示。 图 C.6 显示了对应于刚才讨论的四种参数类型的四种可能的等效电路。 这些等效电路中的每一个都是根据特定参数集描述网络的相应两个方程的直接图形表示。

最后，应该提到的是，还有其他参数集用于表征双端口网络，但本书没有讨论或使用这些参数集。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220426195320733.png" alt="image-20220426195320733" style="zoom:67%;" />

## EXERCISE 

C.1 图 EC.1 显示了晶体管的小信号等效电路模型，计算 h 参数的值

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220426195417536.png" alt="image-20220426195417536" style="zoom:67%;" />

答：$h_{11} \simeq2.6 k\Omega ;h_{12} \simeq2.5×10^{−4};h_{21}\simeq 100;h_{22} \simeq2×10^{−5}$

# APPENDIX D SOME USEFUL NETWORK THEOREMS

## Introduction

在本附录中，我们回顾了三个有助于简化电子电路分析的网络定理：Thévenin‘ s 定理、Norton 定理和源吸收定理。

## D.1 Thévenin‘ s Theorem

Thévenin 定理用于通过电压源 $V_t$ 和串联阻抗 $Z_t$ 来表示网络的一部分，如图 D.1 所示。 图 D.1(a) 显示了一个分为两部分的网络，A 和 B。在图 D.1(b) 中，网络的 A 部分已被其等效的 Thévenin 等效所取代：**电压源 $V_t$ 和 串联阻抗 $Z_t$** ，图 D.1(c) 说明了如何确定 $V_t$：只需**将网络 A 的==<u>两个端子开路</u>==并测量（或计算）==<u>出现在这两个端子之间的电压</u>==**；为了确定 $Z_t$，我们通过**<u>==短路电压源==</u>**和**==<u>开路电流源</u>==**将网络 A 中的**==<u>所有外部（即独立）源减少到零</u>==**，在进行了这种缩减之后，阻抗 $Z_t$ 将等于网络 A 的输入阻抗，如图 D.1(d) 所示。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220426200442071.png" alt="image-20220426200442071" style="zoom:67%;" />

## D.2 Norton's Theorem

诺顿定理是泰维宁定理的对偶，它用于通过电流源 $I_n$ 和并联阻抗 $Z_n$ 来表示网络的一部分，如图 D.2 所示。 图 D.2(a) 显示了一个分为两部分的网络，A 和 B。在图 D.2(b) 中，部分 A 已被诺顿等效物所取代：电流源 $I_n$ 和并联阻抗 $Z_n$，测量（或计算）诺顿的电流源 $I_n$ 的方法如图 D.2(c) 所示：被还原的网络（网络 A）的端子被短路，**电流 $I_n$ 将简单地等于短路电流**；为了确定阻抗 $Z_n$，我们首先将网络 A 中的**外部激励减小到零**：也就是说，我们将**<u>独立电压源短路和开路独立电流源</u>**，在此源消除过程发生后，阻抗 $Z_n$ 将等于网络 A 的输入阻抗。 因此，诺顿阻抗 $Z_n$ 等于 Thévenin 阻抗 $Z_t$ 。 最后，请注意 $I_n = V_t/Z$，其中 $Z = Z_n = Z_t$。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220426200821416.png" alt="image-20220426200821416" style="zoom:67%;" />

## Example D.1

图 D.3(a) 显示了一个双极结型晶体管电路，晶体管是一个三端器件，其端子标记为 E（发射极）、B（基极）和 C（集电极）。 如图所示，基极通过 $R_1$ 和 $R_2$ 组成的分压器连接到直流电源 $V^+$，集电极通过 $R_3$ 连接到直流电源 $V^+$，并通过 $R_4$ 接地,为了简化分析，我们希望应用 Thévenin 定理来简化电路。

![image-20220426201000934](C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220426201000934.png)

**Solution**

Thévenin 定理可用于基极侧，将由 $V^+$、$R_1$ 和 $R_2$ 组成的网络简化为直流电压源 $V_{BB}$，
$$
...
R_B = R_1|| R_2
$$
其中“||” 表示“并联”，在集电极侧，可以应用 Thévenin 定理将由 $V^+$、$R_3$ 和 $R_4$ 组成的网络简化为直流电压源 $V_{CC}$，
$$
...
考虑图 D.4 所示的情况，在分析网络的过程中，我们发现一个受控电流源 $I_x$ 出现在两个节点之间，其电压差就是控制电压 $V_x$，也就是说，$I_x =g_mV_x$，其中 $g_m$ 是电导，我们可以用阻抗 $Z_x = V_x/I_x = 1/g_m$ 代替这个受控源，如图 D.4 所示，因为这个阻抗吸收的电流将等于我们已更换的受控源的电流。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220426201652314.png" alt="image-20220426201652314" style="zoom:67%;" />

## Example D.2

图 D.5(a) 显示了晶体管的小信号等效电路模型。我们想要找到“观察”发射极端子 E 的电阻 $R_{in}$ - 即发射极和地之间的电阻 - 与基极 B 和集电极 C 接地

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220426202641773.png" alt="image-20220426202641773" style="zoom:67%;" />

**Solution**

从图 D.5(a)，我们看到电压 $v_π$ 将等于 $-v_e$。 因此，在 E 和地之间，我们看到电阻 $r_π$ 与电流源并联，从端子 E 汲取电流 $g_mv_e$，后一个源可以用电阻 $(1/g_m)$ 代替，从而得到给定的输入电阻 $R_{in}$ 
$$
R_{in} = r_π ||(1 / g_m)
$$
如图 D.5(b) 所示。
...
单时间常数 (STC) 电路是由或可以简化为一个电抗元件（电感或电容）和一个电阻的电路。 由电感 $L$ 和电阻 $R$ 构成的 STC 电路具有时间常数 $τ = L/R$；由电容 $C$ 和电阻 $R$ 组成的 STC 电路的时间常数 $τ$ 由 $τ = CR$ 给出。

虽然 STC 电路相当简单，但它们在线性和数字电路的设计和分析中发挥着重要作用，例如，**<u>放大器电路的分析通常可以简化为一个或多个 STC 电路的分析</u>**，出于这个原因，我们将在本附录中回顾评估 STC 电路对正弦和其他输入信号（如阶跃和脉冲波形）的响应的过程，后一种信号波形在某些放大器应用中会遇到，但在包括数字电路在内的开关电路中更为重要。

## E.1 Evaluating the Time Constant

分析 STC 电路的第一步是计算其时间常数 $τ$。

### Example E.1

将图 E.1(a) 中的电路简化为 STC 电路，求其时间常数。

**Solution**

归约过程如图 E.1 所示，由戴维宁定理的重复应用组成，从最终电路（图 E.1c），我们得到时间常数为
$$
τ = C \{ R_4 ||[R_3 + (R_1 ||R_2)]\}
$$
<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220426203226177.png" alt="image-20220426203226177" style="zoom:67%;" />

### E.1.1 Rapid Evaluation of $\tau$ 

在许多情况下，重要的是能够快速计算给定 STC 电路的时间常数 $τ$，实现这一目标的一个简单方法是**==首先将激励（外部源）降低到零==**； 也就是说，**==<u>如果激励来自电压源，则将其短路，如果激励来自电流源，则将其断开</u>==**。 然后，如果电路具有一个电抗元件和多个电阻，则“抓住”电抗元件的两个端子（电容或电感）并找到该元件看到的等效电阻 $R_{eq}$。 时间常数则为 $L/R_{eq}$ 或 $CR_{eq}$，例如，在图 E.1(a) 的电路中，我们发现电容器 $C$ “看到”电阻 $R_4$ 与 $R_3$ 和 $R_2$ 的串联组合与 $R_1$ 并联，因此
$$
R_{eq}=\{ R_4 ||[R_3 + (R_1 ||R_2)]\}
$$
时间常数为 $CR_{eq}$。

在某些情况下，可能会发现电路具有一个电阻和多个电容或电感，在这种情况下，程序应该颠倒过来； 也就是说，**“抓住”电阻端子并找到该电阻所见的等效电容 $C_{eq}$ 或等效电感 $L_{eq}$，然后发现时间常数为 $C_{eq}R$ 或 $L_{eq}/R$**，这在示例 E.2 中进行了说明。

### Example E.2

求图 E.2 中电路的时间常数。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220426210238791.png" alt="image-20220426210238791" style="zoom:67%;" />

**Solution**

通过**短路电压源将激励降低到零后**，我们看到电阻 $R$ “看到”了等效电容 $C_1 +C_2$ （电容并联），因此，时间常数 $τ$ 由下式给出
$$
τ = (C_1 +C_2) R
$$
最后，在某些情况下，STC 电路具有不止一个电阻和不止一个电容（或不止一个电感），这种情况需要一些初步工作来简化电路，如示例 E.3 所示。

### Example E.3

在这里，我们表明可以使用 STC 电路分析方法获得图 E.3(a) 中电路的响应。

**Solution**

分析步骤如图 E.3 所示。 在图 E.3(b) 中，我们展示了由两个独立但相等的电压源激励的电路。 读者应该说服自己相信图 E.3(a) 和 E.3(b) 中的电路是等价的，用于获得图 E.3(b) 中排列的“技巧”是一个非常有用的技巧。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220426210610309.png" alt="image-20220426210610309" style="zoom:67%;" />

将 Thévenin 定理应用于 $XX'$ 线左侧的电路，然后应用于该线右侧的电路，得到图 E.3(c) 的电路，**==<u>由于这是一个线性电路，因此可以使用叠加原理获得响应</u>==**。 具体来说，**==<u>输出电压 $v_O$ 将是两个分量 $v_{O1}$ 和 $v_{O2}$ 的总和</u>==**，<u>**第一个分量 $v_{O1}$ 是左侧电压源的输出，而另一个电压源则降至零**</u>，计算 $v_{O1}$ 的电路如图 E.3(d) 所示，它是一个 STC 电路，其时间常数为
$$
τ = (C_1 +C_2)(R_1||R_2)
$$
类似地，**<u>第二个分量 $v_{O2}$ 是左侧电压源减小到零时获得的输出</u>**，它可以从图 E.3(e) 的电路中计算出来，它是一个具有相同时间常数 $τ$ 的 STC 电路。

最后，应该观察到电路是 STC 电路的事实也可以通过将图 E.3(a) 中的独立源 $v_i$ 设置为零来确定。 此外，时间常数立即显而易见。

## E.2 Classification of STC Circuits

STC 电路可分为两类，**<u>低通 (LP) 和高通 (HP) 类型</u>**，每个类别显示明显不同的信号响应，确定 STC 电路是 LP 还是 HP 类型的任务可以通过多种方式完成，其中**<u>最简单的方法是使用频域响应</u>**，具体来说，低通电路通过直流（即频率为零的信号）并衰减高频，在 $ω=∞$ 时传输为零； 因此，**我们可以在 $ω = 0$ 或 $ω=∞$ 时测试电路类型，在 $ω = 0$ 时，电容器应替换为开路** ($1/jωC=∞$)，**电感器应替换为短路** ($jωL = 0$)，那么**<u>==如果输出为零，则电路为高通型==</u>**，而**==<u>如果输出为有限，则电路为低通型</u>==**，或者，我们可以通过将电容器替换为短路 ($1/jωC = 0$) 和电感器替换为开路 ($jωL=∞$) 来测试 $ω=∞$，那么**==<u>如果输出是有限的，则电路属于 HP 类型</u>==**，而**==<u>如果输出为零，则电路属于 LP 类型</u>==**，在表 E.1 中，提供了这些结果的总结，s.c. 代表短路和 o.c. 为开路。

图 E.4 显示了低通 STC 电路的示例，图 E.5 显示了高通 STC 电路的示例。 对于每个电路，我们已经指出了感兴趣的输入和输出变量。 请注意，给定的电路可以属于任一类别，具体取决于输入和输出变量。 敦促读者使用表 E.1 的规则来验证图 E.4 和 E.5 分类正确。

![image-20220426211624205](C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220426211624205.png)

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220426211742448.png" alt="image-20220426211742448" style="zoom:67%;" />

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220426211803652.png" alt="image-20220426211803652" style="zoom:67%;" />

## E.3 Frequency Response of STC Circuits

### E.3.1 Low-Pass Circuits

STC 低通电路的传递函数 $T(s)$ 总是可以写成以下形式
$$
T(s) = \frac{K} {1+(s/ω_0)}\tag{E.1}
$$
其中，对于物理频率，其中 $s = jω$，变为
$$
T (jω) = \frac K {1 + j (ω / ω_0)}\tag{E.2}
$$
其中 $K$ 是 $ω = 0$ (dc) 时传递函数的幅度，$ω_0$ 定义为
$$
ω_0 = 1 / τ
$$
$τ$ 为时间常数，因此幅度响应由下式给出
$$
|T( jω)| = \frac K {\sqrt{1+(ω/ω_0)^2}}\tag{E.3}
$$
相位响应由下式给出
$$
φ (ω) = - \tan ^{- 1} (ω / ω_0)\tag{E.4}
$$
图 E.6 描绘了 STC 低通电路的幅度和相位响应，图 E.6(a) 中所示的幅度响应是方程（E.3）中函数的图，幅度相对于直流增益 $K$ 进行了归一化，并以分贝表示，也就是说，该图适用于 $20 \log|T(jω)/K|$，频率轴使用对数刻度，此外，频率变量已相对于 $ω_0$ 进行了归一化，如图所示，幅度曲线由两条直线渐近线紧密定义，低频渐近线是 0 dB 处的水平直线。 要找到高频渐近线的斜率，请考虑方程式 (E.3) 并设 $ω/ω_0\gg 1$，导致
$$
|T( jω)|\simeq K \frac{ω_0} ω
$$
因此，如果 $ω$ 值加倍，则幅度减半，在对数频率轴上，$ω$ 的倍增表示等间隔的点，每个间隔称为八度音程，幅度函数减半对应于传输减少 6 dB (20 log 0.5=–6 dB)。因此，高频渐近线的斜率为 –6 dB/倍频程。 这可以等效地表示为 –20 dB/decade，其中“decade”表示频率增加了 10 倍。

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220426212552884.png" alt="image-20220426212552884" style="zoom:67%;" />

**幅值-响应曲线的两条==直线渐近线在“拐角频率”或“中断频率” $ω_0$ 处相交==**，实际幅度响应曲线和渐近响应之间的差异在拐角频率处最大，其值为 3 dB，要验证该值是否正确，只需将 $ω = ω_0$ 代入方程式 (E.3) 得到 
$$
|T(jω_0)|  = K/ \sqrt2
$$
因此，在 $ω = ω_0$ 时，增益相对于直流增益下降 $\sqrt2$ 倍，**==<u>这对应于增益降低 3-dB。 拐角频率 $ω_0$ 被适当地称为 3-dB 频率。</u>==**

与幅度响应类似，相位响应曲线，如图 E.6(b) 所示，由直线渐近线紧密定义，请注意，在拐角频率处，相位为 –45°，而对于 $ω\gg ω_0$，相位接近 –90°。 另请注意，在 $0.1ω_0$ 到 $10ω_0$ 的频率范围内，–45°/decade 直线近似于相位函数，最大误差为 5.7°

### Example E.4

考虑图 E.7(a) 所示的电路，其中增益 $μ=-100$ 的理想电压放大器在其反馈路径中连接了一个小 ($10-pF$) 电容，放大器由源电阻为 100 k 的电压源供电，证明此放大器的频率响应 $V_o/V_s$ 与 STC 电路等效，并画出幅值响应

<img src="C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20220426213133578.png" alt="image-20220426213133578" style="zoom:67%;" />

**Solution**

直接分析图 E.7(a) 中的电路得到传递函数
$$
\frac{V_o}{V_s}= \fracμ{1+sRC_f (−μ+1)}
$$
这可以看作是具有直流增益 $μ=-100$（或等效地，40 dB）和时间常数 $τ = RC_f (-μ+1) = 100×10^3 ×10×10^{−12} ×101\simeq 10^{−4} s$ 的低通 STC 电路的电路，对应于频率 $ω_0 = 1/τ = 10^4 rad/s$，幅值响应在图 E.7(b) 中画出。

### E.3.2 High-Pass Circuits

STC 高通电路的传递函数 $T(s)$ 总是可以表示为
$$
T(s) = \frac{Ks} {s+ω_0}\tag{E.5}
$$
对于物理频率 $s = jω$ 变为
$$
T (jω) = \frac K {1 - jω_0 / ω}\tag{E.6}
$$
其中 $K$ 表示当 $s$ 或 $ω$ 接近无穷大时的增益，$ω_0$ 是时间常数 $τ$ 的倒数，
$$
ω_0 = 1 / τ
$$
幅度响应
$$
|T( jω)| = \frac K {\sqrt{1+(ω/ω_0)^2}}\tag{E.7}
$$
和相位响应
$$
φ (ω) = - \tan ^{- 1} (ω / ω_0) \tag {E.8}
$$
如图 E.8 所示，与低通情况一样，幅度和相位曲线由直线渐近线很好地定义，由于与低通情况的相似性（或更恰当地，对偶性），将不再给出进一步的解释。
