...
A(s)=A_MF_H(s)
$$
其中$A_M$是中频带增益（对于集成放大电路而言，这个也是其低频时的放大增益）可以通过分析放大器等效电路并且忽略晶体管内部电容的影响来确定$A_M$的值(即视其为)。 而通过考虑这些电容，我们可以得到$F_H(s)$，该数值可以用其极点和零点（通常是实数）表示，如下所示

![image-20210516185709560](C:\Users\Lenovo\AppData\Roaming\Typora\typora-user-images\image-20210516185709560.png)

其中$ω_{p1}$，$ω_{p2}$......$ω_{pn}$是指n个实极的频率，ωZ2，ωZn是表示n个实传输零点频率的正数、负数或无穷数。由于零点和极点的频率根据定义大于中间带频率，我们从等式（10.71）中可以看出，当s接近中间带频率时，FH（s）接近单位，增益接近AM。
